...
PID 2 首次收到 `policy-applied` 后会连接 AF_UNIX stream，通过普通异步 `write/read` 完成 registry globals、bufferless initial commit、shm/xdg configure，再用 `memfd_create/ftruncate/mmap` 和 `sendmsg(SCM_RIGHTS)` 提交 `ack_configure` 与首个 XRGB8888 buffer；随后只修改共享页，复用已 release 的 buffer 与已 delete-id 的 callback ID完成第二轮 attach/damage/frame/commit/presentation。`slopos-wayland` dispatcher校验对象关系、批次状态、configure serial、buffer geometry、full damage、frame callback、atomic commit、首次 pool descriptor和每轮 callback retirement，`wayland_service.rs` 以双 bank 发布 generation 1/2 给 desktop task。当前 IPC 边界仍缺用户态 listener、普通第三方 client、多 client ownership、通用 ancillary data与持续事件循环；mmap也只支持一个固定地址、单页 shared memfd，没有 munmap。
...
`crates/elf` 是无分配 `no_std` ELF64 parser；`crates/process` 提供固定容量 process/fd 生命周期。`userspace/init` 与 `userspace/desktop` 都是固定在 `0x40000000` 的独立 Rust executable；release artifacts 分别为 26344/36840 bytes，R+X `PT_LOAD` 为 2608/10392 bytes。rootfs builder安装为 inode 23/24；block task分别跨七/九块读入，只有 init 与 ESP 副本逐字节比较。kernel 为每个进程构造三页 code、三页 stack、Linux initial stack 与独立 CR3；PID 2 还可把一个 shared-memory frame映射到紧邻 stack top 的 `0x40006000`，descriptor、mapping与 socket receiver各自持有显式引用。PID 1 最终常驻 `wait4`，PID 2 完成 AF_UNIX Wayland registry/configure/ack 与两轮 presentation 后常驻 config event。完整边界见 [processes.md](processes.md)。
