...
`crates/ipc` 提供固定容量 AF_UNIX local-stream namespace、双向 byte ring、whole-batch exact send，以及两个方向上与一批 bytes 原子关联的单个 generation-checked rights object；process/VFS descriptor object 可保存 file、socket、shared-memory或desktop-event handle。kernel 在 `/run/slopos/wayland-0` 建立 listener，Linux `socket/connect` 建立 PID 2 client/server endpoint，异步 `read/write/sendmsg/recvmsg/poll/close` completion 按 descriptor type 分派。`userspace/desktop` 以固定 256-byte buffer读取配置并提交 policy；首代 apply 后，它经socket发出12-byte registry request，随后把308-byte compositor/shm/xdg/seat/output/pointer/keyboard initial batch以256+52两次write连续发送。288-byte configure wire的前256 bytes经`recvmsg`与一个只读XKB keymap fd一起返回，余下32 bytes经普通`read`返回；desktop分块读取3307-byte keymap并校验hash/NUL/EOF。随后建立3072-byte memfd、把其一页 backing 以 `MAP_SHARED` 映射到 `0x40007000`，并用 `sendmsg(SCM_RIGHTS)` 发送148-byte configured batch与fd。服务端保留同一对象，第二个64-byte batch只经普通`write`复用pool/buffer和更新后的共享页；两轮presentation各返回release/done/delete-id共32 bytes。初始config到达后，PID 2打开`/run/slopos/desktop-events`并用poll同时等待event fd与socket；`kernel/wayland_service.rs`按真实compositor焦点和surface命中持续发布keyboard enter/leave/key/modifiers及pointer enter/leave/motion/button/axis/frame，client以持久stream buffer处理fragmented/coalesced wire。`crates/desktop-protocol` 仍保留旧 `0x534c0003/0004` 信封 decoder兼容测试，但运行 wire 不走该路径；私有 `0x534c0005` staging ABI 已删除。QEMU marker 证明 socket/connect、seat/output/pointer/keyboard discovery、server→client keymap、client→server surface backing、buffer reuse、动态键鼠event、poll suspend/resume及generation 1→2 renderer acknowledge；`make test-wayland-input`还对第二帧四象限图案取样并生成截图。
...
`crates/elf` 是无分配 `no_std` ELF64 parser；`crates/process` 提供固定容量 process/fd 生命周期。`userspace/init` 与 `userspace/desktop` 都是固定在 `0x40000000` 的独立 Rust executable；release artifacts 分别为26344/40960 bytes，R+X `PT_LOAD` 为2608/15072 bytes。rootfs builder对desktop执行strip，再补非alloc确定性padding以维持十个ext4 block的固定布局；它们安装为inode 23/24，block task分别跨七/十块读入，只有init与ESP副本逐字节比较。kernel为每个进程构造四页code、三页stack、Linux initial stack与独立CR3；PID 2还可把一个shared-memory frame映射到紧邻stack top的`0x40007000`，descriptor、mapping与socket receiver各自持有显式引用。PID 1最终常驻`wait4`，PID 2完成AF_UNIX Wayland registry、seat/output/pointer/keyboard discovery、keymap导入、configure/ack与两轮presentation后，常驻Linux poll并同时等待Wayland socket和config event descriptor。完整边界见 [processes.md](processes.md)。
...
`crates/ipc` 提供独立、无分配的具名 local byte-stream core。固定 socket table 以 generation handle 防止 slot 复用后的 stale reference；listener 保存有界 FIFO backlog，每条 accepted connection 由成对 endpoint 和各自 receive ring 构成。`send` 只复制到 peer ring 的可用部分并以 `WouldBlock` 表示 backpressure，输入event使用的`send_exact`则在容量不足时拒绝整批，保证不留下半个协议frame；`recv` 在 peer close 后排空尾部再返回 EOF；`send_with_rights/recv_with_rights` 还把一个 generation-checked object与同批bytes原子关联，并拒绝覆盖未消费rights。6项宿主测试覆盖双向分段读取、backlog saturation、满ring/恢复writable、exact-send framing、close/EOF、duplicate path、stale handle与两个方向的rights transfer。kernel已把它接入process descriptor和block-task executor；PID 2可调用Linux `socket/connect/read/write/sendmsg/recvmsg/poll/close`，其中`sendmsg(SCM_RIGHTS)`把surface backing交给Wayland listener，`recvmsg(SCM_RIGHTS)`接收server只读keymap object，poll把socket readiness与config event合并进一个用户态loop。当前尚未向用户态暴露 `bind/listen/accept`，也没有通用control-message队列、credentials、epoll或多client service ownership。
