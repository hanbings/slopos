...
`crates/elf` 是无分配 `no_std` ELF64 parser；`crates/process` 提供固定容量 process/fd 生命周期。`userspace/init` 与 `userspace/desktop` 都是固定在 `0x40000000` 的独立 Rust executable；release artifacts 分别为 26344/37296 bytes，R+X `PT_LOAD` 为 2608/10184 bytes。rootfs builder安装为 inode 23/24；block task分别跨七/十块读入，只有 init 与 ESP 副本逐字节比较。kernel 为每个进程构造三页 code、三页 stack、Linux initial stack 与独立 CR3。PID 1 最终常驻 `wait4`，PID 2 完成 Wayland registry/configure/ack/presentation 往返后常驻 config event。完整边界见 [processes.md](processes.md)。
