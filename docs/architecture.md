...
`interrupts.rs` 安装包含 kernel/user code/data 与 64-bit TSS 的 GDT、加载 task register、安装 IDT、配置 100 Hz PIT，并为 timer、keyboard、mouse、APIC spurious 及关键 CPU exception 安装 gate。TSS 的 `RSP0` 指向专用 16 KiB privilege stack，供 CPL3 hardware interrupt/exception 切栈；IDT 不再暴露 DPL3 vector `0x80`。IRQ 汇编 stub 保存上下文、对齐栈并调用有界 Rust top half。`process.rs` 则检查 CPUID SYSCALL bit，配置并读回 `EFER.SCE`、`STAR`、`LSTAR`、`FMASK`；fast entry 自行从 user RSP 切到暂停中的 kernel continuation stack，保存 15 个 GPR 与 RCX/R11/RSP。stdout write 可直接 `SYSRETQ`；`openat/read/close` 会复制 frame、返回 kernel CR3/stack，让 block Future 等待 virtio IRQ，完成后经独立 resume entry 恢复 process CR3、通用寄存器、RIP/RFLAGS/RSP 并 `SYSRETQ`。PS/2 top half 读取一个字节、确认 local APIC 并写入固定 SPSC ring；`desktop` future 负责扫描码和 mouse packet 的复杂解析。独立测试会在 PID 1 退出后访问 kernel CR3 中未映射的 1 GiB 地址，实际验证 page-fault vector、error、RIP 和 CR2。

`crates/elf` 是无分配 `no_std` ELF64 parser。它只接受 little-endian x86-64 `ET_EXEC`，有界解析 program-header table；对每个 `PT_LOAD` 校验 `p_filesz <= p_memsz`、file/address overflow、canonical address、2 次幂 alignment 与 offset/vaddr congruence，拒绝空、重叠和 W+X segment，并要求 entry 位于 executable segment。`crates/process` 提供容量参数化的 PID/parent、image context、`Ready/Running/Exited` 生命周期、exit/syscall accounting、reap 和每 slot 独立 VFS fd table；4 项测试覆盖状态与 descriptor isolation。`userspace/init` 是独立 Rust `no_std` executable，linker 把它放到 `0x40000000`；release artifact 为 22928 bytes，其中一个 R+X `PT_LOAD` 从 file offset `0x1000` 装入 1008 bytes。rootfs builder 将 artifact 安装为 `/sbin/slop-init`；ext4 mount/recovery 后，block task 经 path walker 从 inode 23 跨六个逻辑块读入它，并与 UEFI loader 通过 BootInfo v2 保留的 ESP `LOADER_DATA` 副本逐字节比较。实际传给 `process.rs` 的 bytes 来自 root VFS；它解析/copy segment、插入 PID 1，在独立 CR3 下以 `CS=0x23`、`SS=0x1b` 和 `IRETQ` 从 ELF entry 进入 CPL3。程序按 Linux x86-64 寄存器/编号约定以 `SYSCALL` 依次执行只读 `openat/read/close`、stdout `write` 与 `exit`；前三者通过保存 frame 把同步用户调用转为 block task 的异步 ext4/virtio completion，再恢复原用户上下文。完整边界见 [processes.md](processes.md)。
