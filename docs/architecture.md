...
`crates/elf` 是无分配 `no_std` ELF64 parser。它只接受 little-endian x86-64 `ET_EXEC`，有界解析 program-header table；对每个 `PT_LOAD` 校验 `p_filesz <= p_memsz`、file/address overflow、canonical address、2 次幂 alignment 与 offset/vaddr congruence，拒绝空、重叠和 W+X segment，并要求 entry 位于 executable segment。`crates/process` 提供容量参数化的 PID/parent、image/initial-RSP context、`Ready/Running/Exited` 生命周期、exit/syscall accounting、Linux initial-stack builder、reap 和每 slot 独立 VFS fd table；6 项测试覆盖 initial stack、状态与 descriptor isolation/seek。`userspace/init` 是独立 Rust `no_std` executable，linker 把它放到 `0x40000000`；release artifact 为 25896 bytes，其中一个 R+X `PT_LOAD` 从 file offset `0x1000` 装入 2160 bytes。rootfs builder 将 artifact 安装为 `/sbin/slop-init`；ext4 mount/recovery 后，block task 经 path walker 从 inode 23 跨七个逻辑块读入它，并与 UEFI loader 通过 BootInfo v2 保留的 ESP `LOADER_DATA` 副本逐字节比较。kernel 在 stack page 构造 `argc=2`、两项 `argv`、三项 `envp` 和 9 对 auxv；用户 entry assembly 把原始 `RSP` 传给 Rust，PID 1 在 syscall 前逐项核对。程序随后按 Linux x86-64 寄存器/编号约定执行 `O_RDONLY/O_RDWR openat`、read/write/lseek/close、stdout write 与 exit；需要 I/O 的同步调用通过保存 frame 转为 block task 的异步 ext4/virtio completion。完整边界见 [processes.md](processes.md)。
...
`crates/vfs` 是无分配、无标准库的 namespace 状态机：绝对路径最多 16 个 component，mount table 采用最长 component-prefix，fd table 从 3 开始分配并维护 vnode、size、offset 与 read/write access mode。内核把 ext4 注册为 filesystem 1 并挂到 `/`；启动验证先经 path walker 跨七块读取 inode 23 的 `/sbin/slop-init`。PID 1 随后用 Linux `openat/read/close` 异步读出 inode 18 的 76 bytes，再用 `O_RDWR openat`、lseek、异步 write/read 对 inode 31 的 offset 123 执行可逆 16-byte patch。kernel probe 的独立 block-task table 还会以五个 chunk/seek 读取 inode 18，并对 inode 31 写入/恢复 73 bytes。桌面配置候选也复用同一 ext4 walker。进程 fd 已接入有界读写 syscall，但 mount table、`Ext4File` backing slots 与其他 probe fd 仍由单一 block task 专用，不是并发全局 POSIX VFS。
