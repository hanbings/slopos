...
`crates/elf` 是无分配 `no_std` ELF64 parser。它只接受 little-endian x86-64 `ET_EXEC`，有界解析 program-header table；对每个 `PT_LOAD` 校验 `p_filesz <= p_memsz`、file/address overflow、canonical address、2 次幂 alignment 与 offset/vaddr congruence，拒绝空、重叠和 W+X segment，并要求 entry 位于 executable segment。`crates/process` 提供容量参数化的 PID/parent、image/initial-RSP context、`Ready/Running/Exited` 生命周期、exit/syscall accounting、Linux initial-stack builder、reap 和每 slot 独立 VFS fd table；6 项测试覆盖 initial stack、状态、descriptor isolation/seek 与 exit-time `close_all`。`userspace/init` 是独立 Rust `no_std` executable，linker 把它放到 `0x40000000`；release artifact 为 26152 bytes，其中一个 R+X `PT_LOAD` 从 file offset `0x1000` 装入 2416 bytes。rootfs builder 将 artifact 安装为 `/sbin/slop-init`；ext4 mount/recovery 后，block task 经 path walker 从 inode 23 跨七个逻辑块读入它，并与 UEFI loader 通过 BootInfo v2 保留的 ESP `LOADER_DATA` 副本逐字节比较。kernel 在第二个 stack page 构造 `argc=2`、两项 `argv`、三项 `envp` 和 9 对 auxv；用户 entry assembly 把原始 `RSP` 传给 Rust，PID 1 在 syscall 前逐项核对。程序随后按 Linux x86-64 寄存器/编号约定执行 `O_RDONLY/O_RDWR openat`、read/write/lseek/close、stdout write 与 exit；64-byte read/write buffer 横跨两个 virtual stack page，并由逐页 user-copy helper 映射到独立 physical frame。需要 I/O 的同步调用通过保存 frame 转为 block task 的异步 ext4/virtio completion；exit 会释放遗留 fd/backing object，但暂不回收地址空间。完整边界见 [processes.md](processes.md)。
