...
`crates/shell` 是无分配、无标准库的滚动平铺/workspace 与壁纸状态机、niri KDL parser、Waybar JSONC/format/CSS parser、swww CLI/environment parser 和 P3/PNM decoder。column 位于向右延伸的 strip，新窗口不会改变既有 column width；viewport 按 focus edge reveal 或 `never`/`always`/`on-overflow` policy 移动，一个 column 也可纵向 stack 多个 window。每个 workspace 持有独立 strip，KDL 的 named workspace、bind chord/action 和有序 `app-id` window rule 由固定容量结构保存。Waybar module option、固定 96-byte formatted text 与最多 32 条 CSS cascade rule 均不分配。swww 部分以固定容量保存 current/previous path，校验 daemon/output 生命周期，并提供 fade、方向、center/outer transition 的纯像素函数。25 项宿主测试覆盖 parse/reject/open/focus/scroll/stack/close/workspace/rule/format/style、daemon、嵌入式 PNM asset 与 transition。
