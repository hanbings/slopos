...
构建产生两个 disk：64 MiB FAT32 ESP 只供 UEFI loader，128 MiB `SLOPOS_ROOT` ext4 image 作为独立 root disk。内核选择第二个 virtio-blk function，先验证 superblock/group/root，再由异步 component walker 重复执行 inode-table、directory extent 和 child inode 读取，最终核对两个 regular file。`crates/ext4` 校验 superblock、group descriptor、inode 与 directory tail checksum，解析 64-bit block address、depth-0 extent 和 variable-length directory entry，并拒绝未知 incompat feature、dirty state 与 htree。当前只走 group 0、单块目录/文件，仍称为 mount probe。
