...
构建产生两个 disk：64 MiB FAT32 ESP 只供 UEFI loader，128 MiB `SLOPOS_ROOT` ext4 image 作为独立 root disk。内核选择第二个 virtio-blk function，依次读取 sector 2–3、group descriptor block、root inode 所在 table block 和第一个 directory extent。`crates/ext4` 校验 superblock、group descriptor、inode 与 directory tail checksum，解析 64-bit block address、root extent 和 variable-length directory entry。当前只走 group 0/root 的 depth-0 extent，没有通用路径遍历，仍称为 mount probe。
