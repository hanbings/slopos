...
`crates/desktop-protocol` 定义用户态与 kernel 共用的 40-byte `DesktopCommit`，以 magic/version/size、capability、config hash、provider range、wallpaper id 和零 reserved fields 建立拒绝边界；3 项测试覆盖 round-trip 与非法消息。`userspace/desktop` 读取 root VFS 的 Waybar/swww 配置并经私有 syscall 提交 generation 1；kernel `desktop_service.rs` 只接受 PID 2 的首次有效消息，复核 asset hash 后以 release/acquire snapshot 唤醒 desktop task。CPU/Memory 与初始 Aurora 因此来自用户态 policy，但 compositor、surface、输入、reload、swww daemon 和 GOP renderer 仍是 kernel mechanism。
