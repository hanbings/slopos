...
`crates/desktop-protocol` 定义 40-byte `DesktopCommit`、32-byte `DesktopServiceEvent`、最大 3872-byte 的 Wayland request 信封，以及最大 544-byte 的 server-event 信封；11 项测试覆盖版本、长度、capability、wire-only/inline FD token、event kind/sequence、对齐和 reserved drift。`userspace/desktop` 仍以固定 256-byte buffer读取配置并提交 policy；首代 apply 后，它依次发出 12/216/148-byte 标准 Wayland request batch并解析 156/56/32-byte registry/configure/presentation wire，随后以 64-byte attach/damage/frame/commit batch复用 buffer/callback ID并解析第二个 32-byte presentation。`kernel/wayland_service.rs` 只接受 PID 2，以持久的 `slopos-wayland::SingleSurfaceSession` 分派 registry/compositor/shm/xdg-shell 对象，严格要求无 buffer 初始 commit、configure serial ack、window geometry、attach、full damage、frame callback 与最终 commit，再用双 bank发布给 desktop；合成后才生成 buffer release/callback done/delete-id，并在 delete-id 后允许 callback ID 复用。信封中的 `0x534c` 是当前内联 backing 的私有 token，不冒充 OS file descriptor；transport 也明确不是 Unix socket/`SCM_RIGHTS`。QEMU marker 证明 registry/configure 加两轮 presentation 的四次 wire→block/wake→client parse，以及 generation 1→2 的双 bank renderer acknowledge；`capture-desktop.sh` 对第二帧四象限图案取样。配置 hash/generation、custom Waybar 与 reload 行为保持原有验证。
...
PID 2 首次收到 `policy-applied` 后会通过 `0x534c0003/4` 完成 registry globals、bufferless initial commit、shm/xdg configure、`ack_configure`、XRGB8888 buffer commit与 presentation release/callback 往返；随后复用已 release 的 buffer 与已 delete-id 的 callback ID完成第二轮 attach/damage/frame/commit/presentation。`slopos-wayland` dispatcher校验对象关系、批次状态、configure serial、buffer geometry、full damage、frame callback、atomic commit与每轮 callback retirement，`wayland_service.rs` 以双 bank 发布 generation 1/2 给 desktop task。上段所称缺少 Wayland object/surface IPC 是指仍无可连接的 Unix socket、`SCM_RIGHTS`/共享 mapping、普通第三方 client、多 client ownership或持续事件循环，而不是否认这条受限 bootstrap。
...
`crates/elf` 是无分配 `no_std` ELF64 parser；`crates/process` 提供固定容量 process/fd 生命周期。`userspace/init` 与 `userspace/desktop` 都是固定在 `0x40000000` 的独立 Rust executable；release artifacts 分别为 26344/38232 bytes，R+X `PT_LOAD` 为 2608/10776 bytes。rootfs builder安装为 inode 23/24；block task分别跨七/十块读入，只有 init 与 ESP 副本逐字节比较。kernel 为每个进程构造三页 code、三页 stack、Linux initial stack 与独立 CR3。PID 1 最终常驻 `wait4`，PID 2 完成 Wayland registry/configure/ack 与两轮 presentation 后常驻 config event。完整边界见 [processes.md](processes.md)。
