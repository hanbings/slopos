...
`crates/ipc` 提供固定容量 AF_UNIX local-stream namespace、双向 byte ring，以及与一批 bytes 原子关联的单个 generation-checked rights object；process/VFS descriptor object 可保存 file、socket 或 shared-memory handle。kernel 在 `/run/slopos/wayland-0` 建立 listener，Linux `socket/connect` 建立 PID 2 client/server endpoint，异步 `read/write/sendmsg/close` completion 按 descriptor type 分派。`userspace/desktop` 以固定 256-byte buffer读取配置并提交 policy；首代 apply 后，它经socket发出12-byte registry request，随后把296-byte compositor/shm/xdg/seat/output/pointer initial batch以256+40两次write连续发送，并解析156-byte registry与256-byte device/configure wire。再建立3072-byte memfd、把其一页 backing 以 `MAP_SHARED` 映射到 `0x40006000`，并用 `sendmsg(SCM_RIGHTS)` 发送 148-byte configured batch与 fd。服务端保留同一对象，第二个 64-byte batch只经普通 `write` 复用 pool/buffer和更新后的共享页；首轮presentation返回`wl_pointer.enter`加release/done/delete-id共56 bytes，第二轮返回后三项32 bytes。`kernel/wayland_service.rs` 只接受 PID 2，以持久 `SingleSurfaceSession` 分派 registry/compositor/shm/seat/pointer/output/xdg-shell 对象并用双 bank发布给 desktop。`crates/desktop-protocol` 仍保留旧 `0x534c0003/0004` 信封 decoder兼容测试，但运行 wire 不走该路径；私有 `0x534c0005` staging ABI 已删除。QEMU marker 证明 socket/connect、seat/output/pointer discovery与focus、memfd/ftruncate/mmap、带 `SCM_RIGHTS` 的 first pool、无重复 fd 的 buffer reuse、四次 stream wire→block/wake→client parse，以及 generation 1→2 renderer acknowledge；`capture-desktop.sh` 对第二帧四象限图案取样。
...
PID 2 首次收到 `policy-applied` 后会连接 AF_UNIX stream，通过普通异步 `write/read` 完成 registry globals、seat/output/pointer discovery、bufferless initial commit、shm/xdg configure，再用 `memfd_create/ftruncate/mmap` 和 `sendmsg(SCM_RIGHTS)` 提交 `ack_configure` 与首个 XRGB8888 buffer；首轮presentation解析pointer enter，随后只修改共享页，复用已 release 的 buffer 与已 delete-id 的 callback ID完成第二轮 attach/damage/frame/commit/presentation。`slopos-wayland` dispatcher校验对象关系、批次状态、configure serial、buffer geometry、full damage、frame callback、atomic commit、首次 pool descriptor和每轮 callback retirement，`wayland_service.rs` 以双 bank 发布 generation 1/2 给 desktop task。当前 IPC 边界仍缺keyboard keymap/live input fan-out、用户态 listener、普通第三方 client、多 client ownership、通用 ancillary data与持续事件循环；mmap也只支持一个固定地址、单页 shared memfd，没有 munmap。
...
`paging.rs` 从 frame allocator 建立新的 x86-64 PML4/PDPT/PD，以 2 MiB page identity-map 当前 RAM，并以 cache-disabled 映射覆盖 MMIO。PID 1 与 PID 2 各有自己的 PML4；原 kernel huge-page entry 不带 U/S。每个进程的三个 code page 为 user read-only，三页 stack 为 user writable且 physical frame互相独立；ELF loader按页复制最多 12288 bytes，user-copy 对 code/stack 也逐页翻译。reap 会释放四个私有 table、三个 code、三个 stack，共 10 frame。当前还未启用 NX 或 kernel section W^X。
...
`crates/elf` 是无分配 `no_std` ELF64 parser；`crates/process` 提供固定容量 process/fd 生命周期。`userspace/init` 与 `userspace/desktop` 都是固定在 `0x40000000` 的独立 Rust executable；release artifacts 分别为 26344/38176 bytes，R+X `PT_LOAD` 为 2608/11592 bytes。rootfs builder安装为 inode 23/24；block task分别跨七/十块读入，只有 init 与 ESP 副本逐字节比较。kernel 为每个进程构造三页 code、三页 stack、Linux initial stack 与独立 CR3；PID 2 还可把一个 shared-memory frame映射到紧邻 stack top 的 `0x40006000`，descriptor、mapping与 socket receiver各自持有显式引用。PID 1 最终常驻 `wait4`，PID 2 完成 AF_UNIX Wayland registry、seat/output/pointer discovery、configure/ack、pointer enter 与两轮 presentation 后常驻 config event。完整边界见 [processes.md](processes.md)。
...
`crates/ipc` 提供独立、无分配的具名 local byte-stream core。固定 socket table 以 generation handle 防止 slot 复用后的 stale reference；listener 保存有界 FIFO backlog，每条 accepted connection 由成对 endpoint 和各自 receive ring 构成。`send` 只复制到 peer ring 的可用部分并以 `WouldBlock` 表示 backpressure，`recv` 在 peer close 后排空尾部再返回 EOF；`send_with_rights/recv_with_rights` 还把一个 generation-checked object与同批bytes原子关联，并拒绝覆盖未消费rights。5项宿主测试覆盖双向分段读取、backlog saturation、满ring/恢复writable、close/EOF、duplicate path、stale handle与rights transfer。kernel已把它接入process descriptor和block-task executor；PID 2可调用Linux `socket/connect/read/write/sendmsg/close`，其中 `sendmsg(SCM_RIGHTS)` 把shared-memory handle交给固定Wayland listener。当前尚未向用户态暴露 `bind/listen/accept`，也没有通用control-message队列、credentials、poll/epoll或多client service ownership。
