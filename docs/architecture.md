...
构建产生两个 disk：64 MiB FAT32 ESP 只供 UEFI loader，256 MiB `SLOPOS_ROOT` ext4 image 作为独立 root disk。`fs.rs` 持有 `Ext4Mount`/`Ext4File` 和 8-entry FIFO cache；inode 25 用于 fd read-modify-write。隐藏 inode 8 的单一 extent 指向 4096-block journal；内核以五个 scratch frames 保留 ext4/JBD2 superblock 并编码 logical blocks 1–3。active probe 依次持久化 recovery bit、`start=1`、descriptor/data、commit，在各 durability boundary flush 并直接读回 replayable 状态；随后写 home data block、推进 sequence/清 start、清 recovery，再清测试 records。为保持构建镜像可重复，probe 最后恢复 inode 25 的原始数据并把测试 sequence 回卷。当前 transaction 只覆盖一个已分配 data block，尚无启动时 replay 或 metadata update。
