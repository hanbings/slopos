...
`crates/desktop-protocol` 定义用户态与 kernel 共用的 40-byte `DesktopCommit` 和 32-byte `DesktopServiceEvent`，以 magic/version/size、capability、config hash、provider range、wallpaper id、event kind/generation 和零 reserved fields 建立拒绝边界；5 项测试覆盖 commit/event round-trip 与非法消息。`userspace/desktop` 读取 root VFS 的 Waybar/swww 配置并经私有 syscall 提交 generation 1；kernel `desktop_service.rs` 只接受 PID 2 的首次有效消息，复核 asset hash 后以 release/acquire snapshot 唤醒 desktop task。PID 2 随即在第二个私有 syscall 上 Blocked；desktop task 完成 apply 后 acknowledge、唤醒 block task，后者复制 `policy-applied` event到 user stack并把 PID 2 转回 Runnable。CPU/Memory 与初始 Aurora 因此来自用户态 policy，但 compositor、surface、输入、reload、swww daemon 和 GOP renderer 仍是 kernel mechanism。
...
`crates/vfs` 是无分配、无标准库的 namespace 状态机：绝对路径最多 16 个 component，mount table 采用最长 component-prefix，fd table 从 3 开始分配并维护 vnode、size、offset 与 read/write access mode，并可在 owner exit 后执行有界 `close_all`。内核把 ext4 注册为 filesystem 1 并挂到 `/`；启动验证先经 path walker 跨七块读取 inode 23 的 `/sbin/slop-init`。PID 1 随后用 Linux `openat/read/close` 异步读出 inode 18 的 76 bytes，再用 `O_RDWR openat`、lseek、异步 write/read 对 inode 31 的 offset 123 执行跨两页的可逆 64-byte patch；退出后 block task 释放遗留 descriptor/backing。kernel probe 的独立 block-task table 还会以五个 chunk/seek 读取 inode 18，并对 inode 31 写入/恢复 73 bytes。桌面配置候选也复用同一 ext4 walker。进程 fd 已接入有界读写 syscall，但 mount table、`Ext4File` backing slots 与其他 probe fd 仍由单一 block task 专用，不是并发全局 POSIX VFS。
