...
`crates/desktop-protocol` 定义用户态与 kernel 共用的 40-byte `DesktopCommit` 和 32-byte `DesktopServiceEvent`，以 magic/version/size、capability、非零 config hash、provider range、wallpaper id、event kind/generation 和零 reserved fields 建立拒绝边界；5 项测试覆盖 commit/event round-trip、分块/整体 FNV 一致性与非法消息。`userspace/desktop` 用固定 256-byte buffer 在 4096/512-byte 上限内读取 root VFS 的 Waybar/swww 配置，增量计算实际文件 hash并经私有 syscall 提交 policy；kernel `desktop_service.rs` 只接受 PID 2，并要求上一代已实际应用才允许下一代，但不再把 hash 与默认 asset 绑定。VFS config bank 仍独立验证 UTF-8 与各格式语义。PID 2 在第二个私有 syscall 上按 kind/generation Blocked；desktop task 回送 `policy-applied`，VFS config swap 回送 `config-applied`，block task复制 event到 user stack并把 PID 2 转回 Runnable。service 收到每代 config event 后重新读文件并提交下一代 policy；交互回归已验证 config 1→2 对应 policy 2→3，非法 reload 不唤醒服务；独立 QEMU 回归还验证非默认 1541-byte Waybar JSONC 在 policy 1/2 中保持摘要 `0x976a817ac7e7fb85`，以 ordered `output` array 命中 `SLOPOS-1`，再以 `slop-main` name/class namespace、666-byte CSS 的 output-class `#ff79c6` 底边与 name-class `#202640` 背景、4/12px margin、44px exclusive reserve、非固定 center、三向 expand 与 top layer 改变 surface/work-area/module/composition 后完成 workspace 往返点击，并从 clock alternate format、三键与双向滚轮 action 触发格式往返、受限命令和 Sunset/Aurora 切换。1256-byte custom overlay fixture 的 Waybar 摘要为 `0x44700a2efaa41691`（同次提交的 swww 摘要仍为 `0xb6ea39f5fd14c33a`），第二次启动以 string `output: "SLOPOS-1"` 选中当前显示，并把 800 px surface 居中到 x=112、移除 center module，再把 `slop-overlay` mode object 展开为 overlay/非 exclusive/passthrough/visible，并完成 surface 内覆盖窗口的 click-through。第三次 993-byte fixture 的摘要为 `0x3d20db99f86c6b26`，ordered `["!SLOPOS-1", "*"]` 在 wildcard 前排除当前 output，令 bar `selected=false`、reserve=0，并以 y=10 的壁纸像素证明 surface 未被合成。第四次 1011-byte fixture 的摘要为 `0x248fdbf22b5028c3`，`["width > 2000", "height > 700"]` 对 1024×768 做 AND 过滤，首项失败后同样记录 `selected=false`、reserve=0 与未被 bar 覆盖的壁纸像素。CPU/Memory 与 Aurora 因此来自常驻用户态 policy，但 compositor、surface、输入、配置 parser、swww daemon 和 GOP renderer 仍是 kernel mechanism。
