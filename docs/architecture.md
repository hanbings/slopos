...
`crates/shell` 是无分配、无标准库的滚动平铺/workspace 与壁纸状态机、niri KDL parser、Waybar JSONC parser、swww CLI/environment parser 和 P3/PNM decoder。column 位于向右延伸的 strip，新窗口不会改变既有 column width；viewport 按 focus edge reveal 或 `never`/`always`/`on-overflow` policy 移动，一个 column 也可纵向 stack 多个 window。每个 workspace 持有独立 strip，KDL 的 named workspace、bind chord/action 和有序 `app-id` window rule 由固定容量结构保存。swww 部分以固定容量保存 current/previous path，校验 daemon/output 生命周期，并提供 fade、方向、center/outer transition 的纯像素函数。21 项宿主测试覆盖 parse/reject/open/focus/scroll/stack/close/workspace/rule、daemon、嵌入式 PNM asset 与 transition。

`desktop.rs` 当前仍是内核态的早期合成 async task，但三个 surface 已由 workspace 内的 `ScrollLayout` 放入 50% 宽 column。PS/2 解码器跟踪 modifier 和 extended arrow；KDL bind 驱动 column/workspace focus、move 与 close，ordered window rule 把 Config 放入 named workspace。Tab 沿 column focus，标题栏横拖滚动 strip，红色按钮关闭 tile；`assets/waybar-config.jsonc` 决定顶部 bar，`assets/swww.env` 提供同名环境默认值，两张编译时 PNM 经 crop/fit/no resize 后成为窗口下方背景。kernel monitor 的 `SWWW IMG/QUERY/KILL` 会驱动同一个 daemon state；过渡以最多 17 个同步采样帧写入 GOP。它没有声称实现 Wayland object/surface IPC、用户态 client、完整 niri dynamic workspace/rule/action/output/animation、Waybar CSS/完整 module backend，或真正的 swww socket/process/layer-shell。兼容边界见 [desktop-shell.md](desktop-shell.md)。
