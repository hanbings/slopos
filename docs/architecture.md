...
`crates/shell` 是无分配、无标准库的 niri/Waybar/swww 状态机与配置 parser。每个 workspace 保存独立的 horizontal column strip、floating layer、active/previous focus 和有界动态尾部空位。tiled layer 支持 edge reveal/manual scroll、首末/相邻列 focus+reorder、single/visible/focused center、normal/tabbed stack、固定与方向感知 consume/expel、上下 focus/reorder、gap-aware explicit/preset width/height、column/edge maximize、available-width expand 和 close；floating layer 始终位于 tile 上方且不滚动，保存 z-order/rect/default height，支持 layer focus、方向 focus/move、显式/preset resize、center/expand、pointer move/resize 和跨 workspace transfer。跨 workspace 状态机明确区分完整 column 与 focused window：前者保留 stack/focus/width/display，后者以原列宽抽成目标 singleton，两者都先检查目标容量再修改。`toggle-window-floating` 从 tile 当前屏幕位置推导浮窗 rect，送回 tiling 时按当前 focused column 邻近插入；`move-window-to-floating`/`move-window-to-tiling` 和 `focus-floating`/`focus-tiling` 提供幂等的显式目标语义。KDL 以最多 64 项 bind、named workspace、`default-column-display`、`open-on-workspace`、`open-floating true|false` 和有序 rule override 驱动这些行为；proportion 使用 `(working_size - gap) × proportion - gap`，当前 workspace 容量为 4。Waybar module normal/alternate format、format-alt-click、interval/length、三键与双向滚轮 action option、固定 96-byte text/action 与最多 32 条 CSS cascade rule 均不分配；swww 状态机保存 image/clear 生命周期、P3 parser 与 transition。38 项 shell 宿主测试覆盖上述 parse/reject/layout/layer/workspace/rule/format/style、daemon、PNM 与 transition。

`crates/desktop-protocol` 定义用户态与 kernel 共用的 40-byte `DesktopCommit` 和 32-byte `DesktopServiceEvent`，以 magic/version/size、capability、非零 config hash、provider range、wallpaper id、event kind/generation 和零 reserved fields 建立拒绝边界；5 项测试覆盖 commit/event round-trip、分块/整体 FNV 一致性与非法消息。`userspace/desktop` 用固定 256-byte buffer 在 4096/512-byte 上限内读取 root VFS 的 Waybar/swww 配置，增量计算实际文件 hash并经私有 syscall 提交 policy；kernel `desktop_service.rs` 只接受 PID 2，并要求上一代已实际应用才允许下一代，但不再把 hash 与默认 asset 绑定。VFS config bank 仍独立验证 UTF-8 与各格式语义。PID 2 在第二个私有 syscall 上按 kind/generation Blocked；desktop task 回送 `policy-applied`，VFS config swap 回送 `config-applied`，block task复制 event到 user stack并把 PID 2 转回 Runnable。service 收到每代 config event 后重新读文件并提交下一代 policy；交互回归已验证 config 1→2 对应 policy 2→3，非法 reload 不唤醒服务；独立 QEMU 回归还验证非默认 1300-byte Waybar JSONC 在 policy 1/2 中保持摘要 `0xb001e9dac40c640b`，把 workspace module 移到 center 后按新几何完成双向点击，并从 clock alternate format、三键与双向滚轮 action 触发格式往返、受限命令和 Sunset/Aurora 切换。CPU/Memory 与 Aurora 因此来自常驻用户态 policy，但 compositor、surface、输入、配置 parser、swww daemon 和 GOP renderer 仍是 kernel mechanism。

`desktop.rs` 当前仍是内核态 early compositor。它先按 tile 几何渲染 tiled surface，再按 z-order 合成 floating surface；pointer hit-test 反向遍历浮窗，因此重叠时总选最上层。`Mod+V`/`Mod+Shift+V` 分别切换窗口所在层与层焦点，`Mod+Alt+V`/`Mod+Ctrl+V` 显式移入 floating/tiling，`Mod+Alt+G`/`Mod+Alt+T` 显式聚焦 floating/tiling；方向 move bind 对浮窗使用 50 px step，floating 标题栏拖动改变 rect而不是滚动 strip，Super+右键同时调整浮窗宽高，tiled 手势保持原语义。`Mod+W` tabbed display、column/window/workspace 全套 bind、ordered `open-floating` rule、Waybar workspace/action hit-test 与 swww renderer 共享同一 active-window 状态。构造阶段使用 `assets/` bootstrap；ext4 mount 后由 `fs.rs` 按 user/system/fallback 候选加载四份配置，`desktop_config.rs` 通过双 bank generation parse-before-swap，reload 会尽量保留当前 floating/tiled layer。`wallpaper_file.rs` 另以 8 KiB 双 bank 连接 desktop/block task：输入侧发布 path/transition generation，block task 通过 ext4 walker 异步读齐并验证 UTF-8/P3，desktop 完成动画后才 acknowledge；失败 bank 不替换当前 pinned image，因此 missing/invalid path 保留旧壁纸。当前仍没有文件 watcher、Wayland object/surface IPC、普通用户 client、超过 4 个 workspace、浮窗位置持久化或完整 niri rule/action/output/animation；Waybar provider/Pango/完整 GTK CSS 与真正的 swww socket/process/layer-shell 也未实现。兼容边界见 [desktop-shell.md](desktop-shell.md)。
...
`crates/vfs` 是无分配、无标准库的 namespace 状态机：绝对路径最多 16 个 component，mount table 采用最长 component-prefix，fd table 从 3 开始分配并维护 vnode、size、offset 与 read/write access mode，并可在 owner exit 后执行有界 `close_all`。内核把 ext4 注册为 filesystem 1 并挂到 `/`；启动验证先经 path walker 跨七块读取 inode 23 的 `/sbin/slop-init`。PID 1 随后用 Linux `openat/read/close` 异步读出 inode 18 的 76 bytes，再用 `O_RDWR openat`、lseek、异步 write/read 对 inode 31 的 offset 123 执行跨两页的可逆 64-byte patch并显式 close。PID 2 在每代 config event 后重开并关闭 Waybar/swww 文件。kernel probe 的独立 block-task table 还会以五个 chunk/seek 读取 inode 18，并对 inode 31 写入/恢复 73 bytes；桌面配置候选与 swww path image loader 也复用同一 ext4 walker，后者已实测跨 inode 30 的两个 block 读齐 6144-byte P3。进程 fd 已接入有界读写 syscall，但 mount table、`Ext4File` backing slots 与其他 probe fd 仍由单一 block task 专用，不是并发全局 POSIX VFS。
