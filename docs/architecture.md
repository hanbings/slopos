...
`crates/ipc` 提供固定容量 AF_UNIX local-stream namespace与双向 byte ring；process/VFS descriptor object 可保存 file 或 generation-checked socket handle。kernel 在 `/run/slopos/wayland-0` 建立 listener，Linux `socket/connect` 建立 PID 2 client/server endpoint，既有异步 `read/write/close` completion 按 descriptor type 分派。`userspace/desktop` 仍以固定 256-byte buffer读取配置并提交 policy；首代 apply 后，它依次经 socket 发出 12/216/148-byte 标准 Wayland request并解析 156/56/32-byte registry/configure/presentation wire，随后以 64-byte batch复用 buffer/callback ID并解析第二个 32-byte presentation。`kernel/wayland_service.rs` 只接受 PID 2，以持久 `SingleSurfaceSession` 分派 registry/compositor/shm/xdg-shell 对象并用双 bank发布给 desktop；合成后才生成 buffer release/callback done/delete-id。`crates/desktop-protocol` 仍保留旧信封 decoder兼容测试，但运行中的 wire 不再走 `0x534c0003/0004`；3072-byte backing 暂经 `0x534c0005` staging ABI，因为尚无 `SCM_RIGHTS`/mmap。QEMU marker 证明 socket/connect、四次 stream wire→block/wake→client parse，以及 generation 1→2 renderer acknowledge；`capture-desktop.sh` 对第二帧四象限图案取样。
...
`crates/elf` 是无分配 `no_std` ELF64 parser；`crates/process` 提供固定容量 process/fd 生命周期。`userspace/init` 与 `userspace/desktop` 都是固定在 `0x40000000` 的独立 Rust executable；release artifacts 分别为 26344/36608 bytes，R+X `PT_LOAD` 为 2608/10136 bytes。rootfs builder安装为 inode 23/24；block task分别跨七/九块读入，只有 init 与 ESP 副本逐字节比较。kernel 为每个进程构造三页 code、三页 stack、Linux initial stack 与独立 CR3。PID 1 最终常驻 `wait4`，PID 2 完成 AF_UNIX Wayland registry/configure/ack 与两轮 presentation 后常驻 config event。完整边界见 [processes.md](processes.md)。
