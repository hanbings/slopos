...
`crates/shell` 是无分配、无标准库的滚动平铺/workspace 与壁纸状态机、niri KDL parser、Waybar JSONC/format/CSS parser、swww CLI/environment parser 和 P3/PNM decoder。column 位于向右延伸的 strip，新窗口不会改变既有 column width；viewport 按 focus edge reveal 或 `never`/`always`/`on-overflow` policy 移动，一个 column 也可纵向 stack 多个 window。每个 workspace 持有独立 strip，KDL 的 named workspace、bind chord/action、固定/百分比 `set-column-width` 和有序 `app-id` window rule 由固定容量结构保存。Waybar module option、固定 96-byte formatted text 与最多 32 条 CSS cascade rule 均不分配。swww 部分以固定容量保存 current/previous path，校验 daemon/output 生命周期，并提供 fade、方向、center/outer transition 的纯像素函数。26 项宿主测试覆盖 parse/reject/open/focus/scroll/stack/resize/close/workspace/rule/format/style、daemon、嵌入式 PNM asset 与 transition。

`crates/desktop-protocol` 定义用户态与 kernel 共用的 40-byte `DesktopCommit` 和 32-byte `DesktopServiceEvent`，以 magic/version/size、capability、config hash、provider range、wallpaper id、event kind/generation 和零 reserved fields 建立拒绝边界；5 项测试覆盖 commit/event round-trip 与非法消息。`userspace/desktop` 读取 root VFS 的 Waybar/swww 配置并经私有 syscall 提交 policy；kernel `desktop_service.rs` 只接受 PID 2、复核 asset hash，并要求上一代已实际应用才允许下一代。PID 2 在第二个私有 syscall 上按 kind/generation Blocked；desktop task 回送 `policy-applied`，VFS config swap 回送 `config-applied`，block task复制 event到 user stack并把 PID 2 转回 Runnable。service 收到每代 config event 后重新读文件并提交下一代 policy；交互回归已验证 config 1→2 对应 policy 2→3，非法 reload 不唤醒服务。CPU/Memory 与 Aurora 因此来自常驻用户态 policy，但 compositor、surface、输入、配置 parser、swww daemon 和 GOP renderer 仍是 kernel mechanism。
