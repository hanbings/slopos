...
`crates/ipc` 提供固定容量 AF_UNIX local-stream namespace、双向 byte ring，以及两个方向上与一批 bytes 原子关联的单个 generation-checked rights object；process/VFS descriptor object 可保存 file、socket 或 shared-memory handle。kernel 在 `/run/slopos/wayland-0` 建立 listener，Linux `socket/connect` 建立 PID 2 client/server endpoint，异步 `read/write/sendmsg/recvmsg/close` completion 按 descriptor type 分派。`userspace/desktop` 以固定 256-byte buffer读取配置并提交 policy；首代 apply 后，它经socket发出12-byte registry request，随后把308-byte compositor/shm/xdg/seat/output/pointer/keyboard initial batch以256+52两次write连续发送。288-byte configure wire的前256 bytes经`recvmsg`与一个只读XKB keymap fd一起返回，余下32 bytes经普通`read`返回；desktop分块读取3307-byte keymap并校验hash/NUL/EOF。随后建立3072-byte memfd、把其一页 backing 以 `MAP_SHARED` 映射到 `0x40007000`，并用 `sendmsg(SCM_RIGHTS)` 发送148-byte configured batch与fd。服务端保留同一对象，第二个64-byte batch只经普通`write`复用pool/buffer和更新后的共享页；首轮presentation返回`wl_pointer.enter`、`wl_keyboard.enter`及release/done/delete-id共76 bytes，第二轮返回后三项32 bytes。`kernel/wayland_service.rs` 只接受 PID 2，以持久 `SingleSurfaceSession` 分派 registry/compositor/shm/seat/pointer/keyboard/output/xdg-shell 对象并用双 bank发布给 desktop。`crates/desktop-protocol` 仍保留旧 `0x534c0003/0004` 信封 decoder兼容测试，但运行 wire 不走该路径；私有 `0x534c0005` staging ABI 已删除。QEMU marker 证明 socket/connect、seat/output/pointer/keyboard discovery与focus、server→client keymap、client→server surface backing、buffer reuse、四次 stream wire→block/wake→client parse，以及 generation 1→2 renderer acknowledge；`capture-desktop.sh` 对第二帧四象限图案取样。
...
PID 2 首次收到 `policy-applied` 后会连接 AF_UNIX stream，通过普通异步 `write/read` 完成 registry globals、seat/output/pointer/keyboard discovery与bufferless initial commit，再以`recvmsg(SCM_RIGHTS)`取得并校验XKB keymap fd及configure。随后用 `memfd_create/ftruncate/mmap` 和反方向的 `sendmsg(SCM_RIGHTS)` 提交 `ack_configure` 与首个 XRGB8888 buffer；首轮presentation解析pointer/keyboard enter，随后只修改共享页，复用已 release 的 buffer 与已 delete-id 的 callback ID完成第二轮 attach/damage/frame/commit/presentation。`slopos-wayland` dispatcher校验对象关系、批次状态、configure serial、buffer geometry、full damage、frame callback、atomic commit、首次 pool descriptor和每轮 callback retirement，`wayland_service.rs` 以双 bank 发布 generation 1/2 给 desktop task。当前 IPC 边界仍缺持续input fan-out、用户态poll/listener、普通第三方 client、多 client ownership与通用 ancillary data；mmap也只支持一个固定地址、单页 shared memfd，没有 munmap。
...
`crates/elf` 是无分配 `no_std` ELF64 parser；`crates/process` 提供固定容量 process/fd 生命周期。`userspace/init` 与 `userspace/desktop` 都是固定在 `0x40000000` 的独立 Rust executable；release artifacts 分别为26344/40960 bytes，R+X `PT_LOAD` 为2608/12984 bytes。rootfs builder安装为inode 23/24；block task分别跨七/十块读入，只有init与ESP副本逐字节比较。kernel为每个进程构造四页code、三页stack、Linux initial stack与独立CR3；PID 2还可把一个shared-memory frame映射到紧邻stack top的`0x40007000`，descriptor、mapping与socket receiver各自持有显式引用。PID 1最终常驻`wait4`，PID 2完成AF_UNIX Wayland registry、seat/output/pointer/keyboard discovery、keymap导入、configure/ack、pointer/keyboard enter与两轮presentation后常驻config event。完整边界见 [processes.md](processes.md)。
...
`crates/ipc` 提供独立、无分配的具名 local byte-stream core。固定 socket table 以 generation handle 防止 slot 复用后的 stale reference；listener 保存有界 FIFO backlog，每条 accepted connection 由成对 endpoint 和各自 receive ring 构成。`send` 只复制到 peer ring 的可用部分并以 `WouldBlock` 表示 backpressure，`recv` 在 peer close 后排空尾部再返回 EOF；`send_with_rights/recv_with_rights` 还把一个 generation-checked object与同批bytes原子关联，并拒绝覆盖未消费rights。5项宿主测试覆盖双向分段读取、backlog saturation、满ring/恢复writable、close/EOF、duplicate path、stale handle与两个方向的rights transfer。kernel已把它接入process descriptor和block-task executor；PID 2可调用Linux `socket/connect/read/write/sendmsg/recvmsg/close`，其中`sendmsg(SCM_RIGHTS)`把surface backing交给Wayland listener，`recvmsg(SCM_RIGHTS)`接收server只读keymap object。当前尚未向用户态暴露 `bind/listen/accept`，也没有通用control-message队列、credentials、poll/epoll或多client service ownership。
