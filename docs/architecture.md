...
`crates/desktop-protocol` 定义用户态与 kernel 共用的 40-byte `DesktopCommit` 和 32-byte `DesktopServiceEvent`，以 magic/version/size、capability、config hash、provider range、wallpaper id、event kind/generation 和零 reserved fields 建立拒绝边界；5 项测试覆盖 commit/event round-trip、增量 hash 与非法消息。`userspace/desktop` 分块读取 root VFS 的 Waybar/swww 配置并经私有 syscall 提交 policy；kernel `desktop_service.rs` 只接受 PID 2、复核当前 config bank hash，并要求上一代已实际应用才允许下一代。PID 2 在第二个私有 syscall 上按 kind/generation Blocked；desktop task 回送 `policy-applied`，VFS config swap 回送 `config-applied`，block task复制 event到 user stack并把 PID 2 转回 Runnable。service 收到每代 config event 后重新读文件并提交下一代 policy；交互回归已验证 config 1→2 对应 policy 2→3，非法 reload 不唤醒服务，custom-config 回归另验证不同长度的合法 Waybar bytes 能连续发布。CPU/Memory 与 Aurora 因此来自常驻用户态 policy，但 compositor、surface、输入、配置 parser、swww daemon 和 GOP renderer 仍是 kernel mechanism。
