...
构建产生两个 disk：64 MiB FAT32 ESP 只供 UEFI loader，256 MiB `SLOPOS_ROOT` ext4 image 作为独立 root disk。`fs.rs` 持有 `Ext4Mount`/`Ext4File` 和 8-entry FIFO cache；inode 25 用于 fd read-modify-write。隐藏 inode 8 的单一 extent 指向 4096-block journal；内核用 scratch frames 编码 logical blocks 1–3，按 descriptor、data、flush、commit、flush 发布并恢复 inactive records。独立状态 probe 保留 ext4 block 0/JBD2 block 0，依次持久化 recovery bit 和 `start=1`、读回验证，再先清 journal state、后清 recovery bit。records 和 state 仍分开测试，所以尚未构成可 replay transaction。
