...
`crates/shell` 是无分配、无标准库的滚动平铺/workspace 与壁纸状态机、niri KDL parser、Waybar JSONC/format/CSS parser、swww CLI/environment parser 和 P3/PNM decoder。column 位于向右延伸的 strip，新窗口不会改变既有 column width；viewport 按 focus edge reveal 或 `never`/`always`/`on-overflow` policy 移动，一个 column 也可纵向 stack 多个 window。KDL action 可把右侧顶窗 consume 到当前列底部、在同列上下聚焦或重排、以固定/百分比绝对值和相对值调整 focused window 高度并守恒同列像素、恢复等高，再把底窗 expel 回右侧且保留原列焦点。每个 workspace 持有独立 strip与 previous 索引，KDL 的 named workspace、相对/索引/名称/previous bind action、同 strip 列重排、固定/百分比 `set-column-width`、最多 8 项 fixed/proportion `preset-column-widths` 和有序 `app-id` window rule 由固定容量结构保存。workspace normalize 会保留 named 前缀、压缩多余匿名空洞，并在末尾有内容时追加空 layout；当前 desktop 容量为 4。Waybar module format/interval/length、三键与双向滚轮 action option、固定 96-byte formatted text/action 与最多 32 条 CSS cascade rule 均不分配。swww 部分以固定容量保存 current/previous path 或 `0xRRGGBB` clear 状态，校验 daemon/output 生命周期，并提供 fade、方向、center/outer transition 的纯像素函数。27 项宿主测试覆盖 parse/reject/open/focus/scroll/stack/consume/expel/window-reorder/height/column-resize+reorder/close/workspace/rule/format/style、daemon、嵌入式 PNM asset 与 transition。
