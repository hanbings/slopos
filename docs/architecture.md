...
`crates/ipc` 提供固定容量 AF_UNIX local-stream namespace、双向 byte ring、whole-batch exact send，以及两个方向上与一批 bytes 原子关联的单个 generation-checked rights object；process/VFS descriptor object 可保存 file、socket、shared-memory或desktop-event handle。PID 2以Linux `socket/bind/listen`在fd 3建立 `/run/slopos/wayland-0` listener，再以fd 4 `socket/connect`并`accept`出fd 5；异步 `read/write/sendmsg/recvmsg/accept/poll/close` completion 按 descriptor type 分派，accepted endpoint的Wayland协议仍由kernel bootstrap处理。`userspace/desktop` 以固定 256-byte buffer读取配置并提交 policy；首代 apply 后，它经socket发出12-byte registry request，随后把308-byte compositor/shm/xdg/seat/output/pointer/keyboard initial batch以256+52两次write连续发送。288-byte configure wire的前256 bytes经`recvmsg`与一个只读XKB keymap fd 6一起返回，余下32 bytes经普通`read`返回；desktop分块读取3307-byte keymap并校验hash/NUL/EOF。随后在fd 6建立3072-byte memfd、把其一页 backing 以 `MAP_SHARED` 映射到 `0x40007000`，并用 `sendmsg(SCM_RIGHTS)` 发送148-byte configured batch与fd。服务端保留同一对象，第二个64-byte batch只经普通`write`复用pool/buffer和更新后的共享页；两轮presentation各返回release/done/delete-id共32 bytes。初始config到达后，PID 2以fd 6打开`/run/slopos/desktop-events`并用poll同时等待event fd 6与client fd 4；`kernel/wayland_service.rs`按真实compositor焦点和surface命中持续发布keyboard enter/leave/key/modifiers及pointer enter/leave/motion/button/axis/frame，client以持久stream buffer处理fragmented/coalesced wire。`crates/desktop-protocol` 仍保留旧 `0x534c0003/0004` 信封 decoder兼容测试，但运行 wire 不走该路径；私有 `0x534c0005` staging ABI 已删除。QEMU marker 证明 socket/bind/listen/connect/accept、seat/output/pointer/keyboard discovery、server→client keymap、client→server surface backing、buffer reuse、动态键鼠event、poll suspend/resume及generation 1→2 renderer acknowledge；`make test-wayland-input`还对第二帧四象限图案取样并生成截图。
...
`paging.rs` 从 frame allocator 建立新的 x86-64 PML4/PDPT/PD，以 2 MiB page identity-map 当前 RAM，并以 cache-disabled 映射覆盖 MMIO。PID 1 与 PID 2 各有自己的 PML4；原 kernel huge-page entry 不带 U/S。每个进程的四个 code page 为 user read-only，三页 stack 为 user writable且 physical frame互相独立；ELF loader按页复制最多 16384 bytes，user-copy 对 code/stack 也逐页翻译。reap 会释放四个私有 table、四个 code、三个 stack，共 11 frame。当前还未启用 NX 或 kernel section W^X。
...
`crates/elf` 是无分配 `no_std` ELF64 parser；`crates/process` 提供固定容量 process/fd 生命周期。`userspace/init` 与 `userspace/desktop` 都是固定在 `0x40000000` 的独立 Rust executable；release artifacts 分别为26344/40960 bytes，R+X `PT_LOAD` 为2608/15232 bytes。rootfs builder对desktop执行strip，再补非alloc确定性padding以维持十个ext4 block的固定布局；它们安装为inode 23/24，block task分别跨七/十块读入，只有init与ESP副本逐字节比较。kernel为每个进程构造四页code、三页stack、Linux initial stack与独立CR3；PID 2还可把一个shared-memory frame映射到紧邻stack top的`0x40007000`，descriptor、mapping与socket receiver各自持有显式引用。PID 1最终常驻`wait4`，PID 2完成AF_UNIX listener ownership、Wayland registry、seat/output/pointer/keyboard discovery、keymap导入、configure/ack与两轮presentation后，常驻Linux poll并同时等待Wayland socket和config event descriptor。完整边界见 [processes.md](processes.md)。
...
mount 首次解析若只因 `needs_recovery` 被拒绝，会进入受限 recovery parser：通过 journal inode 定位 active `s_start`，读取连续 descriptor/data/commit，联合校验 sequence、UUID、各 tag flags 与 target range，恢复 escaped data 后依次写 home 并 flush；随后清 records、推进 sequence/清 start并 flush，最后清 ext4 recovery bit/checksum。两阶段 feature kernel 先持久化已分配/已增长的 blocks 0/1/33/38/120，再发布目标为原始 free 状态的五 tag transaction，并在 commit 后/home 前停止；普通 kernel 重启同时重放五个 home，再从恢复后的 root VFS 读取 PID 1，随后继续正常桌面启动。当前 scanner 不支持多 transaction、revoke 或 ring wrap。

固定容量 multi-block engine 最多接受八个 home block；descriptor 的首 tag 携 UUID，后续 tag 使用 `SAME_UUID`，每个 tag 独立处理 escape。fd 3 在 EOF 请求 append 后，同一笔五 tag transaction 更新 block 0 superblock free count/checksum、block 1 group descriptor、block 33 bitmap、block 38 inode 31 与 block 120 data；home superblock 在 checkpoint 阶段保持 recovery bit，全部 home flush 后才清 recovery。inode 的 size/i_blocks/inline extent 从 4096/8 sectors/1 block 增至 8192/16/2，descriptor size/offset 随之更新，新增块经普通 fd read 回读；第二笔 truncate transaction 再释放回原始状态。replay scanner 已能解析最多八 tag，但仍只扫描一笔非 wrap transaction。

VFS create probe 复用相同 engine，把 blocks 0/1/36/38/104 作为一个原子集合：更新全局/group 1 free-inode count 与 `itable_unused`，重算 inode bitmap/group checksum，初始化带 extent header/checksum 的 inode 32，并在 inode 27 的线性目录块中拆分 slack、插入带 directory-tail checksum 的 `create-probe`。checkpoint 后正常 path walker 打开 size 0 文件，固定 descriptor table 为它分配读写 fd 3并验证 EOF；close 后第二笔 unlink transaction 合并目录 record、释放 inode bit并逐字节恢复五块。

`crates/vfs` 是无分配、无标准库的 namespace 状态机：绝对路径最多 16 个 component，mount table 采用最长 component-prefix，fd table 从 3 开始分配并维护file、socket、shared-memory或desktop-event object的offset/generation与access mode，并可在 owner exit 后执行有界 `close_all`。内核把 ext4 注册为 filesystem 1 并挂到 `/`；启动验证先经 path walker 跨七块读取 inode 23 的 `/sbin/slop-init`。PID 1 随后用 Linux `openat/read/close` 异步读出 inode 18 的 76 bytes，再用 `O_RDWR openat`、lseek、异步 write/read 对 inode 31 的 offset 123 执行跨两页的可逆 64-byte patch并显式 close。PID 2长期打开`/run/slopos/desktop-events`并以poll观察config generation，每次ready后读取32-byte event，再重开并关闭Waybar/swww文件。kernel probe 的独立 block-task table 还会以五个 chunk/seek 读取 inode 18，并对 inode 31 写入/恢复 73 bytes；桌面配置候选与 swww path image loader 也复用同一 ext4 walker，后者已实测跨 inode 30 的两个 block 读齐并解码 6144-byte PNG。进程 fd 已接入有界读写 syscall，但 mount table、`Ext4File` backing slots 与其他 probe fd 仍由单一 block task 专用，不是并发全局 POSIX VFS。

`crates/ipc` 提供独立、无分配的具名 local byte-stream core。固定 socket table 以 generation handle 防止 slot 复用后的 stale reference；listener 保存有界 FIFO backlog，每条 accepted connection 由成对 endpoint 和各自 receive ring 构成。`send` 只复制到 peer ring 的可用部分并以 `WouldBlock` 表示 backpressure，输入event使用的`send_exact`则在容量不足时拒绝整批，保证不留下半个协议frame；`recv` 在 peer close 后排空尾部再返回 EOF；`send_with_rights/recv_with_rights` 还把一个 generation-checked object与同批bytes原子关联，并拒绝覆盖未消费rights。7项宿主测试覆盖双向分段读取、backlog saturation、非消费式listener readiness、满ring/恢复writable、exact-send framing、close/EOF、duplicate path、stale handle与两个方向的rights transfer。kernel已把它接入process descriptor和block-task executor；PID 2可调用Linux `socket/bind/listen/connect/accept/read/write/sendmsg/recvmsg/poll/close`，其中`sendmsg(SCM_RIGHTS)`把surface backing交给Wayland accepted endpoint，`recvmsg(SCM_RIGHTS)`接收server只读keymap object，poll把socket readiness与config event合并进一个用户态loop。当前listener仅限受信PID 2、固定path与backlog，accepted endpoint仍由kernel protocol处理；也没有通用control-message队列、credentials、epoll或多client service ownership。
