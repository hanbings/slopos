...
`paging.rs` 从 frame allocator 建立新的 x86-64 PML4/PDPT/PD，以 2 MiB page identity-map 当前 RAM，并以 cache-disabled 映射覆盖 GOP framebuffer、LAPIC、IOAPIC 和 PCI BAR MMIO，然后写入并读回 CR3。映射器可建立多个 lower-canonical PML4 slot；这对 OVMF 分配在 768 GiB 的 virtio 64-bit BAR 是实际必需的。PID 1 与 PID 2 各有自己的 PML4：它们复制 kernel entries，但克隆 low PDPT 后在 1 GiB 位置增加独立 user hierarchy；原 low 1 GiB huge-page entry 不带 U/S，因此 kernel identity map 在 CPL3 不可访问。每个进程的 code page 为 user read-only，两页 stack 为 user writable且 physical frame 互相独立。当前还未启用 NX 或 kernel section W^X。`heap.rs` 从 contiguous frames 保留 1 MiB，提供 alignment-aware、并发保护的 bump allocation；启动路径实际分配 128 bytes 并验证首尾。
...
`crates/elf` 是无分配 `no_std` ELF64 parser。它只接受 little-endian x86-64 `ET_EXEC`，有界解析 program-header table；对每个 `PT_LOAD` 校验 `p_filesz <= p_memsz`、file/address overflow、canonical address、2 次幂 alignment 与 offset/vaddr congruence，拒绝空、重叠和 W+X segment，并要求 entry 位于 executable segment。`crates/process` 提供容量参数化的 PID/parent、image/initial-RSP context、`Ready/Running/Blocked/Runnable/Exited` 生命周期、round-robin selection、exit/syscall accounting、Linux initial-stack builder、reap 和每 slot 独立 VFS fd table；6 项测试覆盖 initial stack、状态/调度、descriptor isolation/seek 与 exit-time `close_all`。`userspace/init` 与 `userspace/worker` 是两个独立 Rust `no_std` executable，linker 都把它们放到 `0x40000000`；release artifacts 分别为 26200/25488 bytes，R+X `PT_LOAD` 为 2464/1744 bytes。rootfs builder 将两者安装为 inode 23 `/sbin/slop-init` 与 inode 24 `/sbin/slop-worker`；ext4 mount/recovery 后，block task 各跨七块读入，只有 init 还须与 BootInfo v2 的 ESP `LOADER_DATA` 副本逐字节比较。kernel 为每个进程构造独立 CR3、两页 stack、`argc=2`、两项 `argv`、三项 `envp` 和 9 对 auxv；两个 entry 都在 syscall 前逐项核对。PID 1/2 通过 `sched_yield` cooperative 轮转，并在同一时段各自拥有 fd 3；I/O 调用保存 per-PID frame、转成 `Blocked`，完成后转回 `Runnable` 并恢复对应 CR3。exit 会分别释放遗留 fd/backing object，但暂不回收地址空间。完整边界见 [processes.md](processes.md)。
