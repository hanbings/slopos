...
- 由 `userspace/init` 生成 26344-byte Rust ELF、由 `userspace/desktop` 生成另一个 33016-byte Rust ELF；rootfs builder 分别安装为 inode 23 `/sbin/slop-init` 和 inode 24 `/sbin/slop-shell`，kernel 的 ext4 path walker分别跨七个和九个逻辑块读取全部 bytes；
- UEFI 仍从 ESP `/slopos/init.elf` 读入 BootInfo v2 校验副本；kernel 要求 root VFS image 与该副本逐字节一致，差异会停止启动；
- 从 ELF file offset `0x1000` 分别复制 init 的 2608-byte 与 desktop service 的 6456-byte R+X `PT_LOAD`，逐页复制到各自两个独立、零填充的只读 code frame；
...
- 每个地址空间都在 `0x40002000..0x40004000` 映射两个 CPL3 可读写 stack page；各 physical frame 独立，不依赖物理连续。kernel 从上层页顶向下复制各自的 `argv`/`envp` strings，再编码 `argc=2`、pointer vectors 和 9 对 Linux auxv；PID 1 最终 `RSP=0x40003ec0`，多一项环境的 PID 2 为 `0x40003ea0`，均 16-byte aligned；
...
- 状态机仍实现 `exit`、异步 parent wait、immediate zombie lookup 与 reap：reap 从 process table 移除 slot，并释放 PML4、克隆 low PDPT、user directory/table、两页 code 与两页 stack，共 8 个 physical frame；这些通用边界目前由 6 项 process 宿主测试覆盖；
...
6. 四次 `lseek(3, 123, SEEK_SET)` 分别定位 patch/verify/restore/verify；64-byte scratch buffer 位于 `0x40002fe0..0x40003020`，故意横跨两个 stack page。两次 write 和两次 read 完成可逆 patch，四个 completion marker 都记录 `cross_page=true`；
...
首代 `policy-applied` 返回后，PID 2 在自己的两页 stack 上构造一份 3436-byte 信封。其中 332 bytes 是标准 Wayland little-endian wire，依次包含 registry、compositor、shm、xdg-shell、title/app-id、attach、`damage_buffer`、frame callback 与 atomic commit；其后的 3072 bytes 是 32×24、stride 128 的 XRGB8888 backing snapshot。私有 syscall `0x534c0003` 只负责当前缺少 Unix socket/`SCM_RIGHTS` 时的 bootstrap transport。kernel 仍用 `slopos-wayland` 的普通 frame/argument/object-map dispatcher，并额外要求完整单 `xdg_toplevel` lifecycle、正确的对象关系、FD token、buffer length/stride/format、metadata、full damage、frame callback 和最终 commit。通过验证的 metadata/像素复制到双 bank，desktop task 合成后才 acknowledge；QEMU marker 与截图四个精确像素共同证明 System 窗口中的内容来自 PID 2。这个阶段没有伪装成完整 Wayland：尚无 socket、真实 fd/shared mapping、registry event/xdg configure 往返、buffer release、任意第三方 client 或动态 placement。
...
- 超出当前两页 code/两页 stack mapping 的通用 `copy_from_user`/`copy_to_user`，或 grow/truncate/stat/directory/dup/poll/mmap 等通用 VFS syscall；
