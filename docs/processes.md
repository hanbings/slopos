...
- 由 `userspace/init` 生成 26344-byte Rust ELF、由 `userspace/desktop` 生成另一个 36608-byte Rust ELF；rootfs builder 分别安装为 inode 23 `/sbin/slop-init` 和 inode 24 `/sbin/slop-shell`，kernel 的 ext4 path walker分别跨七个和九个逻辑块读取全部 bytes；
- UEFI 仍从 ESP `/slopos/init.elf` 读入 BootInfo v2 校验副本；kernel 要求 root VFS image 与该副本逐字节一致，差异会停止启动；
- 从 ELF file offset `0x1000` 分别复制 init 的 2608-byte 与 desktop service 的 10136-byte R+X `PT_LOAD`，逐页复制到各自三个独立、零填充的只读 code frame；
...
- 每个地址空间都在 `0x40000000..0x40002000` 映射两个 CPL3 可读、不可写的 code page；
...
首代 `policy-applied` 返回后，PID 2 先执行 Linux `socket(AF_UNIX, SOCK_STREAM, 0)`，再以 `connect` 把 fd 3 连到 `/run/slopos/wayland-0`。第一批 12-byte `get_registry` request 经异步 `write` 进入 client→server receive ring，下一次异步 `read` 令服务端排空并解析 batch，再把包含五个 `wl_registry.global` 的 156-byte event 经 server→client ring 返回。第二批 216 bytes bind compositor/shm/xdg、建立 surface/role、设置 title/app-id并执行无 buffer 初始 commit，换回两种 shm format、`xdg_toplevel.configure(32×24)` 与 serial 1 的 `xdg_surface.configure` 共 56 bytes。PID 2 随后用第三批 148-byte request 发送 `ack_configure`、创建 shm pool/buffer、设置 window geometry、attach、full `damage_buffer`、frame callback 与最终 commit；desktop task 合成后用同一 socket返回 32-byte `wl_buffer.release`、`wl_callback.done(1)` 与 `wl_display.delete_id(11)`。64-byte attach/damage/frame/commit batch复用 buffer 8 与 callback ID 11，generation 2 再产生 callback data 2 与第二次 delete-id。两帧各自的 3072-byte、32×24/stride 128 XRGB8888 backing 暂由私有 `0x534c0005` staging syscall提供，因为当前没有 `SCM_RIGHTS`/mmap；wire 本身不再使用旧的 `0x534c0003/0004` 信封。kernel 用 `slopos-wayland` dispatcher与持久 session 强制 configure serial、像素大小、每轮 attach/damage/frame/commit 和 callback retirement。QEMU 的 `socket/connect`、四组 stream request/event、generation 1→2 双 bank切换、崩溃回放和第二帧四个精确像素共同证明完整往返与 System 窗口像素来源。当前仍无用户态 `bind/listen/accept`、真实 shared backing、任意第三方 client、多 client object ownership、持续 frame loop 或动态 placement。
...
ELF 已与 kernel 分离，实际执行 bytes 来自 root VFS 的固定 `/sbin/slop-init` 与 `/sbin/slop-shell`；ESP/BootInfo 副本目前仍是 PID 1 的强制相等启动信任锚，因此这不是任意路径的通用 `exec`。每进程 descriptor ownership 已实际连接 root ext4 file 与 local socket；Linux `socket/connect/read/write/close` 已承载 Wayland wire，desktop policy/config event 和像素 staging 仍是有界 SlopOS 私有 ABI。单核 cooperative/preemptive scheduler 仍嵌在一个 block task 的专用 suspend/resume loop，fd 1 继续由 syscall handler 特判。现阶段没有：

- 任意路径/多 `PT_LOAD` page mapping、动态链接，或从通用 exec 参数构建初始栈；
- 超出当前三页 code/三页 stack mapping 的通用 `copy_from_user`/`copy_to_user`，或 grow/truncate/stat/directory/dup/poll/mmap 等通用 VFS syscall；
