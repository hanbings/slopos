...
PID 2 首次收到 policy generation 1 后只写一次 28-byte ready message，随后等待 config generation 1；配置到达后，它循环重读两份文件、提交 policy generation 2、等待其 apply，再等待下一代 config。交互测试用有效 `RELOAD` 发布 config generation 2并推动 policy generation 3；非法 CSS 不发布 config generation 3，因此 service 维持 blocked 且没有 policy generation 4。每轮文件 fd 都 close，稳定点的 runtime marker 记录 PID 1=`wait4`、PID 2=`config-applied`、resources=`retained`。
