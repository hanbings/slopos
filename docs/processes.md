...
- 由 `userspace/init` 生成 26344-byte Rust ELF、由 `userspace/desktop` 生成另一个 37296-byte Rust ELF；rootfs builder 分别安装为 inode 23 `/sbin/slop-init` 和 inode 24 `/sbin/slop-shell`，kernel 的 ext4 path walker分别跨七个和十个逻辑块读取全部 bytes；
- UEFI 仍从 ESP `/slopos/init.elf` 读入 BootInfo v2 校验副本；kernel 要求 root VFS image 与该副本逐字节一致，差异会停止启动；
- 从 ELF file offset `0x1000` 分别复制 init 的 2608-byte 与 desktop service 的 10184-byte R+X `PT_LOAD`，逐页复制到各自三个独立、零填充的只读 code frame；
...
首代 `policy-applied` 返回后，PID 2 在自己的三页 stack 上运行三阶段标准 Wayland lifecycle。第一批 12-byte `get_registry` 请求换回 156-byte、包含五个 `wl_registry.global` 的服务端 wire；第二批 216 bytes bind compositor/shm/xdg、建立 surface/role、设置 title/app-id，并执行无 buffer 的首次 commit，换回两种 shm format、`xdg_toplevel.configure(32×24)` 与带 serial 1 的 `xdg_surface.configure` 共 56 bytes。PID 2 解析这些事件后，第三批 148-byte request 发送 `ack_configure`、创建 shm pool/buffer、设置 window geometry、attach、full `damage_buffer`、frame callback 与最终 commit；3252-byte 信封的其余 3072 bytes 是 32×24、stride 128 的 XRGB8888 backing snapshot。私有 syscall `0x534c0003` 负责 request batch，`0x534c0004` 则以 block-task Future 等待有界 server event；两者都只补当前缺少 Unix socket/`SCM_RIGHTS` 的 bootstrap transport。kernel 用 `slopos-wayland` 的普通 frame/argument/object-map dispatcher与分阶段 session 强制 configure serial 在像素发布前被正确 ack。desktop task 合成后才发布 32-byte presentation wire，PID 2 再验证 `wl_buffer.release`、`wl_callback.done(1)` 与 `wl_display.delete_id(11)`。QEMU 三次 `Blocked → wayland-event → Runnable` marker、崩溃回放和截图四个精确像素共同证明完整往返与 System 窗口像素来源。这个阶段没有伪装成完整 Wayland：尚无 socket、真实 fd/shared mapping、任意第三方 client、多 client object ownership 或动态 placement。
...
PID 2 首次收到 policy generation 1 后提交一次 surface、写一次 28-byte ready message，随后等待 config generation 1；配置到达后，它循环重读两份文件、提交 policy generation 2、等待其 apply，再等待下一代 config。若 PID 2 先到 config wait 而 PID 1 仍为 Ready/Runnable，block-task broker 会暂存 desktop wait 并继续调度 PID 1，直到它真实进入 `wait4` 才发布稳定停驻状态。交互测试用有效 `RELOAD` 发布 config generation 2并推动 policy generation 3；非法 CSS 不发布 config generation 3，因此 service 维持 blocked且没有 policy generation 4。每轮文件 fd 都 close，稳定点的 runtime marker 记录 PID 1=`wait4`、PID 2=`config-applied`、resources=`retained`。
