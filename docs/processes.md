...
- 由 `userspace/init` 生成独立 22928-byte Rust ELF；rootfs builder 把它安装为 `/sbin/slop-init`，kernel 的 ext4 path walker 从 inode 23 跨六个逻辑块读取全部 bytes；
- UEFI 仍从 ESP `/slopos/init.elf` 读入 BootInfo v2 校验副本；kernel 要求 root VFS image 与该副本逐字节一致，差异会停止启动；
- 从 ELF file offset `0x1000` 复制 1008-byte `PT_LOAD`，剩余 code page 保持为零；
...
- `openat`/`read`/`close` 把 frame 复制到单进程 pending slot，恢复 kernel CR3/continuation，让 block task 解析 VFS path 或等待 ext4/virtio Future；完成后把 return value 与 read bytes 写回保存的 frame/user stack，重新切换 process CR3，以 `SYSRETQ` 恢复同一 RIP、RSP 和通用寄存器；
...
4. `write(1, message, 18)` 经既有 stdout 特判直接返回 18；
5. `exit(0)` 返回 kernel continuation。

process table 最终记录五次 syscall 和 status 0。路径 copy 最多 128 bytes，单次 read 最多 256 bytes；read destination 必须完全落在单页 user stack，path/stdout input 必须落在单页 code 或 stack mapping。kernel 通过保存的 physical code/stack frame 在 kernel CR3 下复制，不会把任意 user virtual pointer 直接解引用。未知编号返回 `-ENOSYS`，已连接调用对坏 fd、pointer、flag/path 等返回当前子集的负 errno。
...
ELF 已与 kernel 分离，实际执行 bytes 来自 root VFS 的固定路径 `/sbin/slop-init`；ESP/BootInfo 副本目前仍是强制相等的启动信任锚，因此这不是任意路径的通用 `exec`。每进程 descriptor ownership 已实际连接 root ext4 的只读 `openat/read/close`，但 PID 1 仍嵌在单一 block task 的专用 suspend/resume loop 中，系统也仍只有一个实际 running process；fd 1 继续由 syscall handler 特判。现阶段没有：

- 任意路径/多 `PT_LOAD` page mapping、动态链接、`argv`/`envp` 或 auxiliary vector；
- 跨页/多 mapping 的通用 `copy_from_user`/`copy_to_user`，或 `write`、`lseek`、`stat`、directory、dup/poll/mmap 等通用 VFS syscall；
