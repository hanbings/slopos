...
- 由 `userspace/init` 生成 26344-byte Rust ELF、由 `userspace/desktop` 生成另一个 38232-byte Rust ELF；rootfs builder 分别安装为 inode 23 `/sbin/slop-init` 和 inode 24 `/sbin/slop-shell`，kernel 的 ext4 path walker分别跨七个和十个逻辑块读取全部 bytes；
- UEFI 仍从 ESP `/slopos/init.elf` 读入 BootInfo v2 校验副本；kernel 要求 root VFS image 与该副本逐字节一致，差异会停止启动；
- 从 ELF file offset `0x1000` 分别复制 init 的 2608-byte 与 desktop service 的 10776-byte R+X `PT_LOAD`，逐页复制到各自三个独立、零填充的只读 code frame；
...
首代 `policy-applied` 返回后，PID 2 在自己的三页 stack 上运行 configure-gated 标准 Wayland lifecycle。第一批 12-byte `get_registry` 请求换回 156-byte、包含五个 `wl_registry.global` 的服务端 wire；第二批 216 bytes bind compositor/shm/xdg、建立 surface/role、设置 title/app-id，并执行无 buffer 的首次 commit，换回两种 shm format、`xdg_toplevel.configure(32×24)` 与带 serial 1 的 `xdg_surface.configure` 共 56 bytes。PID 2 解析这些事件后，第三批 148-byte request 发送 `ack_configure`、创建 shm pool/buffer、设置 window geometry、attach、full `damage_buffer`、frame callback 与最终 commit；3252-byte 信封的其余 3072 bytes 是 32×24、stride 128 的 XRGB8888 backing snapshot。desktop task 合成后发布 32-byte `wl_buffer.release`、`wl_callback.done(1)` 与 `wl_display.delete_id(11)`。PID 2 解析后以 64-byte attach/damage/frame/commit batch复用 buffer 8 与 callback ID 11，3168-byte 信封携不同的第二帧 backing；generation 2 合成再产生 callback data 2 与第二次 delete-id。私有 syscall `0x534c0003` 负责 request batch，`0x534c0004` 则以 block-task Future 等待有界 server event；两者都只补当前缺少 Unix socket/`SCM_RIGHTS` 的 bootstrap transport。kernel 用 `slopos-wayland` 的普通 frame/argument/object-map dispatcher与持久 session 强制 configure serial、像素大小、每轮 attach/damage/frame/commit 和 callback retirement。QEMU 四次 `Blocked → wayland-event → Runnable` marker、generation 1→2 双 bank切换、崩溃回放和第二帧四个精确像素共同证明完整往返与 System 窗口像素来源。这个阶段没有伪装成完整 Wayland：尚无 socket、真实 fd/shared mapping、任意第三方 client、多 client object ownership、持续 frame loop 或动态 placement。
