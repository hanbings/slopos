# 两个用户进程、cooperative scheduler、wait/reap 与异步 VFS syscall
...
- 由 `userspace/init` 生成 26264-byte Rust ELF、由 `userspace/worker` 生成另一个 25488-byte Rust ELF；rootfs builder 分别安装为 inode 23 `/sbin/slop-init` 和 inode 24 `/sbin/slop-worker`，kernel 的 ext4 path walker 各跨七个逻辑块读取全部 bytes；
- UEFI 仍从 ESP `/slopos/init.elf` 读入 BootInfo v2 校验副本；kernel 要求 root VFS image 与该副本逐字节一致，差异会停止启动；
- 从 ELF file offset `0x1000` 分别复制 init 的 2528-byte 与 worker 的 1744-byte R+X `PT_LOAD`，各 code page 的剩余部分保持为零；
...
- `wait4(-1, status, 0, NULL)` 先验证 child 与 4-byte writable status range，再保存 PID 1 frame、转为 `Blocked` 并让 scheduler 运行 PID 2。child exit 后，block task 把 Linux wait status 写回父进程，令其 `Blocked → Runnable`，以 child PID 作为 `RAX` 恢复；
...
7. PID 1 故意不显式关闭 fd 3；`write(1, message, 18)` 经 stdout 特判直接返回 18，再以 `wait4(-1, &status, 0, NULL)` 阻塞。PID 2 退出后，kernel 返回 child PID 2 与 status 0；PID 1 才 `exit(0)`，随后 cleanup marker 核对 `descriptors=1 backing_objects=1`。
...
- timer preemption、独立 kernel stack/thread、kill/signal/TLS；当前 context switch 只发生在显式 `sched_yield`、阻塞 syscall 或退出之后；
