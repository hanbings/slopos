...
两个 12×8 P3/PNM asset 在启动时完整校验 header、尺寸、max value、component 范围和精确 pixel 数。renderer 实际把 current/previous image 逐像素 blend 或 mask 到 GOP；纯色状态则直接填充 framebuffer，并让 `query` 以 `0xRRGGBB` 报告当前值。交互测试通过 PS/2 输入切到 Sunset，完成 5 个 center 采样帧，由 `query` 读回 `SLOPOS-1`、1024×768 和当前路径，再验证 kill/restart、`none` 重设、`clear 1a2b3c`、纯色 query 与恢复图片。7 项 swww/PNM、15 项 niri layout/shell、7 项 Waybar JSONC/CSS 与 5 项 desktop commit/event protocol 测试，共 34 项。
