...
- `--outputs/-o`、`--resize crop|fit|no|stretch`、等价于 resize no 的 `--no-resize`、`--fill-color RRGGBB`、九向 `--crop-gravity`、`--transition-type/-t`、`--transition-step`、`--transition-fps`、支持小数秒的 `--transition-duration`、四分量 `--transition-bezier`、宽高二分量 `--transition-wave`、`--transition-angle`、`--transition-pos` 与 `--invert-y true|false`；
