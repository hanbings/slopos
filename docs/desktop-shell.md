...
`make test-desktop-custom-config` 复制标准磁盘，在临时 ext4 副本中把默认 904-byte Waybar 文件替换为可解析的 1065-byte JSONC（增加用户注释，把 `niri/window`/`niri/workspaces` 分别移到 left/center，并给 clock 加上 `on-click: status`/`on-click-right: help`/`on-click-middle: swww query`），再从 OVMF 启动。日志要求 PID 2 两轮都读到 1065 bytes、policy generation 1/2 都携带相同的非默认 hash `0x8517ffe3efae3ef4`、config generation 1 正常应用、服务最后阻塞在 `config-applied after_generation=1`；随后真实 PS/2 点击中央数字 `2`/`1`，终端输入未执行的 `ABO`，左击右侧 clock 触发 STATUS，再补成 `ABOUT` 执行，以证明 action 保留输入缓冲；最后右击 clock 触发 HELP、中击触发 SWWW QUERY。测试同时拒绝任何 exit/FATAL，宿主最后运行 `e2fsck -fn`。这个回归证明当前支持有界非默认文件、module placement、点击几何、左/右/中键受限 action 与输入保持，而不是证明完整 Waybar 配置兼容。
...
JSONC parser 支持 `//` 与 `/* */` comment、trailing comma、最多 16 个 module/区域、24 个 module config。module object 当前保存 `format`、`format-alt`、`format-disconnected`、`interval`、`tooltip`、`min-length`、`max-length`、`on-click`、`on-click-right`、`on-click-middle`，跳过未知 nested option，并拒绝 duplicate、非法类型/范围、非 ASCII/控制字符/空 action 与冲突长度。format renderer 支持 `{}`、named replacement 与 `:>N` 右对齐；当前 provider 实际提供 `{value/name/index/total}`、`{title}`、`{usage}`、`{percentage}`、`{ifname}`。
...
4 项 JSONC/format 与 3 项 CSS 测试覆盖 parse、format/action option、replacement、cascade、transparent、box shorthand 和拒绝边界。JSONC/CSS 已从 VFS 成对参与原子 generation reload。当前 registry 包含 `niri/workspaces`、`niri/window`、`custom/launcher`、`network`、`cpu`、`memory`、`clock`：CPU/Memory 的初始值来自 `/sbin/slop-shell` 发布的 snapshot，workspace/window 仍由 kernel niri 状态机提供，network/clock 仍是固定 kernel 值。interval 被验证并保留为 provider 更新策略，但没有常驻用户 provider 或真实 network/CPU/RTC polling。workspace 左击是 registry 中该 module 的直接行为，只识别完整 `{value}` label 内当前固定容量最多四个 workspace 的单字符数字。其他 module 的左键 `on-click`、右键 `on-click-right` 与中键 `on-click-middle` 会保留用户当前输入、经 ASCII 大写化后进入同一个受限桌面命令分派器；当前允许 `HELP`、`STATUS`、`ABOUT`、`CLEAR`、`RELOAD`、`SWWW-DAEMON` 和 `SWWW ...`，显式拒绝 `FAULT`、`RELOAD BAD` 与任意 shell command。Super+右键仍优先进入 niri 列缩放，不触发 bar action。尚无 `on-scroll`、POSIX shell、Pango markup/strftime、format-icons/state、完整 GTK CSS/alpha blend、per-output bar、tray、network/audio/battery backend 或 niri IPC module。parser 接受 bottom/left/right position，但 early framebuffer renderer 当前只允许 top，并在发布 VFS generation 前拒绝其他位置。
