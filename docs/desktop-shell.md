...
`/sbin/slop-shell` 现在是跨 reload 常驻的 service：初次提交 policy generation 1 并收到确认后，它等待初始 config generation 1；收到后打开 `/run/slopos/desktop-events`，不重复发布内容相同的policy，而是用Linux `poll`同时等待下一代config与Wayland socket。有效 `RELOAD` 发布 config generation 2，令 PID 2 以fd 5重读Waybar/swww并提交 policy generation 2；`RELOAD BAD` 不推进config generation，所以PID 2继续poll且不发布新policy。PID 1 在关闭自己的最后一个 fd 后常驻 `wait4` 作为 supervisor，block task 持有两者的 process/VFS runtime。

首代 `policy-applied` 后，PID 2 调用 Linux `socket(AF_UNIX, SOCK_STREAM, 0)` 与 `connect`，把 fd 3 连到 `/run/slopos/wayland-0`。12-byte `get_registry` 先换回156-byte registry；随后308-byte role/device batch绑定compositor/shm/xdg/seat/output并创建pointer/keyboard，以256+52两次普通异步`write`进入同一有界stream。服务端返回288-byte device/configure event：pointer+keyboard capability、seat name、`wl_keyboard.keymap(xkb_v1,3307)`、repeat 25/600、output/shm/xdg配置。PID 2通过Linux `recvmsg`取得前256 bytes和一个server→client `SCM_RIGHTS`只读keymap fd，再读取余下32 bytes；它分块校验keymap长度、FNV-1a、末尾NUL与EOF后关闭fd。随后`memfd_create("slopos-wayland-shm", 0)`复用fd 4，`ftruncate(3072)`并以单页`mmap(MAP_SHARED)`在`0x40007000`建立XRGB8888 backing，再用`sendmsg(SCM_RIGHTS)`把fd 4和148-byte configured batch反向发送。kernel的`slopos-wayland` dispatcher和持久session校验frame/argument/object map、seat/pointer/keyboard/output ownership、批次顺序、configure serial与首次pool descriptor，未ack的buffer commit不能进入双bank。desktop task首次合成后只发布buffer release、callback done与delete-id；PID 2解析后覆写同一共享页，再以普通64-byte `write` 复用buffer 8与callback ID 11且不重复传fd，第二次合成再次发布32-byte presentation event，最后关闭memfd descriptor。初始config generation到达后，PID 2以fd 4打开event descriptor并永久poll fd 3/4；System获得实际焦点时才收到keyboard enter/modifiers，之后以持久512-byte stream buffer严格解析key press/release与pointer enter/leave/motion/button/axis/frame。私有`0x534c0005` staging syscall已移除。2026-07-31由`make test-wayland-input`重新生成的`evidence/desktop.png`中，System窗口的四色surface来自第二帧共享页，脚本同时核对四点backing、实时输入marker与stream-safe poll，而不是只接受一张目视截图。

这仍不是完整的用户态桌面。PS/2 设备采集、四份配置的发现与 parse bank、niri 状态机、swww daemon/transition、placement、GOP renderer 与 composition 仍在 kernel。Wayland wire 已是 AF_UNIX byte stream，seat/output/pointer/keyboard discovery、XKB keymap、动态焦点与持续键盘/指针事件已真实往返，首个pool也已用反方向的真实 `SCM_RIGHTS`/shared mmap，PID 2则以Linux poll统一等待socket/config；但尚无用户态`bind/listen/accept`、通用mmap/munmap、subsurface/layer-shell、动态placement、持续frame loop、普通第三方client或多client object ownership。
