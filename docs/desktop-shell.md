...
- 每个 workspace 同时保存不随 viewport 滚动、始终合成在 tile 上方的 floating layer；`toggle-window-floating` 在两层间移动 focused window，`switch-focus-between-floating-and-tiling` 只切层焦点，方向 focus/move、显式/preset resize、跨 workspace move、标题栏拖动和 Super+右键缩放均按 active layer 分派；
...
当前 kernel desktop 用三个固定 surface 演示这些行为：Terminal/System 位于 `main`，Config 的 `app-id` 规则把它放到 `config` 并以 `open-floating false` 明确保持 tiled；顶部 workspace module 显示 active index。PS/2 parser 跟踪 Super/Ctrl/Shift/Alt 与扩展方向键，使 KDL bind 实际驱动桌面。交互回归先以 `Mod+V` 把 Terminal 从 x=16/y=56/488×696 tile 变为 x=16/y=161/488×485 浮窗；System 单列自动居中到 x=268，`Mod+Shift+V` 聚焦其下的 System 时 Terminal 仍合成在最上层且使用 inactive ring，再切回 Terminal 并以 `Mod+Ctrl+J` 把浮窗向下移动到 y=211。第二次 `Mod+V` 把 Terminal 送回 x=520 tile，重排回左侧后继续其余回归。
