...
`/sbin/slop-shell` 现在是跨 reload 常驻的 service：初次提交 policy generation 1 并收到确认后，它等待初始 config generation 1；收到后打开 `/run/slopos/desktop-events`，不重复发布内容相同的policy，而是用Linux `poll`同时等待下一代config与Wayland socket。有效 `RELOAD` 发布 config generation 2，令 PID 2 以fd 7重读Waybar/swww并提交 policy generation 2；`RELOAD BAD` 不推进config generation，所以PID 2继续poll且不发布新policy。PID 1 在关闭自己的最后一个 fd 后常驻 `wait4` 作为 supervisor，block task 持有两者的 process/VFS runtime。

首代 `policy-applied` 后，PID 2 调用 Linux `socket(AF_UNIX, SOCK_STREAM, 0)`取得listener fd 3，以`bind/listen`建立 `/run/slopos/wayland-0`；再建立client fd 4、`connect`并`accept`出server fd 5。listener生命周期因此由用户态持有，但当前server fd上的Wayland protocol仍由kernel bootstrap处理。12-byte `get_registry` 先换回156-byte registry；随后308-byte role/device batch绑定compositor/shm/xdg/seat/output并创建pointer/keyboard，以256+52两次普通异步`write`进入同一有界stream。服务端返回288-byte device/configure event：pointer+keyboard capability、seat name、`wl_keyboard.keymap(xkb_v1,3307)`、repeat 25/600、output/shm/xdg配置。PID 2通过Linux `recvmsg`取得前256 bytes和一个server→client `SCM_RIGHTS`只读keymap fd 6，再读取余下32 bytes；它分块校验keymap长度、FNV-1a、末尾NUL与EOF后关闭fd。随后`memfd_create("slopos-wayland-shm", 0)`复用fd 6，`ftruncate(3072)`并以单页`mmap(MAP_SHARED)`在`0x40007000`建立XRGB8888 backing，再用`sendmsg(SCM_RIGHTS)`把fd 6和148-byte configured batch反向发送。kernel的`slopos-wayland` dispatcher和持久session校验frame/argument/object map、seat/pointer/keyboard/output ownership、批次顺序、configure serial与首次pool descriptor，未ack的buffer commit不能进入双bank。desktop task首次合成后只发布buffer release、callback done与delete-id；PID 2解析后覆写同一共享页，再以普通64-byte `write` 复用buffer 8与callback ID 11且不重复传fd，第二次合成再次发布32-byte presentation event，最后关闭memfd descriptor。初始config generation到达后，PID 2以fd 6打开event descriptor并永久poll fd 4/6；System获得实际焦点时才收到keyboard enter/modifiers，之后以持久512-byte stream buffer严格解析key press/release与pointer enter/leave/motion/button/axis/frame。私有`0x534c0005` staging syscall已移除。2026-07-31由`make test-wayland-input`重新生成的`evidence/desktop.png`中，System窗口的四色surface来自第二帧共享页，脚本同时核对四点backing、实时输入marker与stream-safe poll，而不是只接受一张目视截图。

这仍不是完整的用户态桌面。PS/2 设备采集、四份配置的发现与 parse bank、niri 状态机、swww daemon/transition、placement、GOP renderer、composition与accepted endpoint的Wayland协议分派仍在 kernel。Wayland wire 已是 AF_UNIX byte stream，用户态`bind/listen/accept`生命周期、seat/output/pointer/keyboard discovery、XKB keymap、动态焦点与持续键盘/指针事件已真实往返，首个pool也已用反方向的真实 `SCM_RIGHTS`/shared mmap，PID 2则以Linux poll统一等待socket/config；但尚无通用listener path/credentials、通用mmap/munmap、subsurface/layer-shell、动态placement、持续frame loop、普通第三方client或多client object ownership。阻塞`accept` continuation已实现但尚未在裸机路径触发。
...
Config surface 的按钮或图形 monitor 的 `RELOAD` 命令会唤醒 block task 并触发运行时 VFS 重读。缺失、超长、非法 UTF-8、parse 错误或 early renderer 不支持的非 top Waybar position 都保留上一代。`make test-wayland-input`确认config generation 1→2使desktop-event fd变为readable、Linux poll恢复并由PID 2提交policy generation 2，同时在随后持续解析Wayland输入；`make test-interaction`再以仅供诊断的`RELOAD BAD`注入非法CSS，确认config generation不推进且service不发布新policy。这证明request/reload/rollback与常驻user service串接路径，不是文件watcher或inotify。当前没有内建配置编辑器，也没有自动监听磁盘变更。
...
同一回归随后从标准 rootfs 再启动 1269-byte fixture。PID 2 initial stack 先以 `envc=4` 验证实际存在 `SLOPOS_WAYBAR_OUTPUT=SLOPOS-1`，Waybar 再展开 string `output: "$SLOPOS_WAYBAR_OUTPUT"` 命中当前显示；`width: 800` 把 bar surface 居中限制到 x=112..912，`no-center: true` 完全移除 center module，`mode: "slop-overlay"` 选择 custom `modes` 项并把有效状态覆盖为 overlay layer、`exclusive=false`、`passthrough=true`、visible。零 reserve 让 Terminal/System 从 y=16 开始，bar 在 x=600/y=39 的 `6558f5` 底边像素覆盖窗口 titlebar。x=100/112、y=10 的 `111144`/`161a2a` 锁定 fixed-width surface 左边界，x=488/y=16 的纯背景锁定 center text 未被实例化。真实左击在 x=487 的 bar surface 内命中下方 Terminal 关闭按钮并关闭 Terminal，同时没有产生 module action，直接证明受限宽度 custom mode 的 pointer 穿透。两代 policy hash 均为 `0xe1a4a589f714b968`。前两次启动都拒绝 exit/FATAL；output 与尺寸排除回归见下两段。这个 QEMU 回归证明当前支持有界非默认 niri/Waybar 文件、app-specific 几何/显示/decoration/opening state、Waybar output string/ordered-array/环境引用选择、name/output class namespace/width/no-center/margin/fixed-center/exclusive/layer/custom mode/passthrough、module placement/alternate format/点击/三键/滚轮 action 与输入保持，但不据此声称 Sway IPC、POSIX signal transport 或完整配置兼容。

第三次短启动注入 993-byte fixture：ordered `output: ["!SLOPOS-1", "*"]` 先排除当前显示，因此后面的 wildcard 不再生效。串口锁定 `selector=array entries=2 selected=false`、`visible=false` 与 `reserved_top=0`；x=100/y=10 保持 Aurora 的 `111144`，而不是默认 bar 的 `161a2a`，直接证明 surface 未被实例化。对应 PID 2 两代 policy hash 都是 `0x3d20db99f86c6b26`；第三次启动同样无 exit/FATAL。

第四次短启动注入 1011-byte fixture：`output-dimensions: ["width > 2000", "height > 700"]` 以 AND 关系检查真实 1024×768 输出，首个严格大于条件失败。串口同时记录 `output selector=any`、`output-dimensions selector=array entries=2 selected=false`、`visible=false` 与 `reserved_top=0`；x=100/y=10 仍为壁纸 `111144`。PID 2 两代 policy hash 均为 `0x248fdbf22b5028c3`，启动无 exit/FATAL；只读 `e2fsck` 在第五次启动后通过。
...
随后 `Mod+Comma` 产生 x=268 的居中 488 px stack；`Mod+W` 验证 System 2/2 与 Terminal 1/2 在 x=268/y=56/488×696 的 tabbed display 间切换，再恢复 340/340 normal stack。显式/preset 窗高覆盖 340→411→reset 340→458→221→340→269→340；`Mod+Ctrl+K/J` 与 `Mod+K/J` 分别验证同列重排/聚焦，`Mod+Period` 固定拆列，`Mod+BracketLeft/Right` 完成左合并→左拆出→右合并→右拆出。列操作还覆盖 x=16→268 居中、488→992→488 maximize、x=0/y=40/1024×728 edge maximize、488→656→488 preset、488→657 available-width expand、两条 319 px 列整体居中到 x=185、488→588→488 显式 resize、左右重排和 Super+右键 488→584→488 pointer resize。workspace 回归覆盖整体上下重排、重排后的 named 引用、keyboard/wheel 索引/名称 focus/move、末尾空位 3→4→3、previous 往返与 Waybar 数字点击。niri 配置已按 user/system/fallback 顺序从 VFS 加载并可整套原子重读；尚未实现超过 4 个 workspace、横向 wheel key、bind repeat、完整 niri action/XKB 命名、multi-output、浮窗位置跨重启持久化、tab 点击/拖曳、复杂 match、animation、overview、IPC、自动文件监听、通用 Wayland transport 或普通第三方 client。

## Waybar 式顶部栏

`modifier-reset` 接受 [Waybar Sway bar client](https://github.com/Alexays/Waybar/blob/master/src/modules/sway/bar.cpp) 的 `press`（默认）与 `release`。它只在 configured mode 为 `hide` 时启用：PS/2 parser 会把 Super 的真实 key-down/key-up 边沿送到 bar 状态机；`press` 在 key-down 时清除基础 mode 可见性，但按住期间仍由 modifier 显示 bar，释放后隐藏；`release` 则在 key-down 后等待，只有按住期间没有命中 niri binding 时才在 key-up 清除基础可见性。已接受的 keyboard/IntelliMouse niri binding 都会标记本次 chord 已有动作，output 未选中时 modifier 也不能绕过过滤。第五次 custom-config 冷启动使用 1012-byte `mode: "hide"`/`modifier-reset: "release"` fixture（Waybar hash `0x8f92f3ad4cec3e9c`）：`Super+2` 的释放记录 `action_free=false/reset_applied=false`，截图中 x=100/y=10 仍是 bar 的 `161a2a`；随后单独 Super 的释放记录 `action_free=true/reset_applied=true`，同点恢复为 Aurora 的 `111144`。此路径由 compositor 直接提供等价事件语义，仍不代表已经实现 Sway IPC transport。

当前画面使用 top bar，并按 left/center/right 三个区域显示 workspace、focused title 与 system status。root VFS 中选中的 Waybar JSONC 已实际决定 `output`、`output-dimensions`、`name`、`position`、`height`、`width`、`spacing`、`margin`/四个 `margin-*`、`fixed-center`、`exclusive` 和三个 module array；仓库默认源是 `assets/waybar-config.jsonc`。当前唯一显示以 port 名 `SLOPOS-1` 和 identifier `SlopOS Virtual Display 0x00000001` 暴露；`output` 接受一条最长 96 bytes 的 string，或最多八条的 ordered array。string 支持精确 name/identifier 与 `!` 反选；array 按出现顺序处理 `!`、精确匹配和首字符 `*` wildcard，先命中或先排除即结束。selector 在去掉可选 `!` 后若以 `$` 开头，就用剩余完整字符串查 PID 2 环境；已知值按 name/identifier 匹配，未知或不匹配后仍做 literal 比较，与 Waybar 当前 `getenv` 路径一致。当前映射完整反映 PID 2 的 `SLOPOS_ROLE`、`XDG_CURRENT_DESKTOP`、`WAYLAND_DISPLAY` 与 `SLOPOS_WAYBAR_OUTPUT` 四项实际 initial-stack environment。未指定或空 string 选择当前显示，空 array 不选择；配置了 selector 后，匹配完成前以及未命中时 effective visible 都为 false，signal 只改变保存的 mode/visibility，不能绕过 output 过滤。`output-dimensions` 同样接受一条 string 或最多八项的 array，每条按 `width|height <|> i32` 解析，所有有效条件以 AND 匹配实际输出尺寸；比较是严格小于/大于，无效 string 与 array 中的非 string 值依 Waybar 行为忽略，因而空或全无效列表会匹配。非空 `output` string 或任何 `output` array（包括空数组）优先于尺寸条件；空 `output` string 才回退到 `output-dimensions`。SlopOS 对支持字段保持有界拒绝边界：单条最多 96 bytes、最多八个有效条件，且 top-level 类型错误会拒绝整份配置。root array 多 bar 与真实多 output 尚未实现。`name` 限 32-byte ASCII 字母数字、`-`/`_`；显式值同时作为 bar CSS class 与保留的 layer-shell namespace，未指定时 namespace 回退 `waybar`、不添加 class。`margin` 接受无单位整数或 CSS 1/2/3/4-value string；与 Waybar 一样，只要任一个逐边字段存在，就完全忽略 shorthand，并把未指定边归零。有符号 margin 以 i32 保存；top renderer 用 top/left/right 约束 surface，exclusive top reserve 为 `max(0, height + margin-top)`，bottom margin 在 top position 下不参与 anchored geometry。`fixed-center true` 优先把 center block 放在 bar 绝对中心，左右空间不足时推开；false 则在 left block 末端与 right block 起点之间居中。kernel 以同一组 origin helper 计算渲染与点击，无论 `niri/workspaces` 位于哪个区域，hit-test 都沿 surface margin、CSS margin/padding、6 px glyph advance 和完整 `{value}` label 的实际位置计算。
...
当前独立测试计数为 shell 56 项、desktop protocol 11 项和 Wayland 13 项，共 80 项；Wayland 组覆盖 frame/argument/object map、core/xdg 分派、FD 边界、seat/pointer/keyboard/output server event编码与 configure-gated 单 surface lifecycle/reject 路径。
