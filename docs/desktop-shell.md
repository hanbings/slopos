...
`make test-desktop-custom-config` 复制标准磁盘，在临时 ext4 副本中把默认 904-byte Waybar 文件替换为可解析的 990-byte JSONC（增加用户注释，把 `niri/window`/`niri/workspaces` 分别移到 left/center，并给 clock 加上 `on-click: status`），再从 OVMF 启动。日志要求 PID 2 两轮都读到 990 bytes、policy generation 1/2 都携带相同的非默认 hash `0x19d79ad62f396652`、config generation 1 正常应用、服务最后阻塞在 `config-applied after_generation=1`；随后真实 PS/2 点击中央数字 `2`/`1`，终端输入未执行的 `ABO`，点击右侧 clock 触发 STATUS，再补成 `ABOUT` 执行，以证明 action 保留输入缓冲；测试同时拒绝任何 exit/FATAL，宿主最后运行 `e2fsck -fn`。这个回归证明当前支持有界非默认文件、module placement、点击几何、受限 action 与输入保持，而不是证明完整 Waybar 配置兼容。
...
JSONC parser 支持 `//` 与 `/* */` comment、trailing comma、最多 16 个 module/区域、24 个 module config。module object 当前保存 `format`、`format-alt`、`format-disconnected`、`interval`、`tooltip`、`min-length`、`max-length`、`on-click`，跳过未知 nested option，并拒绝 duplicate、非法类型/范围、非 ASCII/空 action 与冲突长度。format renderer 支持 `{}`、named replacement 与 `:>N` 右对齐；当前 provider 实际提供 `{value/name/index/total}`、`{title}`、`{usage}`、`{percentage}`、`{ifname}`。
