...
`make test-desktop-custom-config` 复制标准磁盘，在临时 ext4 副本中同时替换两份配置：4053-byte 默认 niri KDL 被改成单页上限内的 4083-byte 用户文件，把 Config 规则的 `open-maximized false`、`open-maximized-to-edges false`、`open-fullscreen false` 与 `open-focused false` 都覆盖为 `true`，把 app-specific `default-column-width` 从 0.5 改为 0.667、`default-window-height` 从 1.0 改为 0.5，并把 `default-column-display` 从 `normal` 改为 `tabbed`、加入 `focus-ring { off; }`；904-byte Waybar 文件则替换为可解析的 1300-byte JSONC（增加用户注释，把 `niri/window`/`niri/workspaces` 分别移到 left/center，并给 clock 加上 `format-alt: UTC ALT`、`on-click: status`、`on-click-right: help`、`on-click-middle: swww query` 及两向 scroll action），再从 OVMF 启动。config generation 1 的 marker 证明 `open-focused true` 先让原本位于后台 workspace 2 的 Config 成为活动窗口，再按底层到顶层记录 x=16/y=56/992×340 column maximize、x=0/y=40/1024×728 edge maximize 与 x=0/y=0/1024×768 fullscreen；首张截图确认无需可见 Waybar 点击即可显示 Config，Waybar、装饰和兄弟窗全被覆盖。真实 `Mod+Shift+F` 先恢复 edge 几何与 Waybar，`Mod+M` 再恢复 992×340，随后两次 `Mod+F` 产生 656→992 px marker；独立截图证明按 `(1024−16)×0.667−16` 恢复 x=184 的规则列宽，同时按 `(768−40−16)×0.5−16` 保留 340 px 规则高度与 tab 指示条，同时规则 marker 与截图证明 Config 的青色 focus ring 已关闭。日志还要求 IntelliMouse 握手报告 `wheel=true`，PID 2 两轮都读到 1300 bytes、policy generation 1/2 都携带相同的非默认 Waybar hash `0xb001e9dac40c640b`、服务最后阻塞在 `config-applied after_generation=1`；退出 fullscreen 后真实 PS/2 点击中央数字 `1`/`2`/`1` 往返，证明其他 workspace 的旧 Terminal 局部焦点没有被强制聚焦规则覆盖。终端输入未执行的 `ABO`，左击右侧 clock 同时把 `UTC` 切成 `UTC ALT` 并触发 STATUS，再补成 `ABOUT` 执行，以证明焦点、格式切换与 action 均保留输入缓冲；右击触发 HELP、中击触发 SWWW QUERY，向上/向下滚轮分别无过渡切到 Sunset/Aurora，第二次左击同时执行 STATUS 并切回 `UTC`。测试同时拒绝任何 exit/FATAL，宿主最后运行 `e2fsck -fn`。这个回归证明当前支持有界非默认 niri/Waybar 文件、app-specific 初始宽高/动态列显示/显式 opening focus/三级 opening state 恢复、module placement、alternate format、点击几何、左/右/中/滚轮受限 action 与输入保持，而不是证明完整配置兼容。
