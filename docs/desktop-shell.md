...
- `--outputs/-o`、`--resize crop|fit|no|stretch`、等价于 resize no 的 `--no-resize`、`--fill-color RRGGBB`、九向 `--crop-gravity`、`--transition-type/-t`、`--transition-step`、`--transition-fps`、`--transition-duration`、`--transition-angle`、`--transition-pos` 与 `--invert-y true|false`；
