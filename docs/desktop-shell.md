...
首代 `policy-applied` 后，PID 2 调用 Linux `socket(AF_UNIX, SOCK_STREAM, 0)` 与 `connect`，把 fd 3 连到 `/run/slopos/wayland-0`。12-byte `get_registry`、216-byte role batch、148-byte configured batch和64-byte buffer-reuse batch都经普通异步 `write` 进入有界 stream；156-byte registry、56-byte configure与两次32-byte presentation event经普通异步 `read` 返回。kernel 的 `slopos-wayland` dispatcher和持久 session 校验 frame/argument/object map、批次顺序与 configure serial，未 ack 的 buffer commit不能进入双 bank。desktop task 合成后发布 `wl_buffer.release`、`wl_callback.done(1)` 与 `wl_display.delete_id(11)`；PID 2 解析后复用 buffer 8 与 callback ID 11，第二次合成发布 generation/callback data 2。两帧的3072-byte XRGB8888 backing 暂由 `0x534c0005` staging syscall提供。2026-07-31 重新生成的 `evidence/desktop.png` 中，System 窗口的四色 surface来自第二帧用户态像素；`capture-desktop.sh` 同时读取 `(900,123)`、`(948,123)`、`(900,159)`、`(948,159)` 四点验证 backing，而不是只接受一张目视截图。

这仍不是完整的用户态桌面。PS/2 输入、四份配置的发现与 parse bank、niri 状态机、swww daemon/transition、placement、GOP renderer 与 composition 仍在 kernel。Wayland wire 已是 AF_UNIX byte stream，但尚无用户态 `bind/listen/accept`、`SCM_RIGHTS`、真实共享映射、subsurface/layer-shell、动态 placement、持续 frame/event loop、普通第三方 client 或多 client object ownership；像素 staging ABI也尚未移除。
