...
首代 `policy-applied` 后，PID 2 调用 Linux `socket(AF_UNIX, SOCK_STREAM, 0)` 与 `connect`，把 fd 3 连到 `/run/slopos/wayland-0`。12-byte `get_registry` 先换回156-byte registry；随后308-byte role/device batch绑定compositor/shm/xdg/seat/output并创建pointer/keyboard，以256+52两次普通异步`write`进入同一有界stream。服务端返回288-byte device/configure event：pointer+keyboard capability、seat name、`wl_keyboard.keymap(xkb_v1,3307)`、repeat 25/600、output/shm/xdg配置。PID 2通过Linux `recvmsg`取得前256 bytes和一个server→client `SCM_RIGHTS`只读keymap fd，再读取余下32 bytes；它分块校验keymap长度、FNV-1a、末尾NUL与EOF后关闭fd。随后`memfd_create("slopos-wayland-shm", 0)`复用fd 4，`ftruncate(3072)`并以单页`mmap(MAP_SHARED)`在`0x40007000`建立XRGB8888 backing，再用`sendmsg(SCM_RIGHTS)`把fd 4和148-byte configured batch反向发送。kernel的`slopos-wayland` dispatcher和持久session校验frame/argument/object map、seat/pointer/keyboard/output ownership、批次顺序、configure serial与首次pool descriptor，未ack的buffer commit不能进入双bank。desktop task首次合成后发布`wl_pointer.enter(pointer=14,surface=6,x=16,y=12)`、`wl_keyboard.enter(keyboard=15,surface=6,keys=empty)`、buffer release、callback done与delete-id；PID 2解析后覆写同一共享页，再以普通64-byte `write` 复用buffer 8与callback ID 11且不重复传fd，第二次合成发布generation/callback data 2，最后关闭memfd descriptor。私有`0x534c0005` staging syscall已移除。2026-07-31重新生成的`evidence/desktop.png`中，System窗口的四色surface来自第二帧共享页；`capture-desktop.sh`同时读取四点验证backing，而不是只接受一张目视截图。

这仍不是完整的用户态桌面。PS/2 输入、四份配置的发现与 parse bank、niri 状态机、swww daemon/transition、placement、GOP renderer 与 composition 仍在 kernel。Wayland wire 已是 AF_UNIX byte stream，seat/output/pointer/keyboard discovery、XKB keymap与首次pointer/keyboard focus已真实往返，首个pool也已用反方向的真实 `SCM_RIGHTS`/shared mmap；但尚无持续pointer/key/modifiers input fan-out、用户态poll/`bind/listen/accept`、通用mmap/munmap、subsurface/layer-shell、动态placement、持续frame loop、普通第三方client或多client object ownership。
