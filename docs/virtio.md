...
文件系统以一个双块 cache prefetch 实际同时发布两个请求；VFS partial-write、JBD2 record/state 和 active data/metadata transaction probes 均串行使用 write/flush。8-entry cache 最终记录 64 hit/54 miss/5 invalidation；所有 direct journal/state I/O 绕过 cache。裸机 `make test-boot` 验证 clean boot 的 193 个请求由 192 次 INTx/ISR/Future completion 唤醒完成；`make test-journal-replay` 的 recovery boot 则完成 205/204。当前 root disk 为 256 MiB，即 524288 个 512-byte sector。
