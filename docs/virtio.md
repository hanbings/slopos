...
文件系统以一个双块 cache prefetch 实际同时发布两个请求；VFS partial-write、JBD2 record/state 和 active data/metadata/block-allocation/create transaction probes 均串行使用 write/flush。8-entry cache 最终记录 74 hit/69 miss/16 invalidation；所有 direct journal/state I/O 绕过 cache。裸机 `make test-boot` 验证 clean boot 的 447 个请求由 446 次 INTx/ISR/Future completion 唤醒完成；五 tag allocation recovery 后继续完整 probes 的 boot 则完成 477/476。当前 root disk 为 256 MiB，即 524288 个 512-byte sector。
