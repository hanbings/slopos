...
共享 `slopos-virtio` crate 对 split-ring byte layout、power-of-two queue size，以及从任意合法 head 开始的三 descriptor block-read chain 做宿主单元测试。文件系统以一个双块 cache prefetch 实际同时发布两个请求；加入 VFS chunk/seek 路径后，8-entry cache 最终记录 50 hit/41 miss。另加未缓存的 superblock，裸机 `make test-boot` 验证 42 次 DMA 请求由 41 次 INTx/ISR/Future completion 唤醒完成。当前 root disk 为 256 MiB，即 524288 个 512-byte sector。
