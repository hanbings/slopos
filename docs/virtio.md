...
文件系统以一个双块 cache prefetch 实际同时发布两个请求；VFS PID 1/2 ELF、init 与 desktop service 的用户态配置读取、PID 1 的可逆 write probe、kernel 桌面 reload、partial-write、JBD2 record/state 和 active data/metadata/block-allocation/create transaction probes 均经同一 transport，mutation 路径串行使用 write/flush。100 Hz user preemption 和 desktop apply event wake 会改变两个进程与后续 cache probe 的合法交错；clean-boot/interaction 证据覆盖 157–163 hit、119–128 miss、16–18 invalidation，并观测到 497/496 至 510/509 的 request/queue-interrupt 计数。测试不固定某一种 interleaving，而是核对 `requests = interrupts + 1` 且 ISR/Future 与 queue interrupt 计数相等；cache summary 另只接受由两次用户写相对 probe summary 的合法先后产生的 16–18 invalidation。五 tag allocation recovery 的当前证据为 543/542。所有 direct journal/state I/O 绕过 cache；当前 root disk 为 256 MiB，即 524288 个 512-byte sector。
