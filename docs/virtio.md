...
文件系统以一个双块 cache prefetch 实际同时发布两个请求；VFS partial-write、JBD2 record 和 recovery/state probes 均串行使用 write/flush。8-entry cache 最终记录 62 hit/49 miss/2 invalidation；所有 direct journal/state I/O 绕过 cache。裸机 `make test-boot` 验证 83 个请求由 82 次 INTx/ISR/Future completion 唤醒完成。当前 root disk 为 256 MiB，即 524288 个 512-byte sector。
