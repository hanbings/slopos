...
共享 `slopos-virtio` crate 对 split-ring byte layout、power-of-two queue size，以及从任意合法 head 开始的三 descriptor block-read chain 做宿主单元测试。文件系统以一个双块 cache prefetch 实际同时发布两个请求；加入 depth-1 extent 路径后，8-entry cache 最终记录 26 hit/23 miss。另加未缓存的 superblock，裸机 `make test-boot` 验证 24 次 DMA 请求由 23 次 INTx/ISR/Future completion 唤醒完成。当前 root disk 为 256 MiB，即 524288 个 512-byte sector。
