...
文件系统以一个双块 cache prefetch 实际同时发布两个请求；VFS partial-write probe 执行两次 read-modify-write，JBD2 probe 再 stage/readback/restore 三个 journal records。8-entry cache 最终记录 62 hit/49 miss/2 invalidation；所有 direct journal I/O 绕过 cache。裸机 `make test-boot` 验证 69 个请求由 68 次 INTx/ISR/Future completion 唤醒完成。当前 root disk 为 256 MiB，即 524288 个 512-byte sector。
