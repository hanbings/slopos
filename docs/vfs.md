...
- 每个 descriptor 保存 file node、local-socket generation handle、shared-memory generation handle或desktop-event generation cursor；file与shared-memory object都保存offset和read/write access mode，收到的keymap descriptor与event stream只能read；
...
PID 1 与 PID 2 通过 `sched_yield` cooperative 交错，也在各自无 syscall TSC 窗口被 100 Hz timer 双向抢占。PID 1 发出 Linux x86-64 `openat(AT_FDCWD, "/etc/slopos/system.conf", O_RDONLY)`，root namespace 解析 inode 18；PID 2 首轮依次打开 inode 20 `/etc/slopos/waybar.jsonc` 与 inode 17 `/etc/slopos/swww.env`。fast handler 保存对应 user frame并回到 block task，各自容量 8 的 fd table 都可返回 fd 3；`Ext4File` 存在按 PID 分隔的 backing array 中，因此同号 fd 不会碰撞。首代 policy 后 PID 2 用 `socket/bind/listen`把listener固定在fd 3，再以fd 4 `socket/connect`并`accept`出fd 5；server先经`recvmsg(SCM_RIGHTS)`把只读keymap object安装为fd 6，desktop按共享offset读到EOF并关闭。随后`memfd_create`复用fd 6，该surface object再经`sendmsg(SCM_RIGHTS)`反向附着到Wayland batch并在第二帧后关闭。初始config event后，PID 2以`openat`把只读`/run/slopos/desktop-events`固定在fd 6，descriptor cursor从当前generation开始；下一代config使其`POLLIN` ready，32-byte read推进cursor，配置文件热重读使用fd 7。file read暂停上下文，异步ext4/virtio completion推进offset；socket read/write/sendmsg/recvmsg、accept与poll走相同suspend/resume boundary，但分派到listener FIFO、全双工byte ring、rights slot、event cursor与Wayland server Future。completion都把结果写回对应user stack/frame并恢复原RIP/RSP/GPR。
...
`make test-vfs` 的 8 项宿主测试覆盖路径规范化、root/`/mnt`/`/mnt/data` 最长挂载匹配、fd offset生命周期、EOF growth、access mode，以及socket/shared-memory/desktop-event object区分、shared object offset/EOF与event generation cursor。`make test-boot` 验证同一状态机驱动真实ext4 cache、virtio DMA、AF_UNIX Wayland stream、只读keymap descriptor、shared backing descriptor和config event stream。

`slopos-process` 的每个 process slot 内嵌一张独立 `FileDescriptorTable`。`make test-process` 会生成 parent/child 两个 record，让它们对同一 `FileNode` 各自取得 fd 3，再给 parent插入generation-checked socket与shared-memory object并验证child的同号fd仍是file；process exit后所有descriptor operation都被拒绝。裸机PID 2已让listener/client/accepted-server fd 3/4/5常驻、keymap/memfd/event-stream复用fd 6、热重读file使用fd 7；fd 1仍由syscall handler特判，`Ext4File` backing object暂存在block task的固定二维数组中。
...
- 用户 syscall 目前只有 root regular-file与只读desktop-event pseudo-file的`openat`、最多256-byte且限于四页code/三页stack/一页shared mapping的read/write、`SEEK_SET lseek`、close、`sched_yield`，以及特判stdout write；Wayland另有严格限定的socket/bind/listen/connect/accept、memfd_create/ftruncate、单页固定地址MAP_SHARED mmap、单iovec/单rights-fd sendmsg和对应的recvmsg。Linux poll只支持最多4项、POLLIN、timeout -1/0与socket/event descriptor。listener只允许PID 2、固定path和有界backlog，accept只支持空peer-address输出；没有通用grow/truncate/stat、directory fd、dup、epoll、anonymous/file mmap、munmap、owner/mode权限或任意并发请求；
