...
| 规格范围 | 状态 | 当前证据与边界 |
|---|---|---|
| 原生图形桌面 | 部分实现 | QEMU 中三个 surface 已进入独立 workspace 的 niri 式 horizontal column strip；列宽稳定、normal/tabbed column display、KDL preset 列宽正反循环、可恢复的 maximize-column、focused window 无 gap edge maximize/restore、精确 single-column/显式 focused/完整可见集合居中、按完整可见列计算的 expand-column-to-available-width、固定与左右方向感知的 consume/expel、同列上下 focus/reorder/height explicit+preset resize/reset、edge scroll、bind column resize/reorder/close、按索引或名称直接切换/移列、previous workspace 往返、容量 4 内的动态尾部空 workspace、Super+右键 pointer resize、window rule、titlebar drag viewport 与顶部 Waybar workspace 点击已自动验证。列宽/窗高 proportion 已采用 niri gap-aware 几何；`Mod+W` 的裸机证据为 340/340 stack → x=268/y=56/488×696 的 System 2/2 tab → 同几何 Terminal 1/2 tab → 原 stack；`Mod+M` 的证据为 x=16/y=56/488×696 → x=0/y=40/1024×728 → 原几何；`Mod+BracketLeft/Right` 完成左合并→左拆出→右合并→右拆出且焦点跟随。PID 2 `/sbin/slop-shell` 已作为常驻服务从 VFS 读取 Waybar/swww 配置、连续发布 provider/wallpaper snapshot，并通过 `policy-applied`/`config-applied` 双向事件跨 reload 休眠与恢复；compositor、surface 与 renderer 仍是 kernel early mechanism，不是用户态 compositor。 |
...
| 图形系统与 Wayland | 部分实现 | framebuffer renderer、niri 式滚动平铺/workspace/bind/rule/normal+tabbed column stack/fixed+directional consume/expel/window focus+reorder+explicit/preset height+reset+edge maximize/column explicit+preset+maximize+single/focused/visible center+available-width resize+reorder/direct indexed+named move/previous toggle、Waybar JSONC option/format/受限三键/滚轮 action + CSS 顶栏及 workspace 数字点击、swww 风格 daemon state/CLI/PNM/纯色 clear/CPU transition 可用；PID 2 经 40-byte commit 连续发布 CPU/Memory 与 Aurora policy，再经两类 32-byte event 收到 policy/config 实际应用确认，34 项 shell + 5 项 protocol 测试及真实跨 reload service、workspace/规则/stack/tabbed/方向合并拆列/缩放/column+edge maximize/单列与可见集合居中/余宽扩展/重排/点击/action/样式/换图/纯色/query/kill/restart 通过。仍无 Wayland wire/object/global/xdg、真实 Waybar hardware provider/POSIX shell action/完整 GTK CSS；常驻 PID 2 是 policy/provider，不是用户态 compositor、独立 bar surface 或 layer-shell client。 |
...
- 最后成功 shell/protocol 单元测试：2026-07-30，`make test-shell`，39 项（shell 34 + desktop protocol 5）。
