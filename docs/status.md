...
| VFS 与文件描述符 | 部分实现 | `no_std` path/mount/fd crate 有 5 项宿主测试；QEMU 把 ext4 挂到 `/` 并取得两个实际 user image。PID 1/2 的独立 fd table 在交错执行中同时各自取得 fd 3 读取配置；PID 1 另以 `O_RDWR` + `lseek/write/read` 对 inode 31 完成跨两页的可逆 64-byte patch并显式 close。PID 2 每次有效 config generation 都重新 open/read/close Waybar 与 swww；swww image broker 另让同一个 block task 按命令路径跨两个 block 读取 inode 30 的 6144-byte PNG。kernel probe fd 另可 EOF 单块 append/truncate，并以读写模式打开刚创建的空文件。mount/backing object 仍是单一 block task 专用，用户 copy 限于已知三页 code/三页 stack mapping 与 256 bytes，尚非通用 POSIX VFS。 |
...
| 图形系统与 Wayland | 部分实现 | framebuffer renderer、niri 式 tiled/floating workspace、Waybar/swww 子集继续可用。`slopos-wayland` 真实 wire/object server 现在维持分阶段单-client session：PID 2 先解析五个 registry global，再做无 buffer 的初始 role commit；服务端发送 shm formats、toplevel geometry 与 xdg configure serial，客户端正确 `ack_configure` 后才可提交 32×24 XRGB8888 buffer。双 bank 合成 3072-byte 用户像素后，服务端再发 buffer release、frame callback done 与 delete-id，PID 2 验证 presentation 后才常驻。11 项 desktop protocol、13 项 Wayland 测试、三次 QEMU block/wake lifecycle marker、journal replay及四点 framebuffer 取样通过。当前 transport 仍是版本化单-client syscall 信封和内联 backing snapshot，不是 Unix socket/`SCM_RIGHTS`/共享 mmap；也无任意第三方 client、多 client ownership、通用重复 buffer lifecycle、client-driven xdg fullscreen/maximize、subsurface/popup、独立 bar/layer-shell surface 或用户态 compositor。 |
...
下一阶段以已验证的双向 configure/presentation lifecycle 为基础，建立 Unix-domain/local socket、真实 FD/shared mapping 和多 client object ownership，并把一次性 snapshot 扩为可重复 buffer attach/release/frame cycle；之后才逐步把固定 placement、bar/wallpaper mechanism 移出 kernel。通用进程主线仍需任意路径/多 `PT_LOAD` exec、mmap、通用 wait/signal、独立线程/kernel stack 与 SMP run queue。
