...
| 图形系统与 Wayland | 部分实现 | framebuffer renderer、niri 式滚动平铺/workspace/bind/rule、Waybar JSONC option/format + CSS 顶栏、swww 风格 daemon state/CLI/PNM/CPU transition 可用；PID 2 经 40-byte protocol 发布 CPU/Memory 与初始 Aurora policy，25 项 shell + 3 项 protocol 测试及真实 workspace/规则/样式/换图/query/kill/restart 通过。仍无 Wayland wire/object/global/xdg、真实 Waybar hardware provider/完整 GTK CSS；PID 2 是一次性 policy producer，不是常驻 compositor/bar/wallpaper daemon 或独立 layer-shell client。 |
...
| Linux x86-64 ABI | 部分实现 | root VFS 中两个独立 Rust ELF64 `ET_EXEC` CPL3 进程使用 Linux x86-64 initial stack/register/syscall number，真实解析各自 `argc/argv/envp/auxv` 并执行 `openat/read/write/lseek/close/sched_yield/wait4/exit`；文件 I/O 保存 per-PID user frame、经异步 ext4/virtio 完成后 `SYSRETQ` 恢复，wait4 支持 child-exit 前后的两种时序。10 项 ELF 与 6 项 process 测试覆盖 parser/initial stack/lifecycle/scheduler/child lookup/fd isolation/cleanup，QEMU 实测 syscall 与 interrupt 两种 frame 的 CR3 switch、同号 fd isolation、跨两页 copy-user 与 wait status copy。desktop commit 另属 SlopOS 私有 syscall ABI。尚无任意路径/多 segment exec、通用页表/demand-page copy、广泛 syscall surface、proxy 或 guest agent。 |
| 网络与 IPC | 尚未实现 | Ethernet/ARP/IP/TCP/UDP/DHCP/DNS 尚未实现；desktop policy commit 只是 PID 2→kernel 的单向、一次性固定消息，不是通用 IPC。 |
...
- 最后成功 shell/protocol 单元测试：2026-07-30，`make test-shell`，28 项（shell 25 + desktop protocol 3）。
