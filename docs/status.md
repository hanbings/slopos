...
| 进程/线程/调度 | 部分实现 | 两个独立 Rust ELF64 从 root `/sbin/slop-init` 与 `/sbin/slop-shell` 分别跨七/九块读入，经严格单 `PT_LOAD` parser 装入独立 CR3；init 另与 UEFI/BootInfo v2 副本比较。kernel 为 PID 1/2 各构造 `argc/argv/envp/auxv`、两页 code/两页 stack、pending syscall/frame 与容量 8 的 fd table。容量 4 的 process table 支持 Ready/Running/Blocked/Runnable/Exited；除 `sched_yield` round-robin 外，100 Hz PIT 已验证双向 user preemption。PID 1 常驻 `wait4`，PID 2 常驻 config event并已从用户态提交 Wayland surface。仍无任意路径/multi-segment exec、thread/kernel preemption、通用 wait/kill/signal、orphan adoption 或 PID reuse。 |
| 内存管理 | 部分实现 | kernel 解析 firmware descriptor stride，建立 frame allocator、自有四级页表并切换 CR3，建立 1 MiB bump heap；PID 1/2 各使用 private PML4、supervisor kernel mapping、两页 user read-only code/two-page writable stack，per-PID bounded copy、跨 code page ELF 装载与跨两个独立 stack physical frame 的 user copy已在 QEMU 验证。reap 释放四个 table、两个 code、两个 stack，共 8 frame。没有 NX、kernel section W^X、COW、demand paging或通用映射回收。 |
...
| 图形系统与 Wayland | 部分实现 | framebuffer renderer、niri 式 tiled/floating workspace、Waybar/swww 子集继续可用。新增 `slopos-wayland` 真实 wire/object server：PID 2 编码 332-byte 标准 registry/compositor/shm/xdg-shell request stream，创建 32×24 XRGB8888 `xdg_toplevel`，经严格对象关系、stride/format/damage/frame/commit 验证后由双 bank发布 3072-byte 用户像素并合成到 System 窗口。8 项 desktop protocol、12 项 Wayland 测试、QEMU lifecycle marker及四点 framebuffer 取样通过。当前 transport 仍是版本化单-client syscall 信封和内联 backing snapshot，不是 Unix socket/`SCM_RIGHTS`/共享 mmap；也无 server registry/configure event、ack-configure、buffer release、任意第三方 client、client-driven xdg fullscreen/maximize、subsurface/popup、独立 bar/layer-shell surface 或用户态 compositor。 |
...
- 最后成功 shell/protocol 单元测试：2026-07-31，`make test-shell`，64 项（shell 56 + desktop protocol 8）。
- 最后成功 Wayland 单元测试：2026-07-31，`make test-wayland`，12 项。
...
下一阶段以已验证的单 client 标准 request stream 为基础，补 server→client registry/xdg configure event、ack-configure、buffer release，再建立 Unix-domain/local socket、真实 FD/shared mapping 和多 client object ownership；之后才逐步把固定 placement、bar/wallpaper mechanism 移出 kernel。通用进程主线仍需任意路径/多 `PT_LOAD` exec、mmap、通用 wait/signal、独立线程/kernel stack 与 SMP run queue。
