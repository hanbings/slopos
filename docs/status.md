...
| 异步内核 | 部分实现 | 三任务 `Future` executor、task-ready bit queue、RawWaker、PIT timer、PS/2 input 与 virtio block INTx→waker→completion 已在 QEMU 运行；PID 1 的同步 `openat/read/write/close` 会保存 user frame、返回 block task 异步等待 I/O，再恢复 CR3/frame，而不是 busy-wait；`lseek` 只更新 descriptor offset。动态 task arena、timer wheel、locks、cancellation、通用 backpressure 和 SMP 尚未实现。 |
| 进程/线程/调度 | 部分实现 | 独立 `userspace/init` ELF64 从 root `/sbin/slop-init` 跨七块读入，并与 UEFI/BootInfo v2 校验副本完全比较后经严格 `PT_LOAD` parser 装入；kernel 构造 16-byte aligned `argc/argv/envp/auxv`，PID 1 进入 Rust 前实际核对。容量 4 的 process table 保存 PID/parent/image/initial RSP、Ready→Running→Exited、status/syscall count 与每 slot 容量 8 的 VFS fd table。PID 1 以独立 CR3、user code/two-page stack 和 CPL3 运行，可为异步文件 syscall 暂停/恢复；exit 后已核对并释放 fd/backing object，最终回到 kernel continuation。仍无任意路径/multi-segment exec、第二个实际 process、thread、scheduler、preemption、通用 context switch、wait/signal 或地址空间回收。 |
| 内存管理 | 部分实现 | kernel 解析 firmware descriptor stride，建立 frame allocator、自有四级页表并切换 CR3，建立 1 MiB kernel bump heap；PID 1 使用 private PML4、supervisor kernel mapping、user read-only code/two-page writable stack，跨两个独立 physical frame 的有界 user copy 已在 QEMU 验证；frame/heap 读回与真实 vector-14 diagnostic 也已验证。没有 NX、kernel section W^X、COW、demand paging 或回收。 |
...
| VFS 与文件描述符 | 部分实现 | `no_std` path/mount/fd crate 有 5 项宿主测试；QEMU 把 ext4 挂到 `/`，从 `/sbin/slop-init` 取得实际 PID 1 image。PID 1 的独立 fd table 先以 `O_RDONLY openat/read/close` 异步读取配置，再以 `O_RDWR` + `lseek/write/read` 对 inode 31 完成跨两页的可逆 64-byte patch；退出时 `close_all` 与 backing-object 数量一致。kernel probe fd 另可 EOF 单块 append/truncate，并以读写模式打开刚创建的空文件。mount/backing object 仍是单一 block task 专用，用户 copy 限于已知 code/two-page stack mappings 与 256 bytes，尚非通用 POSIX VFS。 |
...
下一阶段把当前全局单进程保存点改成按 PID 保存的 runnable/suspended context，装载第二个真实 VFS user image，并先以可重复验证的 cooperative switch 证明两个独立 CR3、stack、fd table 与退出清理；随后接入 timer preemption、wait/reap 和 frame/page-table 回收，再扩展任意路径/多 `PT_LOAD` exec。桌面兼容主线还需动态 workspace、完整 action/rule/output/IPC，Waybar 真实 provider/Pango/action/完整 GTK CSS，以及 swww 任意 VFS image、PNG/JPEG/GIF、多 output 和真正的用户态 IPC/layer-shell。
