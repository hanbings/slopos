...
| 图形系统与 Wayland | 部分实现 | framebuffer renderer、niri 式 tiled/floating workspace/keyboard+wheel bind/cooldown/ordered open+focus+width+height+display+focus-ring+border+shadow+background-mode+opacity+maximize+fullscreen rule/normal+tabbed stack/fixed+directional consume/expel/focus+reorder+explicit/preset resize+column/edge maximize+fullscreen+center/expand/direct move、首末列 focus/reorder、完整 workspace reorder，以及语义分离的整列/单窗跨 workspace 转移可用；Waybar JSONC height/spacing/margin/fixed-center/exclusive/layer/preset+custom mode/start_hidden/visible/passthrough/on-sigusr1+on-sigusr2 action + normal/alternate format + format-alt-click + 受限三键/滚轮 action + CSS 顶栏及 workspace 点击、swww daemon state/CLI/P3+P6 PNM/non-interlaced+Adam7 的 1/2/4/8/16-bit 灰度、8/16-bit 灰度透明/RGB/RGBA及 1/2/4/8-bit indexed palette+`tRNS` PNG/clear/crop-fit-no-stretch/fill/crop-gravity/filter/CPU transition（含 wipe angle、wave 宽高、grow/outer position、invert-y、小数 duration×fps 有界采样与 cubic Bezier easing）与 8 KiB root VFS path load 可用。49 项 shell + 5 项 protocol 测试及真实 custom niri 聚焦/几何/decoration/fullscreen 三级恢复、Waybar surface/work-area/center/top-over-fullscreen/overlay-click-through、Mod+IntelliMouse、workspace identity reorder、full-output restore、strip-boundary、column/window transfer、floating/tabbed/跨 reload/action/样式/异步 PNG 换图回归通过。仍无可连接 Wayland transport/shared surface buffer、client-driven xdg fullscreen/maximize、Waybar Sway IPC/POSIX signal delivery、真实 hardware provider/POSIX shell action/完整 GTK CSS；PID 2 是 policy/provider，不是用户态 compositor、独立 bar surface 或 layer-shell client。 |
| 声明式配置 | 部分实现 | niri KDL、Waybar JSONC/CSS 与 swww environment 按 user/system/fallback 顺序从 root VFS 发现；双 static bank 会先验证四份文本，再按 generation 原子发布。交互测试已验证 config generation 1→2 唤醒 PID 2、policy generation 2→3，以及非法 CSS 保留 generation 2且不唤醒服务；custom-config 回归同时注入单页上限内的 4082-byte niri 与 1392-byte Waybar override，验证 ordered niri window rule 的实际焦点、几何、显示、decoration/alpha/fullscreen 例外、三级恢复与浮动位置记忆，并验证 Waybar signed margin/fixed-center/exclusive/top layer、PID 2 有界分块 hash/连续发布、left/center module placement、alternate format 和三键/双向滚轮 action；1189-byte custom `slop-overlay` fixture 另验证 mode object 展开、零 reserve 与 pointer passthrough。仍无文件 watcher/inotify、普通配置编辑器、结构化 diff，配置发现/parser 也仍属于 kernel service。 |
...
- 最后成功 shell/protocol 单元测试：2026-07-31，`make test-shell`，54 项（shell 49 + desktop protocol 5）。
