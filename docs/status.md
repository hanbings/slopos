...
| 进程/线程/调度 | 部分实现 | 独立 `userspace/init` ELF64 从 root `/sbin/slop-init` 跨两块读入，并与 UEFI/BootInfo v2 校验副本完全比较后经严格 `PT_LOAD` parser 装入；容量 4 的 process table 保存 PID/parent/image、Ready→Running→Exited、status/syscall count 与每 slot 容量 8 的 VFS fd table。PID 1 以独立 CR3、user code/stack page 和 CPL3 运行后恢复 kernel continuation。仍无任意路径/multi-segment exec、第二个实际 process、thread、scheduler、preemption、context switch、wait/signal 或资源回收。 |
...
| VFS 与文件描述符 | 部分实现 | `no_std` path/mount/fd crate 有 5 项宿主测试；QEMU 把 ext4 挂到 `/`，从 `/sbin/slop-init` 取得实际 PID 1 image，fd 3 可读/seek/原位覆写、EOF 单块 append/truncate，并以读写模式打开刚创建的空文件；block task 还从 root VFS 发现并读取四份桌面配置。fd/namespace 仍是 block task 局部、root-only 状态，不是可复用或每进程 POSIX VFS。 |
| 设备与驱动 | 部分实现 | GOP、COM1、QEMU debugcon、PS/2 键鼠可用；校验 XSDT/MADT，自有 GDT/IDT、xAPIC/IOAPIC、100 Hz PIT；PCI/modern virtio-blk 支持 read/write/flush，clean boot 的 463 个请求由 462 次 INTx 唤醒完成。没有通用 descriptor allocator、MSI-X、其他设备类或 application processor。 |
...
| Linux x86-64 ABI | 部分实现 | root VFS 中的独立 Rust ELF64 `ET_EXEC` CPL3 PID 1 使用 Linux x86-64 register/syscall number，发出真实 `SYSCALL`；kernel 配置/readback EFER/STAR/LSTAR/FMASK，write 经 `SYSRETQ` 返回，exit 回到 kernel。10 项 ELF 与 4 项 process 测试覆盖 parser/lifecycle/fd isolation。尚无任意路径/多 segment exec、通用 copy-user/VFS syscall surface、proxy 或 guest agent。 |
...
下一阶段把 process table 的每进程 fd 真正接入用户 open/read/write/close syscall，再把当前固定 `/sbin/slop-init`、强制等于 BootInfo 副本的单 `PT_LOAD` 装载器扩展为任意 VFS executable，并补进程初始 stack/`argc`/`argv`/`envp`/auxv。其后才是可抢占 context switch、wait/资源回收。桌面兼容主线还需动态 workspace、完整 action/rule/output/IPC，Waybar 真实 provider/Pango/action/完整 GTK CSS，以及 swww 任意 VFS image、PNG/JPEG/GIF、多 output 和真正的用户态 IPC/layer-shell。
