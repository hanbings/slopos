...
| 进程/线程/调度 | 部分实现 | 内嵌 ELF64 PID 1 经严格 `PT_LOAD` parser 装入，以独立 CR3、user code/stack page、CPL3、TSS `RSP0` 运行，并经 trap 执行 write/exit 后恢复 kernel continuation。仍无外部 ELF、process table、thread、scheduler、preemption、wait、signal 或资源回收。 |
...
| Linux x86-64 ABI | 部分实现 | ELF64 `ET_EXEC` CPL3 PID 1 使用 Linux x86-64 的寄存器与 syscall 编号调用 write(1) 和 exit(60)；10 项 parser 测试覆盖 `PT_LOAD`、BSS 与拒绝边界。trap 暂为 `int 0x80`，ELF 仍内嵌；尚无外部 binary、`SYSCALL/SYSRET`、通用 syscall surface、proxy 或 guest agent。 |
...
下一阶段应把 ELF image 移出 kernel，构建独立 Rust userspace binary 并从 boot image/VFS 装载多 `PT_LOAD` page；同时建立 `SYSCALL/SYSRET` 入口、process table 与可恢复的 per-process kernel/user context，再把 VFS fd 变为每进程对象并接入 read/write/open/close。并行的存储边界仍包括通用 grow/truncate、JBD2 多 transaction/revoke/ring wrap 与 virtio descriptor allocator；eBPF 仍需 map、program type、attach point 与 ELF relocation。
