...
| 进程/线程/调度 | 部分实现 | PID 1 以独立 CR3、user code/stack page、CPL3、TSS `RSP0` 运行，并经 trap 执行 write/exit 后恢复 kernel continuation。仍无 process table、ELF、thread、scheduler、preemption、wait、signal 或资源回收。 |
...
| Linux x86-64 ABI | 部分实现 | CPL3 PID 1 使用 Linux x86-64 的寄存器与 syscall 编号调用 write(1) 和 exit(60)。trap 暂为 `int 0x80`，程序为内嵌机器码；尚无 ELF、`SYSCALL/SYSRET`、通用 syscall surface、proxy 或 guest agent。 |
...
下一阶段应把内嵌 PID 1 替换为经严格 `PT_LOAD` 校验的 Linux x86-64 ELF，建立 `SYSCALL/SYSRET` 入口、process table 与可恢复的 per-process kernel/user context，再把 VFS fd 变为每进程对象并接入 read/write/open/close。并行的存储边界仍包括通用 grow/truncate、JBD2 多 transaction/revoke/ring wrap 与 virtio descriptor allocator；eBPF 仍需 map、program type、attach point 与 ELF relocation。
