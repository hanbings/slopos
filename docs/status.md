...
| 图形系统与 Wayland | 部分实现 | framebuffer renderer、niri 式滚动平铺/workspace/bind/rule/column stack/consume/expel/window focus+reorder+explicit/preset height+reset/column explicit+preset+maximize+center+available-width resize+reorder/direct indexed+named move/previous toggle、Waybar JSONC option/format/受限三键/滚轮 action + CSS 顶栏及 workspace 数字点击、swww 风格 daemon state/CLI/PNM/纯色 clear/CPU transition 可用；PID 2 经 40-byte commit 连续发布 CPU/Memory 与 Aurora policy，再经两类 32-byte event 收到 policy/config 实际应用确认，30 项 shell + 5 项 protocol 测试及真实跨 reload service、workspace/规则/stack/缩放/居中/余宽扩展/重排/点击/action/样式/换图/纯色/query/kill/restart 通过。仍无 Wayland wire/object/global/xdg、真实 Waybar hardware provider/POSIX shell action/完整 GTK CSS；常驻 PID 2 是 policy/provider，不是用户态 compositor、独立 bar surface 或 layer-shell client。 |
...
- 最后成功 shell/protocol 单元测试：2026-07-30，`make test-shell`，35 项（shell 30 + desktop protocol 5）。
