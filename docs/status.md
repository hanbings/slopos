...
下一阶段应把已验证的 block/inode allocation、extent growth 与 directory mutation 接入 VFS create/append/truncate，而不是只由启动 probe 驱动；JBD2 还需多 transaction、revoke 与 ring wrap。同时把局部 VFS 提升为内核全局对象，并把两个固定请求槽扩展为通用 descriptor allocator。之后需要可回收 heap、动态 task arena 和首个隔离用户地址空间；eBPF 仍需 map、program type、attach point 与 ELF relocation。
