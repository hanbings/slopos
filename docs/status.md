...
| 图形系统与 Wayland | 部分实现 | framebuffer renderer、niri 式 tiled/floating workspace/keyboard+wheel bind/cooldown/ordered open+focus+width+height+display+focus-ring+border+shadow+background-mode+opacity+maximize+fullscreen rule/normal+tabbed stack/fixed+directional consume/expel/focus+reorder+explicit/preset resize+column/edge maximize+fullscreen+center/expand/direct move、首末列 focus/reorder、完整 workspace reorder，以及语义分离的整列/单窗跨 workspace 转移可用；Waybar JSONC normal/alternate format + format-alt-click + 受限三键/滚轮 action + CSS 顶栏及 workspace 点击、swww daemon state/CLI/PNM/clear/CPU transition 与 8 KiB root VFS P3 path load 可用。44 项 shell + 5 项 protocol 测试及真实 custom `default-column-width`/`default-window-height`/`default-column-display`/`default-floating-position`/window `focus-ring`/`border`/`shadow`/`draw-border-with-background`/`opacity`/`open-focused`/`open-maximized`/`open-maximized-to-edges`/`open-fullscreen` 聚焦、decoration/alpha 像素与三级恢复、Mod+IntelliMouse、workspace identity reorder、full-output restore、strip-boundary、column/window transfer、floating/tabbed/跨 reload/action/样式/异步换图回归通过。仍无可连接 Wayland transport/shared surface buffer、client-driven xdg fullscreen/maximize、真实 Waybar hardware provider/POSIX shell action/完整 GTK CSS；PID 2 是 policy/provider，不是用户态 compositor、独立 bar surface 或 layer-shell client。 |
| 声明式配置 | 部分实现 | niri KDL、Waybar JSONC/CSS 与 swww environment 按 user/system/fallback 顺序从 root VFS 发现；双 static bank 会先验证四份文本，再按 generation 原子发布。交互测试已验证 config generation 1→2 唤醒 PID 2、policy generation 2→3，以及非法 CSS 保留 generation 2且不唤醒服务；custom-config 回归同时注入单页上限内的 4082-byte niri 与 1300-byte Waybar override，验证 ordered `default-column-width`/`default-window-height`/`default-column-display`/`default-floating-position`/window `focus-ring`/`border`/`shadow`/`draw-border-with-background`/`opacity`/`open-focused`/`open-maximized`/`open-maximized-to-edges`/`open-fullscreen` 的实际焦点、几何、显示、decoration/alpha/fullscreen 例外、三级恢复与浮动位置记忆、PID 2 有界分块 hash/连续发布、left/center module placement、clock alternate format 和三键/双向滚轮 action 驱动渲染与壁纸切换。仍无文件 watcher/inotify、普通配置编辑器、结构化 diff，配置发现/parser 也仍属于 kernel service。 |
