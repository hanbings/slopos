...
| VFS 与文件描述符 | 部分实现 | `no_std` path/mount/fd crate 有 5 项宿主测试；QEMU 把 ext4 挂到 `/`，从 `/sbin/slop-init` 取得实际 PID 1 image。PID 1 的独立 fd table 已通过 Linux `openat/read/close` 异步读取 `/etc/slopos/system.conf`；kernel probe fd 另可 seek/原位覆写、EOF 单块 append/truncate，并以读写模式打开刚创建的空文件。mount/backing object 仍是单一 block task 专用，用户 syscall 只覆盖 root/O_RDONLY/单页 bounded read，不是通用 POSIX VFS。 |
| 设备与驱动 | 部分实现 | GOP、COM1、QEMU debugcon、PS/2 键鼠可用；校验 XSDT/MADT，自有 GDT/IDT、xAPIC/IOAPIC、100 Hz PIT；PCI/modern virtio-blk 支持 read/write/flush，clean boot 的 474 个请求由 473 次 INTx 唤醒完成。没有通用 descriptor allocator、MSI-X、其他设备类或 application processor。 |
...
| Linux x86-64 ABI | 部分实现 | root VFS 中的独立 Rust ELF64 `ET_EXEC` CPL3 PID 1 使用 Linux x86-64 register/syscall number，真实执行 `openat/read/close/write/exit`；文件调用保存完整 user frame、经异步 ext4/virtio 完成后 `SYSRETQ` 恢复，read bytes 写入 user stack并由用户态核对。10 项 ELF 与 4 项 process 测试覆盖 parser/lifecycle/fd isolation。尚无任意路径/多 segment exec、跨页通用 copy-user、可写/广泛 syscall surface、proxy 或 guest agent。 |
...
下一阶段把当前 root/O_RDONLY/单页 bounded 的用户 fd bridge 扩展为分块 `copy_from_user`/`copy_to_user`、`write` 与 `lseek`，并让 process exit 统一关闭 backing objects。随后把固定 `/sbin/slop-init`、强制等于 BootInfo 副本的单 `PT_LOAD` 装载器扩展为任意 VFS executable，并补进程初始 stack/`argc`/`argv`/`envp`/auxv；再进入第二个实际进程、可抢占 context switch、wait/资源回收。桌面兼容主线还需动态 workspace、完整 action/rule/output/IPC，Waybar 真实 provider/Pango/action/完整 GTK CSS，以及 swww 任意 VFS image、PNG/JPEG/GIF、多 output 和真正的用户态 IPC/layer-shell。
