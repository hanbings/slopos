...
| ext4/btrfs 文件系统 | 部分实现 | 独立 `ReadOnlyMount`/`ReadOnlyFile` API；校验 metadata CRC32C，异步遍历两个路径并读取、核对 regular file；安全拒绝未知 incompat feature、dirty state、htree 与非法 component。仅限 group 0、depth-0、单块目录/文件；无 global namespace/write/journal，btrfs 未实现。 |
...
- 最后成功 ext4 单元测试：2026-07-30，`make test-ext4`，9 项。
