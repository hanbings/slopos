...
| 图形系统与 Wayland | 部分实现 | framebuffer renderer、niri 式 tiled/floating workspace/bind/rule/normal+tabbed stack/fixed+directional consume/expel/focus+reorder+explicit/preset resize+column/edge maximize+center/expand/direct move，以及语义分离的整列/单窗跨 workspace 转移可用；Waybar JSONC normal/alternate format + format-alt-click + 受限三键/滚轮 action + CSS 顶栏及 workspace 点击、swww daemon state/CLI/PNM/clear/CPU transition 与 8 KiB root VFS P3 path load 可用。37 项 shell + 5 项 protocol 测试及真实 column/window transfer、floating/tabbed/跨 reload/action/样式/异步换图回归通过。仍无 Wayland wire/object/global/xdg、真实 Waybar hardware provider/POSIX shell action/完整 GTK CSS；PID 2 是 policy/provider，不是用户态 compositor、独立 bar surface 或 layer-shell client。 |
...
- 最后成功 shell/protocol 单元测试：2026-07-30，`make test-shell`，42 项（shell 37 + desktop protocol 5）。
