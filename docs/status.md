...
| ext4/btrfs 文件系统 | 部分实现 | 独立可重复 ext4 root image；内核经 virtio 异步读取 1024-byte superblock，校验 magic、geometry、64-bit counts、feature masks 与 metadata CRC32C。尚无 group descriptor/inode/extent/directory/VFS/mount/write/journal；btrfs 未实现。 |
| 设备与驱动 | 部分实现 | GOP、COM1、QEMU debugcon、PS/2 键鼠可用；校验 XSDT/MADT，自有 GDT/IDT、xAPIC/IOAPIC、100 Hz PIT；PCI/modern virtio-blk 对第二块 root disk 完成 feature negotiation、split virtqueue、1024-byte DMA read 和 INTx async completion。仍只有一个启动请求；没有缓存、写入、MSI-X、其他设备类或 application processor。 |
...
- 最后成功 ext4 单元测试：2026-07-30，`make test-ext4`，4 项。
...
下一阶段应把当前单请求 virtio probe 扩展为 descriptor free list 与 bounded multi-request queue，读取 ext4 group descriptor、root inode、extent tree 和 directory entry，形成只读 VFS mount。之后需要可回收 heap、动态 task arena 和首个隔离用户地址空间。eBPF 方向仍需 map、program type、attach point 与 ELF relocation。
