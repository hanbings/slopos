...
- 已有 frame allocator、自有 early page table、bump heap、容量 4 的 process table、每 slot fd table，以及从 root `/sbin/slop-init`、`/sbin/slop-worker` 读取后经各自 Linux initial stack 与 `SYSCALL/SYSRETQ` 运行的 PID 1/2 user address space；文件 syscall 可按 PID 保存/恢复 frame、转为 Blocked 并异步等待 block Future，`sched_yield` 会 round-robin 切换两个独立 CR3。read/write copy 已验证跨两个 virtual stack page 和两个独立 physical frame；`wait4(-1)` 会阻塞 PID 1，PID 2 exit 后完成 status copy/reap/wake，两次 exit 分别释放 descriptor/backing object，且两套 page table/code/stack 共 14 个 frame 被回收并验证复用。当前仍强制 init 的单 `PT_LOAD` ELF 与 BootInfo 引导副本相同，user copy 限于一页 code/两页 stack 和 256 bytes，也没有任意路径 exec、thread、timer preemption、通用 wait selector/options/orphan adoption、广泛 VFS syscall或通用 mapping 回收。
