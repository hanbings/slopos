...
- `initrd.slp` 仍不是文件系统；独立 ext4 disk 已有 kernel-internal mount/file API、fd 原位数据写和 JBD2 单块 record 编码。descriptor/data/commit 已真实写入、flush、读回并清理，但保持 inactive；没有 journal superblock/`needs_recovery` 状态转换、checkpoint/replay、文件增长、allocation/metadata write、通用权限或 btrfs。
