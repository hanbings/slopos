...
- `initrd.slp` 仍不是文件系统；独立 ext4 disk 已有 fd 原位数据写与单块 active JBD2 data transaction/checkpoint。内核尚无启动时 crash replay、多块/并发 transaction、文件增长、allocation/metadata write、通用权限或 btrfs。
