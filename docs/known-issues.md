...
- 已有 frame allocator、自有 early page table、bump heap、容量 4 的 process table、每 slot fd table，以及从 root `/sbin/slop-init` 读取后经 Linux initial stack 与 `SYSCALL/SYSRETQ` 运行的 PID 1 user address space；文件 syscall 可保存/恢复 frame 并异步等待 block Future。read/write copy 已验证跨两个 virtual stack page 和两个独立 physical frame，exit 也会释放遗留 descriptor/backing object。当前仍强制这份单 `PT_LOAD` ELF 与 BootInfo 引导副本相同，user copy 限于一页 code/两页 stack 和 256 bytes，也没有任意路径 exec、第二个实际 process、thread、scheduler、preemption、通用 context switch、广泛 VFS syscall或 frame/page-table 回收。
