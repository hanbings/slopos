...
- `initrd.slp` 仍不是文件系统；独立 ext4 disk 已有 fd 原位数据写、最多八 tag 的 active JBD2 transaction、block allocation/inline extent growth 和启动时 replay。scanner 当前只接受零 feature、单个非 wrap transaction；尚无 revoke、多 transaction、inode/directory allocation、面向 fd 的增长/截断、通用权限或 btrfs。
