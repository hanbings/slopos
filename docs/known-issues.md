...
- niri KDL 与 Waybar JSONC 来自编译时 asset，尚未按 XDG/VFS 路径 live reload。顶部栏 module order/height/spacing 可配置，但 provider 仍固定且没有 module option/CSS/action/per-output；swww daemon、图片 decode 和 transition 尚未实现。
