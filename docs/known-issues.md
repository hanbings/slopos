...
- 已有 frame allocator、自有 early page table、bump heap、容量 4 的 process table、每 slot fd table，以及从 root `/sbin/slop-init` 读取后经 `SYSCALL/SYSRETQ` 运行的同步 PID 1 user address space；当前仍强制这份单 `PT_LOAD` ELF 与 BootInfo 引导副本相同，也没有任意路径 exec、第二个实际 process、thread、scheduler、preemption、context switch、通用 copy-user/VFS syscall 或 frame/page-table 回收。
