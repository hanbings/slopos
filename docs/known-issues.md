...
- `initrd.slp` 仍不是文件系统；独立 ext4 disk 已有 fd 原位写与严格单块 EOF append/truncate、最多八 tag 的 active JBD2 transaction、block/inode allocation、线性目录 create/unlink 和启动时 replay。scanner 当前只接受零 feature、单个非 wrap transaction；create 仍是内核 probe，尚无多块/非连续 fd growth、descriptor namespace mutation、revoke、多 transaction、通用权限或 btrfs。
