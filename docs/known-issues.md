...
- `initrd.slp` 仍不是文件系统；独立 ext4 disk 已有 kernel-internal read-only mount/file API，能跨 group 定位 inode、走 checksummed extent tree、线性单块目录并读取多块 regular file。仅真实验证 depth 1，普通 hole 尚未零填充；仍无 htree/symlink/global VFS namespace/write/journal，btrfs 未实现。
