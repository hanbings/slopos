...
- `initrd.slp` 仍不是文件系统；独立 ext4 disk 已有 kernel-internal read-only mount/file API，能走 group 0、depth-0 inline extent、线性单块目录并读取多块 regular file，但尚无多 group/deep extent/htree/symlink/global VFS namespace/write/journal；btrfs 未实现。
...
- virtio-blk 当前可顺序复用一条 descriptor chain，启动时完成 29 次只读请求并跨三轮 ring wrap；没有多 in-flight descriptor 分配、写入、flush/discard、block cache、timeout 或错误恢复。
