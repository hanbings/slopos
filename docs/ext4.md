...
`scripts/make-rootfs.sh` 固定 4096-byte block、32 个 inode、label、filesystem UUID、所有 superblock copy 的 directory hash seed 和 e2fsprogs fake time，并关闭 lazy inode table/journal 初始化。脚本先把旧 image 截断为零，随后在临时 staging tree 中安装 26200-byte `/sbin/slop-init` 与 25488-byte `/sbin/slop-worker`，并加入 6144-byte `Z` payload、九块 `D` payload、一块 `P` write probe，以及 niri/Waybar/swww 的四份默认配置；large-directory seed 与既有 release 文件 hard-link 到同一 inode，因此仍只使用 31 个 inode并为 create probe 保留 inode 32。`D` 文件打出四个交替 hole，五个初始化 run 会强制产生 depth-1 extent leaf。e2fsprogs 1.47.2 的 punch 路径可能先清 allocation bit、后持久化 extent removal；若首轮还没有形成 depth-1 tree，脚本只重发仍缺失的 punch，直到 tree 出现，再从最终 extent tree 动态解析 leaf physical block并重申 allocation，而不依赖前序文件大小。连续生成逐字节一致，SHA-256 为 `3b59c63281b8300a0523fb3ec26dc4d03a028d52f207d5b1e5d528ae4ec20d52`；`e2fsck -fn` 报告 31/32 files、4243/65536 blocks。生成物位于 `target/`，不提交仓库。
