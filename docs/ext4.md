...
fd append 路径读取五个 home block。读写 fd 3 先 seek 到 EOF 4096，并取得不被旧 size 截断的 append window；sequence 1 transaction 把 superblock free count 61311→61310、group 0 free count 32672→32671，在 block bitmap 33 标记 physical block 99，向该块写入全 `G`，并把 inode 25 的 size/i_blocks/inline extent 扩为 8192/16/2。checkpoint 后 descriptor size 扩为 8192、offset 推进 4096 bytes，再从 offset 4096 经普通 `read_descriptor` 读回全部 `G`。descriptor seek 回 4096 后，sequence 2 transaction journal 原始五块并释放 block 99，descriptor 随之 truncate 为 4096；测试回卷 sequence 后磁盘逐字节恢复。
