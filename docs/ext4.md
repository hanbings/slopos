...
24 项宿主测试覆盖高 32-bit count、动态 inode/descriptor size、bad magic、非法 geometry、truncation、checksum corruption、extent tree、fast symlink、未知 feature、dirty state、htree 拒绝，以及 big-endian JBD2 superblock/transaction。allocation 测试按 [Linux ext4 group descriptor/bitmap 布局](https://www.kernel.org/doc/html/latest/filesystems/ext4/group_descr.html) 切换 bit，更新 bitmap CRC32C、group/superblock free count 与 checksum；inode 测试同步更新 size、`i_blocks`、inline `ee_len` 与 checksum，allocate→free 后均逐字节恢复。多 tag 测试覆盖 `SAME_UUID`、逐 tag escape 与 `LAST_TAG`。
