...
inode 25 是 4096-byte write probe，映射到 physical block 98。内核先通过 cache 验证全 `P` 内容，再由读写 fd 在 offset 123 覆写 73 个 `0xa5` bytes。ext4 层对所在块 read-modify-write，等待 write 与 flush completion，失效 cache 后按 fd 读回前后 `P` 边界；随后以同一路径恢复并再次读回。测试后的 image SHA-256 与生成值一致，`e2fsck -fn` 五阶段通过。最终 cache 为 62 hit/47 miss/2 invalidation；连同两次 write、两次 flush 和未缓存 superblock，共 52 个请求由 51 次 queue interrupt 完成。
