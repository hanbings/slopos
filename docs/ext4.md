...
裸机路径选择第二个 virtio-blk 设备（第一个仍是 ESP）。superblock 报告 65536 blocks、32 inodes、2 groups；group 0/1 inode table 分别是 block 37/38。component walker 最终打开 inode 20（group 1），校验 group 1 descriptor 后成对预取它的两个数据块。8-entry FIFO cache 记录 20 hit/14 miss；superblock 保持独立读取，所以实际设备 request 为 15，而双请求批次让 INTx/queue interrupt 为 14。
