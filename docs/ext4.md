...
`scripts/make-rootfs.sh` 固定 4096-byte block、32 个 inode、label、filesystem UUID、所有 superblock copy 的 directory hash seed 和 e2fsprogs fake time，并关闭 lazy inode table/journal 初始化。脚本先把旧 image 截断为零，随后在临时 staging tree 中安装 26152-byte `/sbin/slop-init`，并加入 6144-byte `Z` payload、九块 `D` payload、一块 `P` write probe，以及 niri/Waybar/swww 的四份默认配置；`D` 文件打出四个交替 hole，五个初始化 run 会强制产生 depth-1 extent leaf。e2fsprogs 1.47.2 的 punch 路径可能先清 allocation bit、后持久化 extent removal；若首轮还没有形成 depth-1 tree，脚本只重发仍缺失的 punch，直到 tree 出现，再从最终 extent tree 动态解析 leaf physical block并重申 allocation，而不依赖前序文件大小。另一个目录通过长名称和 hard link 扩展为两个不连续数据块；`/etc/current-release` 是目标为 `slopos-release` 的 fast symlink。连续生成逐字节一致，SHA-256 为 `66bf45ea8f78ad520434f1e434a8b8636869471701b6d4c0f10cbef351706465`；`e2fsck -fn` 报告 31/32 files、4237/65536 blocks。生成物位于 `target/`，不提交仓库。
