...
`scripts/make-rootfs.sh` 固定 4096-byte block、32 个 inode、label、filesystem UUID、所有 superblock copy 的 directory hash seed 和 e2fsprogs fake time，并关闭 lazy inode table/journal 初始化。脚本先把旧 image 截断为零，随后在临时 staging tree 中安装 26264-byte `/sbin/slop-init` 与 25488-byte `/sbin/slop-worker`，并加入 6144-byte `Z` payload、九块 `D` payload、一块 `P` write probe，以及 niri/Waybar/swww 的四份默认配置；large-directory seed 与既有 release 文件 hard-link 到同一 inode，因此仍只使用 31 个 inode并为 create probe 保留 inode 32。`D` 文件打出四个交替 hole，五个初始化 run 会强制产生 depth-1 extent leaf。e2fsprogs 1.47.2 的 punch 路径可能先清 allocation bit、后持久化 extent removal；若首轮还没有形成 depth-1 tree，脚本只重发仍缺失的 punch，直到 tree 出现，再从最终 extent tree 动态解析 leaf physical block并重申 allocation，而不依赖前序文件大小。连续生成逐字节一致，SHA-256 为 `aca219563969cb6238603a9225361e8242cd3c446c3d3581ab0ce8e5ca53c491`；`e2fsck -fn` 报告 31/32 files、4243/65536 blocks。生成物位于 `target/`，不提交仓库。
...
裸机路径选择第二个 virtio-blk 设备（第一个仍是 ESP）。superblock 报告 65536 blocks、32 inodes、2 groups；group 0/1 inode table 分别是 block 37/38。mount/recovery 后，component walker 从 inode 23 的 blocks 86–92 读取 26264-byte `/sbin/slop-init`，再从 inode 24 的 blocks 93–99 读取 25488-byte `/sbin/slop-worker`。PID 1/2 通过各自的异步 `openat/read/close` 读取 inode 18；PID 1 另通过 `O_RDWR openat`、lseek、异步 write/read 对 inode 31 做跨两页的可逆 64-byte patch。随后 walker 打开 inode 30（group 1）并成对预取它的两个数据块；再打开 inode 28，从 root index 读取 leaf block 104，校验 metadata checksum，由第五个 extent 映射 physical block 111，并把 logical block 7 的 hole 返回为 4096 个零。在 inode 29 的 8192-byte 线性目录中，walker 于 logical block 1 找到 hard-link 到 inode 21 的 `tail-29`。最后读取 inode 14 内的 `slopos-release`，回到同一父目录定位 inode 21。独立 VFS fd probe 再以 chunk/seek 读取 inode 18。
