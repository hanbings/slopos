...
fd append 路径读取五个 home block。读写 fd 3 先 seek 到 EOF 4096，并取得不被旧 size 截断的 append window；sequence 1 transaction 把 superblock free count 61291→61290、group 0 free count 32652→32651，在 block bitmap 33 标记紧邻现有 extent的 physical block 119，向该块写入全 `G`，并把 inode 31 的 size/i_blocks/inline extent 扩为 8192/16/2。checkpoint 后 descriptor size 扩为 8192、offset 推进 4096 bytes，再从 offset 4096 经普通 `read_descriptor` 读回全部 `G`。descriptor seek 回 4096 后，sequence 2 transaction journal 原始五块并释放 block 119，descriptor 随之 truncate 为 4096；测试回卷 sequence 后磁盘逐字节恢复。

VFS create probe 的五个 home 是 superblock 0、group descriptor block 1、group 1 inode bitmap 36、inode table 38 与 directory data 104。第一笔 transaction 把全局/group free inode 从 1→0、`itable_unused` 1→0，分配 inode 32，编码 mode 0644、links 1、size/i_blocks 0/0 与空 depth-0 extent header，并在 inode 27 的 checksum 目录中插入 `create-probe`；正常 `resolve_path/open_file` 随即读回该空文件，descriptor table 复用 fd 3 以 `ReadWrite` 打开，读取返回 EOF 0 bytes。close 后第二笔 transaction 以共享 remover 合并目录 slack、释放 bit并恢复原始 inode slot。timer preemption 与 desktop event wake 令 clean-boot/interaction cache/request 绝对计数随合法 interleaving 变化；现有证据覆盖 157–163 hit、119–128 miss、16–18 invalidation，且设备请求恒为 queue interrupt 加一。固定 image hash 与 fsck 五阶段通过。当前确定性测试镜像仅留一个 free inode，这不是通用容量设计。

`make test-journal-replay` 提供独立两阶段 crash-consistency 回归。第一阶段以 feature kernel 把 blocks 0/1/33/38/119 持久化为“free count -1、bitmap 119 allocated、inode 31 size 8192/extent 2、data 全 G”的旧 home，再置 recovery/start、发布目标为五块原始 free 状态的 descriptor/data/commit，并在 commit flush 后、home checkpoint 前停止 QEMU。宿主直接确认 `needs_recovery`、free blocks 61290、bitmap 119、inode size/blockcount 8192/16 与全 G data。第二阶段使用普通 kernel 与同一 root disk 重启：mount recovery 验证 sequence 1、UUID 与五个 tag，依次重放并读回全部 home，然后清七个 records、推进 sequence 2/start 0，最后清 ext4 recovery bit/checksum。启动从恢复后的两个 VFS ELF 执行 PID 1/2 的 initial-stack validation、cooperative yield、双向 timer preemption、AF_UNIX Wayland configure与两轮 presentation及异步 VFS 读写，再继续配置加载、allocation 与 create probes并进入常驻桌面服务；脚本核对最后的 virtio request 恒比 queue interrupt 多一而不依赖固定调度次序。宿主逐块比较五个 crash home 与注入前快照，确认 free blocks 61291、block 119 free、inode 4096/8、无 recovery feature且 `e2fsck -fn` 通过。脚本最终重建标准 root image并核对固定 SHA-256。
