...
`scripts/make-rootfs.sh` 固定 4096-byte block、32 个 inode、label、filesystem UUID、所有 superblock copy 的 directory hash seed 和 e2fsprogs fake time，并关闭 lazy inode table/journal 初始化。脚本在临时 staging tree 中安装 4848-byte `/sbin/slop-init`，并加入 6144-byte `Z` payload、九块 `D` payload、一块 `P` write probe，以及 niri/Waybar/swww 的四份默认配置；`D` 文件打出四个交替 hole，五个初始化 run 会强制产生 depth-1 extent leaf。另一个目录通过长名称和 hard link 扩展为两个不连续数据块；`/etc/current-release` 是目标为 `slopos-release` 的 fast symlink。最后归一化全部 inode metadata。当前 e2fsprogs 1.47.2 下两次生成逐字节一致，SHA-256 为 `738c3d0b09a70540fc529eb48b55511264ac99544d92e48637ca1f5908767737`；`e2fsck -fn` 报告 31/32 files、4232/65536 blocks。生成物位于 `target/`，不提交仓库。
...
裸机路径选择第二个 virtio-blk 设备（第一个仍是 ESP）。superblock 报告 65536 blocks、32 inodes、2 groups；group 0/1 inode table 分别是 block 37/38。mount/recovery 后，component walker 先从 inode 23 跨两个逻辑块读取 4848-byte `/sbin/slop-init`。随后打开 inode 30（group 1）并成对预取它的两个数据块；再打开 inode 27，从 root index 读取 leaf block 92，校验 metadata checksum，由第五个 extent 映射 physical block 99，并把 logical block 7 的 hole 返回为 4096 个零。在 inode 28 的 8192-byte 线性目录中，walker 于 logical block 1 找到指向 inode 29 的 `tail-29`。最后读取 inode 14 内的 `slopos-release`，回到同一父目录定位 inode 21。VFS fd 路径再次以 chunk/seek 读取 inode 18。

inode 31 是 4096-byte write probe，映射到 physical block 105。内核先通过 cache 验证全 `P` 内容，再由读写 fd 在 offset 123 覆写 73 个 `0xa5` bytes。ext4 层对所在块 read-modify-write，等待 write 与 flush completion，失效 cache 后按 fd 读回前后 `P` 边界；随后以同一路径恢复并再次读回。测试后的 image SHA-256 与生成值一致，`e2fsck -fn` 五阶段通过。
...
fd append 路径读取五个 home block。读写 fd 3 先 seek 到 EOF 4096，并取得不被旧 size 截断的 append window；sequence 1 transaction 把 superblock free count 61304→61303、group 0 free count 32665→32664，在 block bitmap 33 标记 physical block 106，向该块写入全 `G`，并把 inode 31 的 size/i_blocks/inline extent 扩为 8192/16/2。checkpoint 后 descriptor size 扩为 8192、offset 推进 4096 bytes，再从 offset 4096 经普通 `read_descriptor` 读回全部 `G`。descriptor seek 回 4096 后，sequence 2 transaction journal 原始五块并释放 block 106，descriptor 随之 truncate 为 4096；测试回卷 sequence 后磁盘逐字节恢复。

VFS create probe 的五个 home 是 superblock 0、group descriptor block 1、group 1 inode bitmap 36、inode table 38 与 directory data 90。第一笔 transaction 把全局/group free inode 从 1→0、`itable_unused` 1→0，分配 inode 32，编码 mode 0644、links 1、size/i_blocks 0/0 与空 depth-0 extent header，并在 inode 26 的 checksum 目录中插入 `create-probe`；正常 `resolve_path/open_file` 随即读回该空文件，descriptor table 复用 fd 3 以 `ReadWrite` 打开，读取返回 EOF 0 bytes。close 后第二笔 transaction 以共享 remover 合并目录 slack、释放 bit 并恢复原始 inode slot。整个 clean boot 总计 463 requests/462 queue interrupts，cache 为 134 hit/85 miss/16 invalidation；固定 image hash 与 fsck 五阶段通过。当前确定性测试镜像仅留一个 free inode，这不是通用容量设计。

`make test-journal-replay` 提供独立两阶段 crash-consistency 回归。第一阶段以 feature kernel 把 blocks 0/1/33/38/106 持久化为“free count -1、bitmap 106 allocated、inode 31 size 8192/extent 2、data 全 G”的旧 home，再置 recovery/start、发布目标为五块原始 free 状态的 descriptor/data/commit，并在 commit flush 后、home checkpoint 前停止 QEMU。宿主直接确认 `needs_recovery`、free blocks 61303、bitmap 106、inode size/blockcount 8192/16 与全 G data。第二阶段使用普通 kernel 与同一 root disk 重启：mount recovery 验证 sequence 1、UUID 与五个 tag，依次重放并读回全部 home，然后清七个 records、推进 sequence 2/start 0，最后清 ext4 recovery bit/checksum。启动从恢复后的 `/sbin/slop-init` 执行 PID 1，再继续配置加载、allocation 与 create probes并进入桌面，总计 498 requests/497 queue interrupts；宿主逐块比较五个 crash home 与注入前快照，确认 free blocks 61304、block 106 free、inode 4096/8、无 recovery feature且 `e2fsck -fn` 通过。脚本最终重建标准 root image 并核对固定 SHA-256。
