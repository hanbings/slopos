...
`scripts/make-rootfs.sh` 固定 4096-byte block、32 个 inode、label、filesystem UUID、所有 superblock copy 的 directory hash seed 和 e2fsprogs fake time，并关闭 lazy inode table/journal 初始化。脚本先把旧 image 截断为零，随后在临时 staging tree 中安装 26344-byte `/sbin/slop-init` 与 38176-byte `/sbin/slop-shell`，并从仓库内的确定性 gzip+base64 资产还原 6144-byte `vfs-wallpaper.png`（12×8 Adam7 16-bit RGB，长 `tEXt` ancillary padding、两个 IDAT、动态 Huffman 与 pass-row filter 0–4）、九块 `D` payload、一块 `P` write probe，以及 niri/Waybar/swww 的四份默认配置；large-directory seed 与既有 release 文件 hard-link 到同一 inode，因此仍只使用 31 个 inode并为 create probe 保留 inode 32。`D` 文件打出四个交替 hole，五个初始化 run 会强制产生 depth-1 extent leaf。e2fsprogs 1.47.2 的 punch 路径可能先清 allocation bit、后持久化 extent removal；若首轮还没有形成 depth-1 tree，脚本只重发仍缺失的 punch，直到 tree 出现，再从最终 extent tree 动态解析 leaf physical block并重申 allocation，而不依赖前序文件大小。连续生成逐字节一致，SHA-256 为 `5d5e7562b424c0cb66616ec5b8fb3089815b7b4a9f3e81cf83bca7226ed2e2c7`；`e2fsck -fn` 报告 31/32 files、4246/65536 blocks。生成物位于 `target/`，不提交仓库。
...
裸机路径选择第二个 virtio-blk 设备（第一个仍是 ESP）。superblock 报告 65536 blocks、32 inodes、2 groups；group 0/1 inode table 分别是 block 37/38。mount/recovery 后，component walker 从 inode 23 的 blocks 86–92 读取 26344-byte `/sbin/slop-init`，再从 inode 24 的 blocks 93–102 读取 38176-byte `/sbin/slop-shell`。PID 1 通过异步 `openat/read/close` 读取 inode 18；PID 2 通过相同路径分块读取 inode 20 的 Waybar 配置与 inode 17 的 swww environment，在 4096/512-byte 上限内增量计算 hash并验证非空 EOF；默认文件当前为 904/244 bytes，每个有效 config generation 都会触发新一轮读取。PID 1 另通过 `O_RDWR openat`、lseek、异步 write/read 对 inode 31 做跨两页的可逆 64-byte patch。随后 walker 打开 inode 30（group 1）的 `/usr/share/slopos/vfs-wallpaper.png` 并成对预取它的两个数据块；交互路径还会按同一路径再次异步读齐，校验 PNG signature/chunk CRC/zlib Adler，解开动态 DEFLATE 与 Adam7 pass-row filter，将 16-bit sample 缩放后 scatter 为精确 12×8 RGB8 raster。再打开 inode 28，从 root index 读取 leaf block 107，校验 metadata checksum，由第五个 extent 映射 physical block 114，并把 logical block 7 的 hole 返回为 4096 个零。在 inode 29 的 8192-byte 线性目录中，walker 于 logical block 1 找到 hard-link 到 inode 21 的 `tail-29`。最后读取 inode 14 内的 `slopos-release`，回到同一父目录定位 inode 21。独立 VFS fd probe 再以 chunk/seek 读取 inode 18。

inode 31 是 4096-byte write probe，映射到 physical block 119。PID 1 先以读写 fd 在 offset 123 异步覆写/验证/恢复 64 bytes；buffer 横跨两个 user stack page，但仍落在同一 file block。block task 随后通过 cache 验证全 `P` 内容，再用独立读写 fd 覆写 73 个 `0xa5` bytes。两条路径都由 ext4 对所在块 read-modify-write，等待 write 与 flush completion，失效 cache 后按 fd 读回，最后恢复原内容。测试后的 image SHA-256 与生成值一致，`e2fsck -fn` 五阶段通过。
...
fd append 路径读取五个 home block。读写 fd 3 先 seek 到 EOF 4096，并取得不被旧 size 截断的 append window；sequence 1 transaction 把 superblock free count 61290→61289、group 0 free count 32651→32650，在 block bitmap 33 标记紧邻现有 extent的 physical block 120，向该块写入全 `G`，并把 inode 31 的 size/i_blocks/inline extent 扩为 8192/16/2。checkpoint 后 descriptor size 扩为 8192、offset 推进 4096 bytes，再从 offset 4096 经普通 `read_descriptor` 读回全部 `G`。descriptor seek 回 4096 后，sequence 2 transaction journal 原始五块并释放 block 120，descriptor 随之 truncate 为 4096；测试回卷 sequence 后磁盘逐字节恢复。

VFS create probe 的五个 home 是 superblock 0、group descriptor block 1、group 1 inode bitmap 36、inode table 38 与 directory data 105。第一笔 transaction 把全局/group free inode 从 1→0、`itable_unused` 1→0，分配 inode 32，编码 mode 0644、links 1、size/i_blocks 0/0 与空 depth-0 extent header，并在 inode 27 的 checksum 目录中插入 `create-probe`；正常 `resolve_path/open_file` 随即读回该空文件，descriptor table 复用 fd 3 以 `ReadWrite` 打开，读取返回 EOF 0 bytes。close 后第二笔 transaction 以共享 remover 合并目录 slack、释放 bit并恢复原始 inode slot。timer preemption 与 desktop event wake 令 clean-boot/interaction cache/request 绝对计数随合法 interleaving 变化；现有证据覆盖 157–163 hit、119–128 miss、16–18 invalidation，且设备请求恒为 queue interrupt 加一。固定 image hash 与 fsck 五阶段通过。当前确定性测试镜像仅留一个 free inode，这不是通用容量设计。

`make test-journal-replay` 提供独立两阶段 crash-consistency 回归。第一阶段以 feature kernel 把 blocks 0/1/33/38/120 持久化为“free count -1、bitmap 120 allocated、inode 31 size 8192/extent 2、data 全 G”的旧 home，再置 recovery/start、发布目标为五块原始 free 状态的 descriptor/data/commit，并在 commit flush 后、home checkpoint 前停止 QEMU。宿主直接确认 `needs_recovery`、free blocks 61289、bitmap 120、inode size/blockcount 8192/16 与全 G data。第二阶段使用普通 kernel 与同一 root disk 重启：mount recovery 验证 sequence 1、UUID 与五个 tag，依次重放并读回全部 home，然后清七个 records、推进 sequence 2/start 0，最后清 ext4 recovery bit/checksum。启动从恢复后的两个 VFS ELF 执行 PID 1/2 的 initial-stack validation、cooperative yield、双向 timer preemption、AF_UNIX Wayland device discovery/configure与两轮 presentation及异步 VFS 读写，再继续配置加载、allocation 与 create probes并进入常驻桌面服务；脚本核对最后的 virtio request 恒比 queue interrupt 多一而不依赖固定调度次序。宿主逐块比较五个 crash home 与注入前快照，确认 free blocks 61290、block 120 free、inode 4096/8、无 recovery feature且 `e2fsck -fn` 通过。脚本最终重建标准 root image并核对固定 SHA-256。
