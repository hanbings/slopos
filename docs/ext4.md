...
fd append 路径读取五个 home block。读写 fd 3 先 seek 到 EOF 4096，并取得不被旧 size 截断的 append window；sequence 1 transaction 把 superblock free count 61290→61289、group 0 free count 32651→32650，在 block bitmap 33 标记紧邻现有 extent 的 physical block 120，向该块写入全 `G`，并把 inode 31 的 size/i_blocks/inline extent 扩为 8192/16/2。checkpoint 后 descriptor size 扩为 8192、offset 推进 4096 bytes，再从 offset 4096 经普通 `read_descriptor` 读回全部 `G`。descriptor seek 回 4096 后，sequence 2 transaction journal 原始五块并释放 block 120，descriptor 随之 truncate 为 4096；测试回卷 sequence 后磁盘逐字节恢复。
...
`make test-journal-replay` 提供独立两阶段 crash-consistency 回归。第一阶段以 feature kernel 把 blocks 0/1/33/38/120 持久化为“free count -1、bitmap 120 allocated、inode 31 size 8192/extent 2、data 全 G”的旧 home，再置 recovery/start、发布目标为五块原始 free 状态的 descriptor/data/commit，并在 commit flush 后、home checkpoint 前停止 QEMU。宿主直接确认 `needs_recovery`、free blocks 61289、bitmap 120、inode size/blockcount 8192/16 与全 G data。第二阶段使用普通 kernel 与同一 root disk 重启：mount recovery 验证 sequence 1、UUID 与五个 tag，依次重放并读回全部 home，然后清七个 records、推进 sequence 2/start 0，最后清 ext4 recovery bit/checksum。启动从恢复后的两个 VFS ELF 执行 PID 1/2 的 initial-stack validation、cooperative yield、双向 timer preemption、三阶段 Wayland configure/presentation 与异步 VFS 读写，再继续配置加载、allocation 与 create probes并进入常驻桌面服务；脚本核对最后的 virtio request 恒比 queue interrupt 多一而不依赖固定调度次序。宿主逐块比较五个 crash home 与注入前快照，确认 free blocks 61290、block 120 free、inode 4096/8、无 recovery feature且 `e2fsck -fn` 通过。脚本最终重建标准 root image并核对固定 SHA-256。
