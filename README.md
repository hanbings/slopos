...
桌面现在已有第一条双向用户态服务边界，但尚未整体迁出内核。`/sbin/slop-shell` 是 lifecycle-aware 的一次性 policy/provider 进程：它验证 root VFS 的 Waybar/swww 默认配置、发布 generation 1，并阻塞到 kernel desktop 回送 `policy-applied` 事件后才退出；kernel 仍持有 niri 状态机、输入、swww daemon state、GOP renderer 与窗口 surface。运行时 reload 也仍由 kernel block/desktop task 完成。系统内还没有普通配置编辑器或文件变更 watcher，Waybar 没有完整 GTK CSS、Pango、action 或硬件 backend；swww 还不是长期驻留 Unix-socket/Wayland layer-shell daemon，图片来源暂限两个编译时 PNM asset。完整兼容边界见 [docs/desktop-shell.md](docs/desktop-shell.md)，保守完成度见 [docs/status.md](docs/status.md)。
