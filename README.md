...
- 纵向 workspace 切换、named workspace、KDL `binds` 与顺序叠加的 `window-rule`；规则可分别覆盖 `open-on-workspace`、`open-floating true|false` 与一次性的 `open-maximized true|false` 最大化列初态；
...
- 非默认但可解析的 niri KDL 与 Waybar JSONC 能从临时 ext4 副本注入；实机回归确认 Config 按 `open-maximized true` 以保留 Waybar/gap 的 992 px 列打开，Waybar override 仍跨 config generation 重新发布；用户服务拒绝空文件和超过 4096/512-byte 上限的输入；
- 两张可在运行时切换的嵌入式 P3/PNM 壁纸；
- niri KDL、Waybar JSONC/CSS 与 swww 配置/状态机子集（含全局 Mod+IntelliMouse 滚轮绑定与初始最大化规则），加桌面提交/事件协议，共 46 项宿主测试；
