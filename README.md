...
QEMU 另挂载一个可重复生成的 128 MiB ext4 root disk。内核经第二个 virtio-blk 连续完成 29 次 DMA/INTx/Future 读取，校验 metadata CRC32C；独立 `ReadOnlyMount`/file API 的异步 component walker 实际核对两个配置文件，并通过 inline extent run 读取 6144-byte 多块文件。当前还没有全局 VFS namespace、mount table 或 fd。
