...
PID 1 由 `userspace/init` 独立构建并写入 root image 的 `/sbin/slop-init`；kernel 经 ext4 path walker 跨六个文件块读出 22928 bytes，并要求它与 BootInfo v2 保留的 ESP 引导副本逐字节相同后才执行。它使用独立 user code/stack page、GDT user segments 与 supervisor kernel map；`no_std` ELF crate 严格校验 ELF64/x86-64/`ET_EXEC` 和 `PT_LOAD`，按 `p_filesz` 复制 executable 并从 ELF entry 进入。容量 4 的固定 process table 保存 image/context、`Ready → Running → Exited`、exit status、syscall count 与容量 8 的每进程 fd 表。程序按 Linux x86-64 寄存器和编号约定发出真实 `SYSCALL`：`openat(AT_FDCWD, "/etc/slopos/system.conf", O_RDONLY)` 取得进程 fd 3，`read` 把 76-byte ext4 内容复制到用户 stack，`close` 释放 descriptor，随后 `write(1, ..., 18)` 与 `exit(0)` 完成。文件 syscall 会保存完整用户 frame、恢复 kernel CR3 并让现有 block Future 等待 virtio IRQ，完成后再恢复进程 CR3/寄存器并 `SYSRETQ`；没有在 handler 内 busy-wait。当前仍无 scheduler、通用可写 VFS syscall、跨页 user copy 或第二个实际进程。

内核还会在启动时通过一个独立的 eBPF verifier 执行内建测试程序；当前只是无动态分配、前向控制流的安全子集，并不声称兼容 Linux eBPF。

QEMU 另挂载一个可重复生成的 256 MiB、双 block-group ext4 root disk。异步 mount/file API 核对复杂读取路径；读写 fd 3 除原位覆写外，还能从 EOF 4096 追加一整块：五 tag transaction 分配 block 110、把 inode 31 extent 增长到 8192，descriptor size/offset 同步推进，新增数据再经 fd 读回，最后 truncate/释放恢复。另一组 transaction 分配 inode 32、插入空文件 `/usr/share/slopos/create-probe`，以读写 fd 3 打开并验证 EOF，再 close/unlink。标准 clean boot 的 8-entry cache 记录 136 hit/96 miss/16 invalidation，共 474 个设备请求、473 次队列中断。两阶段 crash-injection 还会停在 allocation commit 后/home 前，再由普通 kernel 于下次 mount 重放、清理并继续进入桌面。当前 replay 支持最多八个 tag 的零-feature、连续且非 wrap transaction；这些 create/growth 操作仍是启动回归路径，没有通用可写 namespace 或 syscall。
