...
- Waybar JSONC 驱动的 left/center/right 顶部 module 栏、height/spacing、CSS shorthand 或逐边有符号 surface margin、`fixed-center`/`exclusive` 几何、module format/alternate-format/interval/length、可配置 alternate-format click、左/右/中键与上下滚轮 action option，以及可点击的 `niri/workspaces` 数字；
...
- 非默认但可解析的 niri KDL 与 Waybar JSONC 能从临时 ext4 副本注入；实机回归确认 `open-focused true` 会让规则放到后台 workspace 的 Config 主动获得焦点并以覆盖 Waybar 的 1024×768 fullscreen 打开，退出后依次恢复保留 Waybar 的 edge/column/app-rule 初始尺寸；`focus-ring { off; }`、4 px active border 与带 RGBA、offset/spread/softness 的 window shadow 均按规则合成，`draw-border-with-background false` 让 `opacity 0.75` 的 surface 透出不同位置的壁纸而不是边框填充，fullscreen 则强制不透明并省略全部 decoration；Waybar `margin: "4 12"` 把 surface 精确限制到 `(12,4)–(1012,44)` 并将 exclusive reserve 改为 44 px，`fixed-center false` 又让 center block 在左右 block 之间动态居中且保持真实 workspace 点击。脚本以九个确定 framebuffer 像素验证这些路径；`default-floating-position x=24 y=24 relative-to="bottom-right"` 在移入浮动层时产生精确右下间距，移动后的浮动位置经过 tiling 往返仍会恢复。切回 main 后旧 Terminal 焦点也保持不变；
