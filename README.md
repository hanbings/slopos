...
- Tab/`Mod+方向键`、`Mod+数字` 与 `Mod+Alt+C/M` 切换焦点/workspace，`Mod+K/J` 在同列上下聚焦，`Mod+Ctrl+K/J` 在同列上下重排 focused window，`Mod+Comma/Period` 合并/拆出底窗，`Mod+Shift+Minus/Equal` 调整 focused window 高度、`Mod+Ctrl+Shift+R` 循环 preset 窗高、`Mod+Ctrl+R` 重置等高，`Mod+R`/`Mod+Shift+R` 正反循环 preset 列宽，`Mod+F` 最大化/恢复列，`Mod+C` 居中当前列，`Mod+Tab` 往返上一 workspace，`Mod+Shift+左右` 重排列、`Mod+Shift+上下`、`Mod+Ctrl+数字` 或 `Mod+Ctrl+Alt+C/M` 跨 workspace 移列、`Mod+Minus/Equal` 缩放列、`Mod+Q` 关闭窗口；
...
- niri KDL、Waybar JSONC/CSS 与 swww 配置/状态机子集，加桌面提交/事件协议，共 34 项宿主测试；
