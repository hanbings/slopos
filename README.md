...
- 纵向 workspace 切换、named workspace、KDL `binds` 与顺序叠加的 `window-rule`；规则可分别覆盖 `open-on-workspace`、`open-floating true|false`、`open-focused true|false`、tiled/floating 共用的 `default-column-width`/`default-window-height { fixed|proportion; }`、动态 `default-column-display "normal"|"tabbed"`，以及一次性的 `open-maximized`、`open-maximized-to-edges` 与 `open-fullscreen` true/false 初态；
...
- 非默认但可解析的 niri KDL 与 Waybar JSONC 能从临时 ext4 副本注入；实机回归确认 `open-focused true` 会让规则放到后台 workspace 的 Config 主动获得焦点并以覆盖 Waybar 的 1024×768 fullscreen 打开，退出后依次恢复保留 Waybar 的 1024×728 edge maximize、992×340 column maximize 与 app rule 的 66.7%/656×340 初始尺寸；切回 main 后旧 Terminal 焦点仍保留。Waybar override 仍跨 config generation 重新发布，用户服务拒绝空文件和超过 4096/512-byte 上限的输入；
- 两张可在运行时切换的嵌入式 P3/PNM 壁纸；
- niri KDL、Waybar JSONC/CSS 与 swww 配置/状态机子集（含全局 Mod+IntelliMouse 滚轮绑定、app-specific 初始宽高/焦点、两类最大化与全屏规则），加桌面提交/事件协议，共 48 项宿主测试；
