...
- 纵向 workspace 切换、named workspace、KDL `binds` 与顺序叠加的 `window-rule`；规则可分别覆盖 `open-on-workspace`、`open-floating true|false`、`default-column-width { fixed|proportion; }` 与一次性的 `open-maximized true|false` 最大化列初态；
...
- niri KDL、Waybar JSONC/CSS 与 swww 配置/状态机子集（含全局 Mod+IntelliMouse 滚轮绑定、app-specific 初始列宽与初始最大化规则），加桌面提交/事件协议，共 46 项宿主测试；
