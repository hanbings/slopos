...
QEMU 另挂载一个可重复生成的 256 MiB、双 block-group ext4 root disk。内核经第二个 virtio-blk 连续完成 33 次 DMA/INTx/Future 读取，校验 group 0/1 metadata CRC32C；异步 mount/file API 实际核对两个配置文件，并读取位于 inode group 1 的 6144-byte 多块文件。当前还没有全局 VFS namespace、mount table 或 fd。
