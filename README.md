...
QEMU 另挂载一个可重复生成的 256 MiB、双 block-group ext4 root disk。异步 mount/file API 核对两个配置文件，读取位于 inode group 1 的 6144-byte 多块文件，并沿 checksummed depth-1 extent leaf 读取另一个稀疏文件的逻辑块 8；8-entry read cache 记录 26 hit/23 miss，24 个设备 DMA 请求中有一个双请求批次。当前还没有全局 VFS namespace、mount table 或 fd。
