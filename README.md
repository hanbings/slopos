...
PID 1 完成 18 次 syscall 与跨两页可逆 write/read，显式关闭最后一个 fd 后以 `wait4(-1)` 常驻为 supervisor；PID 2 用 256-byte buffer 流式读取 Waybar JSONC 与 swww environment，确认 EOF并增量计算 FNV-1a，再经 `slopos-desktop-v1` ABI 发布 provider 值与 Aurora 初始壁纸策略。首代 policy 应用后，PID 2 以 Linux `socket/connect` 把 fd 3 连到 `/run/slopos/wayland-0`；308-byte initial Wayland batch 依 `SOCK_STREAM` 语义拆成 256+52 bytes。PID 2 绑定 compositor/shm/xdg/seat/output，并创建 `wl_pointer` object 14 与 `wl_keyboard` object 15。服务端返回 pointer+keyboard capability、seat/output 信息和两层 xdg configure；288-byte configure event 的前 256 bytes 通过 Linux `recvmsg` 交付，并用 `SCM_RIGHTS` 附带只读 keymap fd，余下 32 bytes 由普通 `read` 取完。客户端读取完整 3307-byte NUL-terminated XKB v1 keymap，校验长度、FNV-1a、末尾 NUL 与 EOF 后关闭 fd。随后 PID 2 以 `memfd_create("slopos-wayland-shm", 0)` 复用 fd 4、`ftruncate` 为 3072 bytes，并把同一物理页 `mmap(MAP_SHARED)` 到 `0x40007000`；首个 configured batch 再用 `sendmsg(SCM_RIGHTS)` 反向传递 surface backing，第二帧只重写共享页并复用 pool/buffer。最终 framebuffer 四象限颜色来自共享页的第二帧。初始呈现只发布 release/callback/delete-id；真正的 `wl_keyboard.enter/leave/key/modifiers` 与 `wl_pointer.enter/leave/motion/button/axis/frame` 由 compositor 的实时焦点和命中测试驱动。初始配置 generation 1 到达后，PID 2 以 fd 4 打开只读 `/run/slopos/desktop-events`，再用 Linux `poll` 永久同时等待 fd 3 的 Wayland stream 与 fd 4 的配置 generation；用户态保留跨 read 的 frame buffer，可解析拆分或合并的消息。配置 bank 继续独立执行 UTF-8、JSONC/CSS/KDL/environment parse-before-swap；有效 `RELOAD` 的 config generation 2 已通过同一 poll loop 触发 policy generation 2，非法 reload 仍不发布下一代。私有 `0x534c0005` 像素 staging ABI 已移除；当前仍缺用户态 `bind/listen/accept`、通用 mmap/munmap 与多 client Wayland ownership。framebuffer、PS/2 采集、niri 状态机与最终 GOP 合成目前仍属于 kernel mechanism。
...
- 同一 PID 2 以 Linux `socket/connect/read/write/sendmsg/recvmsg/poll` 连接 `/run/slopos/wayland-0`，通过标准 Wayland core/xdg-shell 请求创建单个 `wl_shm` XRGB8888 `xdg_toplevel`，并绑定 seat/output、创建 pointer/keyboard；它解析 registry、seat capability/name、output geometry/mode/scale/name/description/done、keyboard keymap/repeat、configure 与 presentation 服务端事件并发送正确的 `ack_configure`。server 先经 `recvmsg(SCM_RIGHTS)` 交付只读 XKB keymap fd，client 校验后关闭；`memfd_create/ftruncate/mmap(MAP_SHARED)` 再建立 surface backing，首个 pool 经 `sendmsg(SCM_RIGHTS)` 反向传递 fd，首次 release 后只更新同一共享页并复用 buffer/callback ID 完成第二次 commit/presentation。此后客户端永久 poll Wayland socket 与配置事件 descriptor，并以持续 frame decoder 消费动态 pointer enter/leave/motion/button/axis/frame、keyboard enter/leave/key/modifiers；kernel server 严格分派对象生命周期并把第二帧用户像素合成进 System 窗口；
...
- niri KDL、Waybar JSONC/CSS 与 swww 配置/状态机子集（含真实 `cooldown-ms` 限流的全局 Mod+IntelliMouse 滚轮绑定、app-specific 初始宽高/焦点/浮动锚点、两类最大化与全屏规则），加桌面提交/事件协议、Wayland wire/server 与 local-stream core，共 86 项相关宿主测试（56 + 11 + 13 + 6）；
