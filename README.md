...
- Tab/`Mod+方向键`、`Mod+数字` 与 `Mod+C/M` 切换焦点/workspace，`Mod+K/J` 在同列上下聚焦，`Mod+Ctrl+K/J` 在同列上下重排 focused window，`Mod+Comma/Period` 合并/拆出底窗，`Mod+Shift+Minus/Equal` 调整 focused window 高度，`Mod+Tab` 往返上一 workspace，`Mod+Shift+左右` 重排列、`Mod+Shift+上下`、`Mod+Ctrl+数字` 或 `Mod+Ctrl+C/M` 跨 workspace 移列、`Mod+Minus/Equal` 缩放列、`Mod+Q` 关闭窗口；
