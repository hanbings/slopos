...
PID 1 由 `userspace/init` 独立构建并写入 root image 的 `/sbin/slop-init`；kernel 经 ext4 path walker 跨七个文件块读出 25896 bytes，并要求它与 BootInfo v2 保留的 ESP 引导副本逐字节相同后才执行。它使用独立 user code/stack page、GDT user segments 与 supervisor kernel map；`no_std` ELF crate 严格校验 ELF64/x86-64/`ET_EXEC` 和 `PT_LOAD`，按 `p_filesz` 复制 executable 并从 ELF entry 进入。kernel 还在用户 stack 构造 16-byte aligned Linux `argc=2`、`argv`、3 项 `envp` 与 9 对 auxv，PID 1 在第一条 Rust 指令前实际解析并核对这些内容。容量 4 的固定 process table 保存 image/context、`Ready → Running → Exited`、exit status、syscall count 与容量 8 的每进程 fd 表。程序先以 `O_RDONLY openat/read/close` 核对 76-byte 配置，再以 `O_RDWR` 打开 inode 31，使用 `lseek` 和异步 `write/read` 覆写、验证并恢复 16 bytes，最后执行 stdout `write` 与 `exit(0)`，共 15 次 Linux x86-64 syscall。文件 I/O syscall 会保存完整用户 frame、恢复 kernel CR3 并让现有 block Future 等待 virtio IRQ，完成后再恢复进程 CR3/寄存器并 `SYSRETQ`；没有在 handler 内 busy-wait。当前仍无 scheduler、跨页 user copy、通用文件增长/metadata syscall 或第二个实际进程。
