...
root image 现在包含两个独立 Rust `no_std` ELF：inode 23 的 `/sbin/slop-init`（26344 bytes）和 inode 24 的 `/sbin/slop-shell`（40960 bytes）。kernel 经 ext4 path walker 分别跨七个和十个文件块读入；PID 1 image 仍须与 BootInfo v2 保留的 ESP 引导副本逐字节相同，desktop service 则只来自 root VFS。两者各有独立 CR3、四页只读 user code、三页 user stack、Linux `argc/argv/envp/auxv`、保存的 syscall/interrupt frame、pending request 和容量 8 的 fd 表。容量 4 的 process table 实现 `Ready/Running/Blocked/Runnable/Exited`；除 `sched_yield` cooperative round-robin 外，100 Hz PIT 现在会在 CPL3 保存全部 15 个 GPR、RIP/RFLAGS/RSP 后抢占当前进程，并由 block-task continuation 选择另一个 Ready/Runnable CR3，再以 `IRETQ` 恢复异步中断上下文，避免把 RCX/R11 当成 syscall scratch。QEMU 已实测 PID 1→2 与 PID 2→1 的非合作式切换，两个用户程序各用约 100,000,000 TSC tick 的无 syscall 窗口证明至少被抢占一次，并在其余交错执行中各自拥有数字相同但 ownership 独立的 fd 3。

PID 1 完成 18 次 syscall 与跨两页可逆 write/read，显式关闭最后一个 fd 后以 `wait4(-1)` 常驻为 supervisor；PID 2 用 256-byte buffer 流式读取 Waybar JSONC 与 swww environment，确认 EOF并增量计算 FNV-1a，再经 `slopos-desktop-v1` ABI 发布 provider 值与 Aurora 初始壁纸策略。首代 policy 应用后，PID 2 以 Linux `socket/bind/listen` 在 fd 3 建立 `/run/slopos/wayland-0` listener，再以 fd 4 `socket/connect` 并 `accept` 出 fd 5；当前受信 bootstrap 仍由 kernel 在 accepted endpoint 上处理 Wayland protocol。308-byte initial Wayland batch 依 `SOCK_STREAM` 语义拆成 256+52 bytes。PID 2 绑定 compositor/shm/xdg/seat/output，并创建 `wl_pointer` object 14 与 `wl_keyboard` object 15。服务端返回 pointer+keyboard capability、seat/output 信息和两层 xdg configure；288-byte configure event 的前 256 bytes 通过 Linux `recvmsg` 交付，并用 `SCM_RIGHTS` 附带只读 keymap fd，余下 32 bytes 由普通 `read` 取完。客户端读取完整 3307-byte NUL-terminated XKB v1 keymap，校验长度、FNV-1a、末尾 NUL 与 EOF 后关闭 fd。随后 PID 2 以 `memfd_create("slopos-wayland-shm", 0)` 复用 fd 6、`ftruncate` 为 3072 bytes，并把同一物理页 `mmap(MAP_SHARED)` 到 `0x40007000`；首个 configured batch 再用 `sendmsg(SCM_RIGHTS)` 反向传递 surface backing，第二帧只重写共享页并复用 pool/buffer。最终 framebuffer 四象限颜色来自共享页的第二帧。初始呈现只发布 release/callback/delete-id；真正的 `wl_keyboard.enter/leave/key/modifiers` 与 `wl_pointer.enter/leave/motion/button/axis/frame` 由 compositor 的实时焦点和命中测试驱动。初始配置 generation 1 到达后，PID 2 以 fd 6 打开只读 `/run/slopos/desktop-events`，再用 Linux `poll` 永久同时等待 fd 4 的 Wayland stream 与 fd 6 的配置 generation；配置热重读文件使用 fd 7。用户态保留跨 read 的 frame buffer，可解析拆分或合并的消息。配置 bank 继续独立执行 UTF-8、JSONC/CSS/KDL/environment parse-before-swap；有效 `RELOAD` 的 config generation 2 已通过同一 poll loop 触发 policy generation 2，非法 reload 仍不发布下一代。私有 `0x534c0005` 像素 staging ABI 已移除；当前仍缺通用 AF_UNIX path/credentials、普通第三方 client、多 client Wayland ownership与通用 mmap/munmap。framebuffer、PS/2 采集、niri 状态机与最终 GOP 合成目前仍属于 kernel mechanism。

内核还会在启动时通过一个独立的 eBPF verifier 执行内建测试程序；当前只是无动态分配、前向控制流的安全子集，并不声称兼容 Linux eBPF。

QEMU 另挂载一个可重复生成的 256 MiB、双 block-group ext4 root disk。异步 mount/file API 核对复杂读取路径；读写 fd 3 除原位覆写外，还能从 EOF 4096 追加一整块：五 tag transaction 分配 block 120、把 inode 31 extent 增长到 8192，descriptor size/offset 同步推进，新增数据再经 fd 读回，最后 truncate/释放恢复。另一组 transaction 分配 inode 32、插入空文件 `/usr/share/slopos/create-probe`，以读写 fd 3 打开并验证 EOF，再 close/unlink。由于 timer preemption 与 desktop event wake 会合法改变进程/cache probe 的交错次序，测试核对设备请求恒等于队列中断加一，而不把某一种合法调度次序写死。两阶段 crash-injection 还会停在 allocation commit 后/home 前，再由普通 kernel 于下次 mount 重放、清理并继续进入桌面。当前 replay 支持最多八个 tag 的零-feature、连续且非 wrap transaction；这些 create/growth 操作仍是启动回归路径，没有通用可写 namespace mutation syscall。

![SlopOS scrolling-tile desktop](evidence/desktop.png)

上图于 2026-07-31 由 `make test-wayland-input` 从当前 QEMU/OVMF 镜像重新生成：顶部是配置驱动的 Waybar 式状态栏，主体是 niri 式横向双列桌面；Terminal 中保留已执行的 `RELOAD`，右侧 System 是 active window，鼠标位于 PID 2 的四色 `xdg_toplevel` XRGB8888 surface 上。脚本保存 PPM/PNG并精确核对 surface 四个 framebuffer 像素；同一回归还经真实 PS/2/QMP 输入验证 `a` 的按下/释放、pointer motion、button press/release、vertical axis 与 frame，并要求 PID 2 的 stream-safe `poll` loop 全部解析成功。wire 走 AF_UNIX `SOCK_STREAM`，backing 来自 `memfd_create` 后的共享 mmap，并在首个 configured batch 通过 `SCM_RIGHTS` 传给服务端。
...
- 同一 PID 2 以 Linux `socket/bind/listen/connect/accept/read/write/sendmsg/recvmsg/poll` 建立并使用 `/run/slopos/wayland-0`：listener/client/accepted-server 分别常驻 fd 3/4/5，当前 accepted endpoint 的协议处理仍由 kernel bootstrap server 承担。客户端通过标准 Wayland core/xdg-shell 请求创建单个 `wl_shm` XRGB8888 `xdg_toplevel`，并绑定 seat/output、创建 pointer/keyboard；它解析 registry、seat capability/name、output geometry/mode/scale/name/description/done、keyboard keymap/repeat、configure 与 presentation 服务端事件并发送正确的 `ack_configure`。server 先经 `recvmsg(SCM_RIGHTS)` 交付只读 XKB keymap fd 6，client 校验后关闭；`memfd_create/ftruncate/mmap(MAP_SHARED)` 再在 fd 6 建立 surface backing，首个 pool 经 `sendmsg(SCM_RIGHTS)` 反向传递 fd，首次 release 后只更新同一共享页并复用 buffer/callback ID 完成第二次 commit/presentation。此后客户端永久 poll Wayland socket fd 4 与配置事件 fd 6，并以持续 frame decoder 消费动态 pointer enter/leave/motion/button/axis/frame、keyboard enter/leave/key/modifiers；kernel server 严格分派对象生命周期并把第二帧用户像素合成进 System 窗口；
...
- niri KDL、Waybar JSONC/CSS 与 swww 配置/状态机子集（含真实 `cooldown-ms` 限流的全局 Mod+IntelliMouse 滚轮绑定、app-specific 初始宽高/焦点/浮动锚点、两类最大化与全屏规则），加桌面提交/事件协议、Wayland wire/server 与 local-stream core，共 87 项相关宿主测试（56 + 11 + 13 + 7）；
...
桌面现在已有跨运行时 reload 的双向用户态服务边界和首个实际用户态 Wayland surface，但尚未整体迁出内核。`/sbin/slop-shell` 是 lifecycle-aware 的常驻 policy/provider/client 进程：它验证 root VFS 配置，以 Linux `bind/listen/accept` 持有 listener 生命周期，经 AF_UNIX stream 与双向 `SCM_RIGHTS` 完成 registry、seat/output/pointer/keyboard discovery、XKB keymap 导入、configure/ack 与两轮 commit/presentation，再用 Linux `poll` 同时等待配置 generation 与持续 Wayland 键鼠事件；PID 1 则常驻 `wait4` 作为 supervisor。kernel 仍持有配置文件发现/parse bank、niri 状态机、PS/2 采集与命中测试、swww daemon state、accepted endpoint 的 Wayland protocol bootstrap、GOP renderer 与固定窗口 placement。当前只有 PID 2 可绑定固定路径，尚无普通第三方 Wayland client、多 client object ownership、通用 AF_UNIX path/credentials、通用 mmap/munmap、配置编辑器或 watcher；Waybar/swww 也还不是独立 layer-shell client。完整兼容边界见 [docs/desktop-shell.md](docs/desktop-shell.md)，保守完成度见 [docs/status.md](docs/status.md)。
...
- QEMU 10.0.11；
- OVMF 2025.02；
- LLVM 的 `llvm-strip`/`llvm-objcopy`，以及 `mtools`、`dosfstools`、`e2fsprogs`、`socat`（交互回归的 QMP 输入 socket）；
- 可选 `netpbm`，用于把 QEMU PPM 截图转换为 PNG；
- 可选 `libxkbcommon-tools`，用于以 `xkbcli compile-keymap --from-xkb` 独立校验仓库内 XKB keymap。
...
`make test-acpi` 在宿主运行 RSDP/XSDT/MADT parser 的构造表测试，`make test-ebpf` 运行 verifier/interpreter 边界测试，`make test-elf` 的 10 项测试覆盖 ELF64 header、`PT_LOAD`、BSS、范围/对齐/重叠/W^X 与 entry validation，`make test-process` 的 6 项测试覆盖 Linux 初始栈、PID/capacity、`Blocked/Runnable` 生命周期、round-robin selection、child lookup/reap（含 immediate zombie）、每进程 file/socket/shared-memory fd isolation与退出清理，`make test-shell` 运行 56 项 shell 与 11 项 desktop protocol 测试，覆盖 niri/Waybar/swww 状态机及两个版本化桌面 ABI；`make test-wayland` 的 13 项测试覆盖 frame/argument/object map、registry/core/xdg 分派、`wl_shm` FD 边界，以及 seat/pointer/keyboard/output 与完整实时输入事件编码。`make test-pci` 运行 PCI multifunction/capability 枚举测试，`make test-virtio` 检查 split-ring layout 及 read/write/flush descriptor chain，`make test-ext4` 的 28 项测试覆盖 superblock/group/inode/extent/directory/symlink、block/inode allocation、目录项 mutation、多 tag JBD2 records 和 recovery/state 更新，`make test-vfs` 的 8 项测试另检查 descriptor file/socket/shared-memory/desktop-event object与共享 offset。`make test-boot` 在 OVMF 中验证两个 root VFS ELF、独立 CR3、双向 timer preemption、异步 file/socket syscall、PID 1 supervisor 与 PID 2 service 常驻、fd isolation、policy/config generation，以及 PID 2 对 `/run/slopos/wayland-0` 的 `bind/listen/connect/accept` listener 生命周期、registry→seat/output/pointer/keyboard discovery→keymap fd→configure→ack-configure→两轮 presentation 标准 Wayland 往返、`memfd_create/ftruncate/mmap`、双向 `SCM_RIGHTS`、事件 descriptor 与 Linux `poll`。`make test-wayland-input` 进一步执行真实配置 reload、键盘、指针与滚轮输入，要求 PID 2 的 stream-safe poll loop 解析全部动态 Wayland event，并生成当前桌面截图。`make test-interaction` 与 `make test-desktop-custom-config` 继续覆盖真实 PS/2、niri/Waybar/swww 行为；`make test-page-fault` 核验 vector 14/CR2；`make test-journal-replay` 验证五 tag crash/replay、桌面继续运行和宿主 fsck。

`make test-ipc` 另运行 7 项具名 local-stream core 测试，覆盖 bind/listen/connect/accept、无损耗 listener readiness、双向分段 byte stream、backlog、ring backpressure/readiness、close/EOF、stale-handle generation、不会留下半帧的 exact send，以及两个方向上与一批 stream bytes 原子关联的单个 generation-checked rights object。QEMU 进一步验证该 core 已进入 process descriptor、Linux `socket/bind/listen/connect/accept/read/write/sendmsg/recvmsg/poll/close` 与 block-task 异步完成路径：client→server 传 surface memfd，server→client 传只读 XKB keymap fd。`accept` 的 Blocked/Runnable continuation 已实现并以宿主 readiness 测试覆盖；本轮 QEMU 路径因先 self-connect 再 accept，只验证立即成功分支。当前 bind path、owner与backlog仍是 PID 2 的有界 bootstrap，accepted endpoint由kernel处理Wayland协议；尚无 epoll、credentials、通用 ancillary data或多client service ownership。
