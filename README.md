...
- 非默认但可解析的 niri KDL 与 Waybar JSONC 能从临时 ext4 副本注入；实机回归确认 `open-focused true` 会让规则放到后台 workspace 的 Config 主动获得焦点并以 1024×768 fullscreen 打开，`layer: "top"` 的 Waybar 仍在它上方合成并吞掉空白栏点击，退出后依次恢复 edge/column/app-rule 初始尺寸；`focus-ring { off; }`、4 px active border 与带 RGBA、offset/spread/softness 的 window shadow 均按规则合成，`draw-border-with-background false` 让 `opacity 0.75` 的 surface 透出不同位置的壁纸而不是边框填充，fullscreen 则强制不透明并省略全部 window decoration；Waybar ordered `output` array 命中 `SLOPOS-1` 后，自动 output CSS class 把底边改为 `#ff79c6`，`name: "slop-main"` 的同名 class 再把 bar 背景改为 `#202640`；`margin: "4 12"` 把 surface 精确限制到 `(12,4)–(1012,44)` 并将 exclusive reserve 改为 44 px，`fixed-center false` 与三个 `expand-*` 又让 center block 依 GTK packing 平分剩余空间，真实 workspace 点击随左右 block 宽度重新定位。第二次短启动以 string `output: "SLOPOS-1"` 配合 `width: 800`/`no-center: true` 与 `modes.slop-overlay`，把 bar 居中限制到 x=112..912、移除 center module、将 reserve 归零并启用 passthrough：bar 覆盖 y=16 的窗口标题栏，在 surface 内点击仍穿透并关闭 Terminal。第三次启动用 `["!SLOPOS-1", "*"]` 排除当前 output，使 bar 不实例化、reserve 归零且 signal 不能绕过过滤。脚本以十六个确定 framebuffer 像素验证这些路径；`default-floating-position x=24 y=24 relative-to="bottom-right"` 在移入浮动层时产生精确右下间距，移动后的浮动位置经过 tiling 往返仍会恢复。切回 main 后旧 Terminal 焦点也保持不变；
- 两张可在运行时切换的嵌入式 P3/PNM 壁纸；
- niri KDL、Waybar JSONC/CSS 与 swww 配置/状态机子集（含真实 `cooldown-ms` 限流的全局 Mod+IntelliMouse 滚轮绑定、app-specific 初始宽高/焦点/浮动锚点、两类最大化与全屏规则），加桌面提交/事件协议，共 58 项宿主测试；
...
`make test-acpi` 在宿主运行 RSDP/XSDT/MADT parser 的构造表测试，`make test-ebpf` 运行 verifier/interpreter 边界测试，`make test-elf` 的 10 项测试覆盖 ELF64 header、`PT_LOAD`、BSS、范围/对齐/重叠/W^X 与 entry validation，`make test-process` 的 6 项测试覆盖 Linux 初始栈、PID/capacity、`Blocked/Runnable` 生命周期、round-robin selection、child lookup/reap（含 immediate zombie）、每进程 fd isolation/seek 与退出清理，`make test-shell` 的 58 项测试覆盖 niri KDL workspace/keyboard+wheel bind/`cooldown-ms`/ordered `default-column-width`/`default-window-height`/`default-column-display`/`default-floating-position`/window `focus-ring`/`border`/`shadow`/`draw-border-with-background`/`opacity`/`open-maximized`/`open-maximized-to-edges`/`open-fullscreen`/`open-focused` rule/floating layer/fullscreen/workspace reorder/column width/window height/preset/tabbed column/directional consume-or-expel/column+window workspace transfer/strip-boundary focus+reorder/column+edge maximize/focused+visible center/expand、Waybar JSONC output/option/signal/format 与 CSS、swww CLI/daemon/PNM/PNG/transition/filter、滚动平铺状态机及桌面提交/事件协议，`make test-pci` 运行 PCI multifunction/capability 枚举测试，`make test-virtio` 检查 split-ring layout 及 read/write/flush descriptor chain，`make test-ext4` 的 28 项测试覆盖 superblock/group/inode/extent/directory/symlink、block/inode allocation、目录项 mutation、多 tag JBD2 records 和 recovery/state 更新，`make test-vfs` 的 5 项测试检查绝对路径、mount-prefix、fd offset/access mode、`close_all` 与 EOF growth。`make test-boot` 在 OVMF 中验证两个 root VFS ELF→独立 Linux initial stack/CR3→CPL3 cooperative yield 与双向 timer preemption、异步 `openat/read/write/close/wait4` suspend/completion、PID 1 supervisor 与 PID 2 service 常驻、同号 fd isolation、跨两页 user copy、policy/config 两类应用事件及连续 generation，以及 virtio 请求/INTx completion 的 `requests = interrupts + 1` 关系、桌面配置和全部存储探针。`make test-interaction` 注入真实 PS/2 命令，验证 Waybar 默认 SIGUSR1 toggle 的 bar 隐藏/恢复及 40→0→40 px working-area 重排、config generation 2 唤醒 PID 2并产生 policy generation 3、非法 CSS 回滚、swww Sunset 换图、root VFS 双块动态 DEFLATE PNG 异步加载与 missing-path 保留旧图、纯色 `clear`/`query`、kill/restart、grow/wipe/fade/wave、duration/Bezier 与全部五种 resize filter 的整机路径；随后验证单窗/整列跨 workspace 的差异、Home/End 首末列导航/重排、PageUp/PageDown workspace 整体重排与 named 引用、150 ms 双包 burst 限流、四组 Mod+IntelliMouse workspace/column focus+move+reorder、tiled/floating 全屏与原层恢复、覆盖状态下拒绝 Waybar 点击、显式 floating/tiling move 与 focus target，以及 `Mod+V` 的 tiling→floating→跨层焦点→浮窗下移→tiling 闭环和完整 niri resize/center/expand/maximize/tabbed/consume/workspace/pointer 回归；`make test-desktop-custom-config` 同时注入非默认 niri/Waybar，验证 `open-focused true` 把 Config 激活到目标 workspace、`open-fullscreen true` 的 1024×768 初态、退出后的三级几何恢复、main 旧焦点、tab 指示条、`opacity 0.75` 与空心 decoration、右下浮动锚点、4 px border/软阴影，并验证 Waybar `SLOPOS-1` string/ordered-array output 选择与排除、4/12px margin、44px exclusive reserve、非固定 center 与三向 expand 分配、name/output class 样式、固定 width、no-center、左右/中间 placement、`format-alt` 往返、五类 module action、IntelliMouse packet、壁纸双向切换与输入保持；`make test-page-fault` 核验 vector 14/CR2；`make test-journal-replay` 验证五 tag crash/replay、桌面继续运行和宿主 fsck。
