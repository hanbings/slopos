...
root image 现在包含两个独立 Rust `no_std` ELF：inode 23 的 `/sbin/slop-init`（26200 bytes）和 inode 24 的 `/sbin/slop-worker`（25488 bytes）。kernel 经 ext4 path walker 各跨七个文件块读入；PID 1 image 仍须与 BootInfo v2 保留的 ESP 引导副本逐字节相同，worker 则只来自 root VFS。两者各有独立 CR3、user code page、两页 user stack、Linux `argc/argv/envp/auxv`、保存的 syscall frame、pending request 和容量 8 的 fd 表。容量 4 的 process table 实现 `Ready/Running/Blocked/Runnable/Exited`，`sched_yield` 驱动 round-robin cooperative switch；QEMU 实测 PID 1 与 PID 2 交错运行，并在同一时刻各自拥有数字相同但 ownership 独立的 fd 3。PID 1 执行 16 次 syscall，完成原有跨两页可逆 write/read 后故意让退出清理释放遗留 fd；worker 执行 7 次 syscall，独立读取配置并正常 close。异步文件调用会保存完整用户 frame、恢复 kernel CR3 并让 block Future 等待 virtio IRQ，completion 将 `Blocked → Runnable` 后恢复该进程 CR3/寄存器并 `SYSRETQ`；没有在 handler 内 busy-wait。当前调度仍是显式 yield，不含 timer preemption、wait/reap syscall 或地址空间回收。
