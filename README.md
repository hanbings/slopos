...
- 同一 PID 2 通过标准 Wayland core/xdg-shell 请求创建单个 `wl_shm` XRGB8888 `xdg_toplevel`，解析 registry/configure/presentation 服务端事件并发送正确的 `ack_configure`；kernel server 严格分派对象生命周期并把用户像素合成进 System 窗口，当前仍是有界 syscall bootstrap transport；
