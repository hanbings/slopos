...
QEMU 另挂载一个可重复生成的 256 MiB、双 block-group ext4 root disk。异步 mount/file API 核对复杂读取路径，固定容量 VFS 通过 fd 3 读写已分配数据。内核校验 inode 8 的 JBD2 superblock，先对 inode 25 的数据块执行完整单块事务，再以两笔事务修改/恢复 inode-table block 中的 size 与 metadata checksum；每笔都持久化 recovery/start，按 flush 边界发布并读回 descriptor/data/commit，checkpoint home block 并推进 sequence。标准 clean boot 的 8-entry cache 记录 64 hit/54 miss/5 invalidation，共 193 个设备请求、192 次队列中断。两阶段 crash-injection 还会停在 commit 后/home 前，再由普通 kernel 于下次 mount 重放、清理并继续进入桌面。当前 replay 只支持零 feature、连续且不跨 journal 尾部的单块 transaction；尚无 allocation/extent/directory transaction、文件增长或通用可写 namespace。
...
`make test-acpi` 在宿主运行 RSDP/XSDT/MADT parser 的构造表测试，`make test-ebpf` 运行 verifier/interpreter 边界测试，`make test-pci` 运行 PCI multifunction/capability 枚举测试，`make test-virtio` 检查 split-ring layout 及 read/write/flush descriptor chain，`make test-ext4` 的 20 项测试覆盖 superblock/group/inode/extent/directory/symlink、inode metadata 编码、JBD2 records 和 recovery/state 更新，`make test-vfs` 检查绝对路径、mount-prefix、fd offset 和 access mode。`make test-boot` 在 OVMF 中验证上述硬件路径、193 次 virtio 请求及 192 次 INTx completion、fd write、active data/metadata transaction、IRQ、async timer 和桌面循环。`make test-interaction` 注入真实 PS/2 键鼠事件；`make test-page-fault` 核验 vector 14、RIP、error code 和 CR2；`make test-journal-replay` 先生成 committed/未 checkpoint 的 dirty disk，再以普通 kernel 重启验证 mount-time replay、205 次请求/204 次 completion、桌面继续运行和宿主 fsck。
