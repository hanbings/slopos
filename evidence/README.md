...
| `custom-config-serial.log` | `make test-desktop-custom-config` | 不同长度、left/center placement 与 clock 左/右键 action 的合法 Waybar override 经 PID 2 分块 hash、两代 policy、config apply、中央 workspace 点击与受限 action 生效 |
...
`make test-desktop-custom-config` 不修改仓库标准镜像：脚本复制 ESP/rootfs，用 `debugfs` 在临时 ext4 中把 inode 20 替换成 1024-byte、仍可解析的 JSONC，把 `niri/window`/`niri/workspaces` 分别放到 left/center，并为 clock 配置 `on-click: status`/`on-click-right: help`。`custom-config-serial.log` 证明 PID 2 在 policy generation 1 与 config generation 1 唤醒后的 generation 2 都读到 1024 bytes，并两次提交新的 Waybar hash `0xe839832a11174af2`，而不是默认 `0xd34d4a92c88d065b`；swww hash 保持不变。末尾 marker 为 `desktop service parked ... after_generation=1`，随后真实 PS/2 点击中央 workspace `2`/`1` 往返；终端先收到尚未执行的 `ABO`，左击右侧 clock 后日志记录 action accepted 与 `command=STATUS`，截图保留 `ABO` 输入，补入 `UT` 后又产生 `command=ABOUT`；最后右击 clock 产生 `button=right action=help` 与 `command=HELP`，第三张截图保存 HELP 画面。全程没有 exit/FATAL，宿主 `e2fsck -fn` 也接受测试副本。这覆盖有效自定义文件、module placement、点击几何、左右键受限 action 和输入缓冲保持，不覆盖自动文件监听、POSIX shell 或完整 Waybar schema。
