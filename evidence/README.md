...
| `waybar-overlay-serial.log` | `make test-desktop-custom-config` | 1189-byte `mode: "slop-overlay"` custom-mode fixture 解析为 overlay layer、零 exclusive reserve、passthrough 与 visible，并让 bar 下方的 System 关闭按钮收到左击 |
...
| `wallpaper-vfs-loaded.png` | `make test-interaction` | `swww img /usr/share/slopos/vfs-wallpaper.png` 令 block task 从 inode 30 异步读齐两个 block，经双 bank 解码 16-bit PNG 并发布 RGB8 raster 后完成 center transition；Terminal 显示 `SWWW VFS IMAGE APPLIED` |
