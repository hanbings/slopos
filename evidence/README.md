...
virtio layout 测试由 `make test-virtio` 执行。裸机 `SLOPOS-VIRTIO` 证据来自真实 descriptor DMA 与 INTx→waker→Future：queue size 8，root device 报告 524288 sectors，cache 以 20 hit/14 miss 减少重复 block lookup，并实际执行 1 个双请求批次；另加 superblock 后共完成 15 次 DMA 请求，top half/queue interrupt 计数均为 14。
