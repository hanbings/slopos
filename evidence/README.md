...
PID 2 的 16 个 syscall 打开 inode 20 Waybar JSONC，以 offset 0/256/512/768 的四个 chunk 读齐 904 bytes并验证 EOF，再打开 inode 17 swww environment，读齐 172 bytes并验证 EOF。`SLOPOS-DESKTOP-SERVICE` submission marker 记录 protocol 1、40-byte commit、`waybar-provider/swww-policy` capability、CPU 0、Memory 36、Aurora，以及 hash `0xd34d4a92c88d065b`/`0xc5edd6e5c5369f52`；私有 syscall marker 记录编号 `1397489665` 与 result 0。desktop task 随后记录 snapshot generation 1、相同 owner/capability/provider 值及 kernel renderer boundary；swww marker 证明初始 Aurora 只在 policy 到达后应用。PID 2 stdout/exit 后 cleanup 为 0/0。

timer 使 wait 顺序真实可变。`page-fault-serial.log`、`journal-replay-serial.log` 与最新 capture 走父进程先 `wait4` 的 blocked→child reap→wake 路径；`serial.log` 和 `interaction-serial.log` 则记录 PID 2 先成为 zombie，PID 1 后续 wait 立即完成。两条路径都写回 child 2/status 0；PID 2/PID 1 reap marker 各记录释放 7 个地址空间 frame并立即复用。`make test-process` 的 6 项宿主测试另行覆盖 initial stack、Blocked/Runnable/round-robin、PID/parent/child lookup/lifecycle/reap、exit cleanup 和 per-process fd isolation/seek；四条 QEMU 回归都重复经过 hardware syscall fast path 与双向 timer preemption。当前证据不证明任意 exec、多 segment mapping、并发 syscall、通用 wait selector/options/orphan adoption 或通用 namespace mutation。
