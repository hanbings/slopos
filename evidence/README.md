...
| `desktop.png` | `scripts/capture-desktop.sh` | 2026-07-31 从当前 AF_UNIX Wayland 镜像重生成；展示 niri 式 column strip、Waybar 式顶部三区域，以及 System 窗口内来自 PID 2 第二帧 XRGB8888 buffer 的四色 surface；脚本精确核对 `(900,123)`/`(948,123)`/`(900,159)`/`(948,159)` |
...
首代 policy acknowledge 后，串口先记录 AF_UNIX listener、PID 2 的 Linux `socket` fd 3 与 `connect /run/slopos/wayland-0`。12/216/148/64-byte Wayland request均经异步 stream `write` 进入 server endpoint，156/56/32/32-byte event均经 server endpoint写回并由异步 `read` 交给 PID 2；每组 marker同时记录 ring request/event byte数与 blocked I/O completion。客户端逐项核对五个 global、两种 shm format、32×24 toplevel configure、serial 1、release/done/delete-id和两次 callback data。renderer acknowledge令 generation 1/2分别切换双 bank 0/1；`capture-desktop.sh` 读取第二帧四色 quadrant的四个精确 framebuffer像素。两帧3072-byte backing仍经明确记录的 `0x534c0005` staging syscall，因为尚无 `SCM_RIGHTS`/shared mapping；证据也不声称多客户端或持续 frame loop。
...
ext4 parser 测试由 `make test-ext4` 执行。裸机日志证明 4096-byte block、65536 blocks、32 inodes、2 groups、group 0 inode table 37、root extent 39 和 6 个 root entries；superblock/group/inode/directory checksum 均由内核校验。`/sbin/slop-init` 是 inode 23、26344 bytes/seven blocks，`/sbin/slop-shell` 是 inode 24、36608 bytes/nine blocks。6144-byte Adam7 16-bit RGB PNG `vfs-wallpaper.png` 是 inode 30；`deep-extent.bin` 是 inode 28，后者从 root index进入 leaf block 106，验证 checksum后读取 logical block 8 的 physical block 113，并将 logical block 7 hole零填充。inode 29 的两个目录块均经 checksum parser，目标 hard link 在第二块解析为 inode 21；path walker还从 inode 14取得 inline symlink target。

`write-probe.bin` 是 inode 31 / physical block 118。PID 1 先在 offset 123 处理两次跨页 64-byte payload并恢复；kernel probe 再处理两次 73-byte payload。ext4 层每次都执行整块 read-modify-write、flush、cache invalidation 与 fd 读回。固定 metadata 后镜像 SHA-256 为 `c20b31e59588a2c04b289332e39157945fcdcda51634a69d36b3d4abfb9ea10e`；启动测试后的 hash相同且 `e2fsck -fn` 报告31/32 files、4245/65536 blocks。这只证明已分配数据块的有界原位写，不证明用户态文件增长或 metadata mutation syscall。
...
第二个裸机 marker 证明 sequence 1 / target block 118 的 descriptor/data/commit records 被写到 32802–32804；descriptor+data 和 commit 分别由 flush 隔开，三块读回一致，之后清零恢复。marker 明确带 `active=false`，因为尚未写 journal state 或 ext4 recovery bit。
...
第四个 marker 证明真正组合的单块 active data transaction：recovery/start 和 descriptor/data/commit 均跨 flush 持久化，DMA readback 验证此时可 replay；home block 118 checkpoint 后推进 sequence 2/start 0 并清 recovery。测试收尾清 records、恢复全 `P` home block并将 sequence 回卷到 1，因此启动后的 image SHA-256 仍为固定值且 `e2fsck -fn` 通过。
...
第六个 marker 证明 fd 3 的 append/truncate 与五 tag allocation transaction 同步覆盖 blocks 0/1/33/38/119。descriptor 在 EOF 4096 取得 4096-byte append window；内核把 superblock/group free count 各减一、更新 block bitmap CRC32C 与 descriptor checksum、增长 inode size/i_blocks/extent，并把 node size 扩为 8192、offset 推进到 EOF。新增 logical block 1 经普通 fd read 路径读回全 `G`；第二笔 transaction 释放 block，descriptor truncate 回 4096，五块逐字节恢复。

第七个 marker 证明 VFS create/unlink transaction 同步覆盖 blocks 0/1/36/38/104。全局/group 1 free inode 与 `itable_unused` 由 1→0，inode bitmap CRC、group checksum、inode 32 checksum 和 directory tail checksum 全部重算；正常 path walker 打开 size 0 的 `create-probe`，固定表为它复用读写 fd 3 且 read 返回 EOF。close 后第二笔 transaction 经共享 directory remover 与 inode-bitmap encoder 回到原始五块。最终固定 hash/fsck 排除 inode、目录项或计数泄漏。

两阶段 recovery 证据来自独立 injection/replay 日志。phase 1 marker 明确记录 sequence 1/start 1、五个 targets 0/1/33/38/119、`allocated/grown` 旧状态、`free/original` 新状态与 `after_commit_before_home` 停止点；宿主同时确认 recovery feature、free blocks 61290、bitmap、inode 31 size/blockcount 与全 G data。phase 2 普通 kernel在任何 ext4 path read前报告五 tag replay，随后执行恢复后的两个 user ELF、双向 timer preemption、desktop policy commit、AF_UNIX Wayland两轮 presentation及异步VFS读写。宿主比较五个 crash home与注入前快照，确认free blocks 61291、block 119释放并运行五阶段fsck；脚本最后恢复固定-hash标准镜像。

VFS 宿主测试由 `make test-vfs` 执行。6 项测试覆盖 path/mount/fd offset、access mode、EOF growth与file/socket object区分。裸机 marker证明从固定 root path读出 inode 23/24 的七/九块 ELF；PID 1以fd 3异步读取inode 18，PID 2首轮以fd 3读取inode 20/17并提交policy，随后让Wayland socket常驻fd 3、以fd 4完成配置热重读。四次AF_UNIX request/event阶段完成configure、buffer/callback reuse与两轮presentation。kernel probe继续覆盖file append/truncate与create/open/close/unlink；mount与二维file backing array仍是block task局部固定容量状态。
