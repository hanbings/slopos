...
首代 policy acknowledge 后，PID 2 通过 `0x534c0003`/`0x534c0004` 完成三阶段 Wayland 往返。12-byte `get_registry` 请求产生 156-byte event，客户端逐项核对五个 global；216-byte binding/role/title/app-id/bufferless initial commit 产生 56-byte event，包含 `wl_shm.format` 的 ARGB/XRGB8888、32×24 空 state 的 `xdg_toplevel.configure` 与 serial 1 的 `xdg_surface.configure`；148-byte configured commit 则先 `ack_configure(1)`，再创建 XRGB8888 buffer、设置 window geometry、attach/damage/frame/commit，并携带 3072 bytes 像素，完整信封为 3252 bytes。renderer acknowledge 后发布 32-byte `wl_buffer.release`、`wl_callback.done(1)` 和 `wl_display.delete_id(11)`，PID 2 三次都经 `Blocked → Runnable` 后解析精确对象与参数。server accept、desktop acknowledge 与 compositor render marker 使用同一 surface generation 1，目的地为 System window、scale 3；`capture-desktop.sh` 又读取四色 quadrant 中的四个精确 framebuffer 像素，证明并非只验证 metadata。当前仍是 PID 2 专用 syscall bootstrap，不是 Unix socket、`SCM_RIGHTS`、共享 mapping、多客户端或可重复 buffer lifecycle。
