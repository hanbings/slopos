...
| `custom-config-serial.log` | `make test-desktop-custom-config` | 4082-byte niri override 的 rule size/tabbed display、opening/focus/三级 state、opacity/空心 border/shadow/floating position 与 1372-byte Waybar override 一起从临时 ext4 原子应用；Waybar marker 记录 4/12px margin、1000×40 surface、44px exclusive reserve、非固定 center，随后经 PID 2 分块 hash、两代 policy、left/center placement、真实 workspace 点击、alternate format 与三键/双向滚轮 action 生效 |
...
| `custom-config-edge-maximized.png` | `make test-desktop-custom-config` | 对初始 fullscreen Config 执行 `Mod+Shift+F` 后，精确恢复底层 x=0/y=44/1024×724 edge maximize；Waybar 重新可见，layout gap/border 仍移除 |
...
`make test-desktop-custom-config` 不修改仓库标准镜像：脚本复制 ESP/rootfs，用 `debugfs` 把默认 3797-byte niri KDL 替换成 4082-byte 用户文件，并将 904-byte Waybar JSONC 替换成 1372-byte 用户文件。niri override 覆盖 opening focus、三种 opening state、66.7%×50% 尺寸、tabbed display、floating anchor、focus ring、border、shadow、background-mode 与 opacity；Waybar override 加入 `margin: "4 12"`、`fixed-center: false`、`exclusive: true`，交换 window/workspace 的 left/center placement，并给 clock 配置 alternate format、三键与双向滚轮 action。Waybar geometry marker 精确记录 x=12/y=4/1000×40、margin 4/12/4/12 与 reserved top 44；因此 Config 的三级底层几何变为 x=16/y=60/992×338 column maximize、x=0/y=44/1024×724 edge maximize 和 x=184/y=60/656×338 rule size，而 fullscreen 保持 x=0/y=0/1024×768。截图与脚本核对 fullscreen surface `171c2b`；普通 surface x=200/800、y=350 为 `222247`/`222a4b`，border x=182 为 `ffb86c`，shadow x=844/y=200 为 `0a0a1f`。bar 的 x=0/12、y=20 和 x=20、y=2/4 又分别为 wallpaper/panel，直接证明横向和顶部 margin。首次 floating rect 为 x=8/y=406/992×338，移动后 y=430 经 tiling 往返仍恢复。`custom-config-serial.log` 证明 PID 2 两轮都读到 1372 bytes，并提交相同的非默认 hash `0x527c71ac5687438d`；末尾仍阻塞在 `config-applied after_generation=1`。退出 fullscreen 后，workspace block 会随左侧 Config/Terminal title 宽度改变而移动；更新后的真实 PS/2 坐标依次点击 `1`/`2`/`1`，既证明非固定 center/hit-test 共用几何，也证明 main 的旧 Terminal 局部焦点未被覆盖。clock 左击往返 `UTC`/`UTC ALT` 并执行 STATUS，右击执行 HELP，中击执行 SWWW QUERY，双向滚轮切换 Sunset/Aurora；输入缓冲仍补成 ABOUT。十一张截图覆盖 opening focus、fullscreen/edge/column/rule 四态、Waybar geometry/workspace、alternate、输入/action 与壁纸。全程没有 exit/FATAL，宿主 fsck 通过；这不覆盖 layer/mode/passthrough、自动文件监听、POSIX shell 或完整配置 schema。
