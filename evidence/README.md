...
| `interaction-serial.log` | `make test-interaction` | PS/2 键盘触发 VFS 配置 reload/rollback、执行 `STATUS`/swww image/clear/query，鼠标横拖 viewport、Super+右键缩放、点击 Waybar workspace，Super bind 固定与左右方向感知的合并/拆列、normal/tabbed display、显式与 preset 窗高/reset/同列重排与聚焦、gap-aware 显式/preset/maximized/single+focused+visible centered/available-width 列操作、无 gap edge maximize/restore、重排/按索引切换与移列/关闭窗口 |
...
shell/protocol 状态机由 `make test-shell` 验证。17 项 layout 测试覆盖 niri KDL、稳定 strip coordinate、focus/scroll/single+focused+visible center/expand/normal+tabbed stack/fixed+directional consume/expel/window+column reorder/gap-aware explicit+preset+column/edge maximize width/height resize+reset/close；3 项 niri shell 测试覆盖 named workspace、64 项有界 bind 表、rule/size/preset/display parse/reject、ordered override、同 strip reorder、跨 workspace move 和容量内动态 normalize；4 项 Waybar JSONC/format 与 3 项 CSS 测试覆盖 module list/option、replacement、cascade、color/box/border 与拒绝边界；7 项 swww 测试覆盖 CLI/environment parse、daemon lifecycle/output、P3/PNM 边界、两个嵌入式 asset 和 transition mask/blend；5 项 desktop protocol 测试覆盖两种 event kind 的 round-trip 与 header/capability/range/generation/reserved 拒绝边界，共 39 项。裸机先由 PID 2 发布 provider/wallpaper policy并等待 apply event，再从 `/etc/slopos/{niri.kdl,waybar.jsonc,waybar.css,swww.env}` 读取 kernel reload bank并原子发布 config generation 1；apply marker 记录 3 个 workspace、42 个 bind、6 个 Waybar module config 与 12 条 CSS rule，service 随后重读并发布 policy generation 2。
...
`write-probe.bin` 是 inode 31 / physical block 116。PID 1 先在 offset 123 处理两次跨页 64-byte payload并恢复；kernel probe 再处理两次 73-byte payload。ext4 层每次都执行整块 read-modify-write、flush、cache invalidation 与 fd 读回。固定 metadata 后镜像 SHA-256 为 `d45187ea25557038b239f57f9a624be77bc3b228b179ad35b605798a7a23c679`；启动测试后的 hash 相同且 `e2fsck -fn` 报告 31/32 files、4243/65536 blocks。这只证明已分配数据块的有界原位写，不证明用户态文件增长或 metadata mutation syscall。
