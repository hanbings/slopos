...
用户进程证据从 UEFI 日志开始：loader 从 ESP 读取 25896-byte `/slopos/init.elf`，BootInfo v2 令 kernel 在 `LOADER_DATA` allocation 保留同一大小的校验副本。executor/block task 挂载 root 后，`SLOPOS-VFS` marker 证明从 inode 23 的 `/sbin/slop-init` 跨七个逻辑块读取 25896 bytes，且 `matches_boot=true`。process-table marker 记录 capacity 4、PID 1 Ready、每进程 fd capacity 8；load marker 记录 `source=vfs path=/sbin/slop-init format=elf64`、entry `0x40000000`、1 个 segment、2160 load/memory bytes，以及独立 CR3、user code/stack、不同 physical frame 与 `code=user-readonly stack=user-writable kernel=supervisor`。initial-stack marker 记录 `RSP=0x40001ec0`、16-byte alignment、`argc=2`、3 项 environment、9 对 auxv 与 320 used bytes；用户 ELF 自己在发出 syscall 前逐项核对 pointer/string/value。fast-path marker 证明 CPUID 检查后的 MSR readback 为 `STAR=0x10000800000000`、`FMASK=0x47700`、`EFER.SCE=true`，且 LSTAR 指向 kernel entry。

15 个 syscall marker 先证明 `O_RDONLY openat/read/close` 读取并核对 inode 18 的 76-byte 配置；随后 `O_RDWR openat` 复用 fd 3，四次 `lseek(SEEK_SET, 123)` 与两组 async write/read 把 inode 31 的 16 bytes patch 为 `0xa5`、读回验证、恢复为 `P` 并再次验证。最后 stdout write 经 `SYSRETQ` 返回 18，exit status 0 回到 kernel；retained record 为 Exited/15 syscalls。`make test-process` 的 6 项宿主测试另行覆盖 initial stack、PID/lifecycle/reap 和 per-process fd isolation/seek；交互、page-fault 和 journal 两阶段测试都重复经过硬件 fast path。它证明每进程 fd 已连接真实 root ext4 backing object，且同步文件 I/O 由暂停和 IRQ completion 实现；但仍只覆盖固定 root、单页 bounded user copy 与一个 block-task 驱动的 running process，不证明任意 exec、多 segment mapping、调度、并发 syscall 或通用 namespace mutation ABI。
...
ext4 parser 测试由 `make test-ext4` 执行。裸机日志证明 4096-byte block、65536 blocks、32 inodes、2 groups、group 0 inode table 37、root extent 39 和 6 个 root entries；superblock/group/inode/directory checksum 均由内核校验。`/sbin/slop-init` 是 inode 23、25896 bytes/seven blocks。`multiblock.bin` 是 inode 30，`deep-extent.bin` 是 inode 27；后者从 root index 进入 leaf block 97，验证 extent-block checksum 后读取 logical block 8 的 physical block 104，并将 logical block 7 的 hole 零填充。inode 28 的两个目录块均经 checksum parser，目标条目在第二块解析为 inode 29。path walker 还从 inode 14 取得 inline target，并在同一父目录解析到 inode 21。

`write-probe.bin` 是 inode 31 / physical block 110。PID 1 先在 offset 123 处理两次 16-byte payload并恢复；kernel probe 再处理两次 73-byte payload。ext4 层每次都执行整块 read-modify-write、flush、cache invalidation 与 fd 读回。固定 metadata 后镜像 SHA-256 为 `a324677b1147c0c2463562c3a1ce4ae73cf916d1e566003ab95b5d5969e5df9f`；启动测试后的 hash 相同且 `e2fsck -fn` 报告 31/32 files、4237/65536 blocks。这只证明已分配数据块的有界原位写，不证明用户态文件增长或 metadata mutation syscall。
