...
首代 policy acknowledge 后，PID 2 在自己的 stack 上构造 3436-byte surface 信封并通过 `0x534c0003` 提交。串口记录其中 332-byte 标准 Wayland wire 建立 registry/compositor/shm/xdg_toplevel 对象链，surface/buffer/callback ID 为 6/8/11，title 为 `SlopOS Userspace`、app-id 为 `slopos-system`；其后 3072 bytes 是 32×24、stride 128、format 1 的 XRGB8888 snapshot。server accept、desktop acknowledge 与 compositor render 三条 marker 使用同一 generation 1，目的地为 System window、scale 3。`capture-desktop.sh` 又读取四色 quadrant 中的四个精确 framebuffer 像素，证明并非只验证 metadata。当前这是 PID 2 专用 syscall bootstrap，不是 Unix socket、`SCM_RIGHTS`、共享 mapping 或完整 Wayland configure/event round trip。
...
ext4 parser 测试由 `make test-ext4` 执行。裸机日志证明 4096-byte block、65536 blocks、32 inodes、2 groups、group 0 inode table 37、root extent 39 和 6 个 root entries；superblock/group/inode/directory checksum 均由内核校验。`/sbin/slop-init` 是 inode 23、26344 bytes/seven blocks，`/sbin/slop-shell` 是 inode 24、33016 bytes/nine blocks。6144-byte Adam7 16-bit RGB PNG `vfs-wallpaper.png` 是 inode 30，长 `tEXt` ancillary chunk 延伸文件并让两个 IDAT 位于跨块读取路径，所以同时承担 multiblock/prefetch、Adam7/16-bit decode 与压缩 swww path-load probe；`deep-extent.bin` 是 inode 28，后者从 root index 进入 leaf block 106，验证 extent-block checksum 后读取 logical block 8 的 physical block 113，并将 logical block 7 的 hole 零填充。inode 29 的两个目录块均经 checksum parser，目标 hard link 在第二块解析为 inode 21。path walker 还从 inode 14 取得 inline target，并在同一父目录解析到 inode 21。

`write-probe.bin` 是 inode 31 / physical block 118。PID 1 先在 offset 123 处理两次跨页 64-byte payload并恢复；kernel probe 再处理两次 73-byte payload。ext4 层每次都执行整块 read-modify-write、flush、cache invalidation 与 fd 读回。固定 metadata 后镜像 SHA-256 为 `2959e79b49ebe4259414e420500d2a70a57ca7f6c0dc3d355702cf9760be2da8`；启动测试后的 hash 相同且 `e2fsck -fn` 报告 31/32 files、4245/65536 blocks。这只证明已分配数据块的有界原位写，不证明用户态文件增长或 metadata mutation syscall。
