...
required_markers=(
    ...
    "SLOPOS-VFS: executable loaded path=/sbin/slop-shell inode=24 bytes=36840 blocks=9 matches_boot=not-required role=desktop-service"
    ...
    "SLOPOS-EXT4: depth-one extent valid inode=28 leaf_block=106 logical_block=8 bytes=4096 metadata_checksum=valid path=/usr/share/slopos/deep-extent.bin"
    ...
    "SLOPOS-VFS: writable descriptor valid fd=3 inode=31 physical_block=118 offset=123 bytes=73 writes=2 flushes=2 cache_invalidations=2 restored=true path=/usr/share/slopos/write-probe.bin"
    ...
    "SLOPOS-EXT4: journal records staged sequence=1 target_block=118 descriptor_block=32802 data_block=32803 commit_block=32804 writes=6 flushes=3 verified=true restored=true active=false"
    ...
    "SLOPOS-EXT4: active journal transaction valid sequence=1 target_block=118 recovery=true start=1 records=descriptor/data/commit replayable_readback=true home_checkpointed=true next_sequence=2 test_sequence_rewound=true restored=true writes=13 flushes=10"
    ...
    "SLOPOS-EXT4: fd append journal transactions valid fd=3 inode=31 block=119 bitmap_block=33 group_descriptor_block=1 inode_table_block=38 append_bytes=4096 size=4096/8192/4096 extent_blocks=1/2/1 checksums=superblock/group/bitmap/inode/data transactions=2 sequences=1/2 final_sequence=3 test_sequence_rewound=true restored=true"
    "SLOPOS-EXT4: VFS create journal transactions valid fd=3 inode=32 parent_inode=27 inode_bitmap_block=36 group_descriptor_block=1 inode_table_block=38 directory_block=104 free_inodes=1/0/1 size=0 access=readwrite checksums=superblock/group/bitmap/inode/directory transactions=2 sequences=1/2 final_sequence=3 test_sequence_rewound=true restored=true path=/usr/share/slopos/create-probe"
    ...
    "SLOPOS-PROCESS: pid=2 parent=1 source=vfs path=/sbin/slop-shell argv1=--session format=elf64 entry=0x40000000 segments=1 file_bytes=36840 load_bytes=10392 memory_bytes=10392"
    ...
    "SLOPOS-WAYLAND-SERVER: request received pid=2 transport=AF_UNIX/SOCK_STREAM path=/run/slopos/wayland-0 wire_bytes=216 scm_rights=false"
    "SLOPOS-WAYLAND-SERVER: event sent pid=2 transport=AF_UNIX/SOCK_STREAM kind=2 sequence=2 wire_bytes=56 read_bytes=56"
    ...
    "SLOPOS-WAYLAND-SERVER: event sent pid=2 transport=AF_UNIX/SOCK_STREAM kind=3 sequence=3 wire_bytes=32 read_bytes=32"
