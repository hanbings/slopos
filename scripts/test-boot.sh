...
required_markers=(
    ...
    "SLOPOS-VFS: executable loaded path=/sbin/slop-shell inode=24 bytes=38232 blocks=10 matches_boot=not-required role=desktop-service"
    ...
    "SLOPOS-PROCESS: pid=2 parent=1 source=vfs path=/sbin/slop-shell argv1=--session format=elf64 entry=0x40000000 segments=1 file_bytes=38232 load_bytes=10776 memory_bytes=10776"
    ...
    "SLOPOS-SCHED: pid=2 state=blocked->runnable reason=wayland-event kind=1 sequence=1 wire_bytes=156"
    ...
    "SLOPOS-SCHED: pid=2 state=blocked->runnable reason=wayland-event kind=2 sequence=2 wire_bytes=56"
    "SLOPOS-WAYLAND-SERVER: commit accepted pid=2 generation=1 transport=syscall-bootstrap-v1 lifecycle=registry/configure/ack-configure objects=registry/compositor/shm/xdg_toplevel surface=6 buffer=8 callback=11 geometry=32x24 stride=128 format=1 title=\"SlopOS Userspace\" app_id=slopos-system wire_bytes=148 pixel_bytes=3072"
    "SLOPOS-SYSCALL: pid=2 abi=slopos-wayland-bootstrap-v1 entry=syscall return=sysretq nr=1397489667 phase=configured-commit envelope_bytes=3252 wire_bytes=148 pixel_bytes=3072 sequence=1 origin=cpl3 result=0"
    ...
    "SLOPOS-SCHED: pid=2 state=blocked->runnable reason=wayland-event kind=3 sequence=3 wire_bytes=32"
    "SLOPOS-WAYLAND-SERVER: commit accepted pid=2 generation=2 transport=syscall-bootstrap-v1 lifecycle=configured-buffer-reuse objects=registry/compositor/shm/xdg_toplevel surface=6 buffer=8 callback=11 geometry=32x24 stride=128 format=1 title=\"SlopOS Userspace\" app_id=slopos-system wire_bytes=64 pixel_bytes=3072"
    "SLOPOS-SYSCALL: pid=2 abi=slopos-wayland-bootstrap-v1 entry=syscall return=sysretq nr=1397489667 phase=configured-commit envelope_bytes=3168 wire_bytes=64 pixel_bytes=3072 sequence=2 origin=cpl3 result=0"
    ...
    "SLOPOS-SCHED: pid=2 state=blocked->runnable reason=wayland-event kind=3 sequence=4 wire_bytes=32"
