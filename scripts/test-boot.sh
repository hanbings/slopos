...
required_markers=(
    ...
    "SLOPOS-VFS: executable loaded path=/sbin/slop-shell inode=24 bytes=40960 blocks=10 matches_boot=not-required role=desktop-service"
    ...
    "SLOPOS-EXT4: depth-one extent valid inode=28 leaf_block=107 logical_block=8 bytes=4096 metadata_checksum=valid path=/usr/share/slopos/deep-extent.bin"
    ...
    "SLOPOS-VFS: writable descriptor valid fd=3 inode=31 physical_block=119 offset=123 bytes=73 writes=2 flushes=2 cache_invalidations=2 restored=true path=/usr/share/slopos/write-probe.bin"
    ...
    "SLOPOS-EXT4: journal records staged sequence=1 target_block=119 descriptor_block=32802 data_block=32803 commit_block=32804 writes=6 flushes=3 verified=true restored=true active=false"
    ...
    "SLOPOS-EXT4: active journal transaction valid sequence=1 target_block=119 recovery=true start=1 records=descriptor/data/commit replayable_readback=true home_checkpointed=true next_sequence=2 test_sequence_rewound=true restored=true writes=13 flushes=10"
    ...
    "SLOPOS-EXT4: fd append journal transactions valid fd=3 inode=31 block=120 bitmap_block=33 group_descriptor_block=1 inode_table_block=38 append_bytes=4096 size=4096/8192/4096 extent_blocks=1/2/1 checksums=superblock/group/bitmap/inode/data transactions=2 sequences=1/2 final_sequence=3 test_sequence_rewound=true restored=true"
    "SLOPOS-EXT4: VFS create journal transactions valid fd=3 inode=32 parent_inode=27 inode_bitmap_block=36 group_descriptor_block=1 inode_table_block=38 directory_block=105 free_inodes=1/0/1 size=0 access=readwrite checksums=superblock/group/bitmap/inode/directory transactions=2 sequences=1/2 final_sequence=3 test_sequence_rewound=true restored=true path=/usr/share/slopos/create-probe"
    ...
    "user_code=0x40000000 user_stack=0x40007000"
    "code_frames="
    "code=user-readonly stack=user-writable kernel=supervisor"
    "stack_frames="
    "SLOPOS-PROCESS: pid=1 initial_stack abi=linux-x86_64 rsp=0x40006ec0 aligned=16 stack_pages=3 argc=2 argv0=/sbin/slop-init argv1=--system envc=3 auxv_pairs=9 bytes=320"
    "SLOPOS-PROCESS: pid=2 parent=1 source=vfs path=/sbin/slop-shell argv1=--session format=elf64 entry=0x40000000 segments=1 file_bytes=40960 load_bytes=15232 memory_bytes=15232"
    "SLOPOS-PROCESS: pid=2 initial_stack abi=linux-x86_64 rsp=0x40006ea0 aligned=16 stack_pages=3 argc=2 argv0=/sbin/slop-shell argv1=--session envc=4 auxv_pairs=9 bytes=352"
    ...
    "SLOPOS-VFS: process read complete pid=2 fd=3 inode=20 offset=768 requested=256 bytes=136 user_pages=1 cross_page=false async=true"
    "SLOPOS-VFS: process open complete pid=2 fd=3 inode=17 bytes=244 access=readonly async=true path=/etc/slopos/swww.env"
    "SLOPOS-VFS: process read complete pid=2 fd=3 inode=17 offset=0 requested=256 bytes=244 user_pages=1 cross_page=false async=true"
    "SLOPOS-DESKTOP-SERVICE: policy submitted pid=2 generation=1 protocol=1 capabilities=waybar-provider/swww-policy/wayland-surface cpu=0 memory=36 wallpaper=aurora"
    ...
    "SLOPOS-IPC: AF_UNIX SOCK_STREAM namespace ready sockets=8 backlog_capacity=2 bytes_per_direction=512 listener_owner=userspace"
    "SLOPOS-SYSCALL: pid=2 abi=linux-x86_64 entry=syscall return=sysretq nr=41 socket domain=AF_UNIX type=SOCK_STREAM protocol=0 fd=3 origin=cpl3"
    ...
    "SLOPOS-SYSCALL: pid=2 abi=linux-x86_64 entry=syscall return=sysretq nr=41 socket domain=AF_UNIX type=SOCK_STREAM protocol=0 fd=4 origin=cpl3"
    "SLOPOS-IPC: AF_UNIX connect queued pid=2 path=/run/slopos/wayland-0 client=1:1 transport=bounded-stream"
    ...
    "SLOPOS-IPC: AF_UNIX accepted pid=2 path=/run/slopos/wayland-0 listener=0:1 client=1:1 server=2:1 ownership=userspace-listener/kernel-protocol"
    ...
    "SLOPOS-WAYLAND-SERVER: request received pid=2 transport=AF_UNIX/SOCK_STREAM path=/run/slopos/wayland-0 wire_bytes=12 scm_rights=false"
    "SLOPOS-WAYLAND-SERVER: event sent pid=2 transport=AF_UNIX/SOCK_STREAM kind=1 sequence=1 wire_bytes=156 read_bytes=156"
    "SLOPOS-IPC: process read complete pid=2 fd=4 family=AF_UNIX type=SOCK_STREAM requested=256 bytes=156 user_pages=1 async=true"
    "SLOPOS-SYSCALL: pid=2 abi=linux-x86_64 entry=syscall return=suspended nr=47 recvmsg fd=4 requested=256 control_capacity=24 user_pages=1 origin=cpl3"
    ...
    "SLOPOS-WAYLAND-SERVER: request received pid=2 transport=AF_UNIX/SOCK_STREAM path=/run/slopos/wayland-0 wire_bytes=308 scm_rights=false"
    "SLOPOS-WAYLAND-SERVER: event sent pid=2 transport=AF_UNIX/SOCK_STREAM kind=2 sequence=2 wire_bytes=288 read_bytes=256 scm_rights=true"
    ...
    "SLOPOS-IPC: process read complete pid=2 fd=4 family=AF_UNIX type=SOCK_STREAM requested=32 bytes=32 user_pages=1 async=true"
    "SLOPOS-IPC: process read complete pid=2 fd=6 object=memfd shared=0:1 offset=3072 requested=235 bytes=235 object_bytes=3307 user_pages=1 async=false"
    ...
    "SLOPOS-IPC: process close complete pid=2 fd=6 object=memfd shared=0:1 async=false"
    "SLOPOS-SYSCALL: pid=2 abi=linux-x86_64 entry=syscall return=sysretq nr=319 memfd_create name=slopos-wayland-shm flags=0 fd=6 object=1:1 origin=cpl3"
    "SLOPOS-SYSCALL: pid=2 abi=linux-x86_64 entry=syscall return=sysretq nr=77 ftruncate fd=6 bytes=3072 object=1:1 origin=cpl3 result=0"
    "SLOPOS-SYSCALL: pid=2 abi=linux-x86_64 entry=syscall return=sysretq nr=9 mmap address=0x40007000 bytes=4096 prot=read/write flags=MAP_SHARED fd=6 offset=0 object=1:1 object_bytes=3072 physical="
    "SLOPOS-SYSCALL: pid=2 abi=linux-x86_64 entry=syscall return=suspended nr=46 sendmsg fd=4 requested=148 control=SCM_RIGHTS rights_fd=6 user_pages=1 origin=cpl3"
    "SLOPOS-IPC: process write complete pid=2 fd=4 family=AF_UNIX type=SOCK_STREAM operation=sendmsg/SCM_RIGHTS requested=148 bytes=148 user_pages=1 async=true"
    ...
    "SLOPOS-WAYLAND-SERVER: request received pid=2 transport=AF_UNIX/SOCK_STREAM path=/run/slopos/wayland-0 wire_bytes=148 scm_rights=true"
    ...
    "SLOPOS-WAYLAND-SERVER: event sent pid=2 transport=AF_UNIX/SOCK_STREAM kind=3 sequence=3 wire_bytes=32 read_bytes=32 scm_rights=false"
    "SLOPOS-WAYLAND-SERVER: commit accepted pid=2 generation=2 transport=AF_UNIX/SOCK_STREAM backing=SCM_RIGHTS/mmap-shared-v1 lifecycle=configured-buffer-reuse objects=registry/compositor/shm/seat/pointer/keyboard/output/xdg_toplevel surface=6 buffer=8 callback=11 seat=12 pointer=14 keyboard=15 output=13 geometry=32x24 stride=128 format=1 title=\"SlopOS Userspace\" app_id=slopos-system wire_bytes=64 pixel_bytes=3072"
    "SLOPOS-WAYLAND-SERVER: request received pid=2 transport=AF_UNIX/SOCK_STREAM path=/run/slopos/wayland-0 wire_bytes=64 scm_rights=false"
    ...
    "SLOPOS-WAYLAND-SERVER: event sent pid=2 transport=AF_UNIX/SOCK_STREAM kind=3 sequence=4 wire_bytes=32 read_bytes=32"
    "SLOPOS-IPC: process close complete pid=2 fd=6 object=memfd shared=1:1 async=false"
    ...
    "SLOPOS-VFS: process event stream open complete pid=2 fd=6 object=desktop-events cursor_generation=1 access=readonly path=/run/slopos/desktop-events"
    "SLOPOS-SYSCALL: pid=2 abi=linux-x86_64 entry=syscall return=suspended nr=7 poll nfds=2 events=POLLIN timeout=-1 state=blocked origin=cpl3"
    ...
    "SLOPOS-SWWW: policy applied owner_pid=2 image=/usr/share/backgrounds/slopos-aurora.ppm output=SLOPOS-1 transition=simple step=32 fps=30"
    "SLOPOS-DESKTOP-SERVICE: policy applied generation=1 owner_pid=2 capabilities=waybar-provider/swww-policy/wayland-surface cpu=0 memory=36 wallpaper=/usr/share/backgrounds/slopos-aurora.ppm renderer=kernel-mechanism"
