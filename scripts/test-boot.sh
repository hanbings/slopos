...
required_markers=(
    ...
    "SLOPOS-VFS: executable loaded path=/sbin/slop-shell inode=24 bytes=33016 blocks=9 matches_boot=not-required role=desktop-service"
    ...
    "user_code=0x40000000 user_stack=0x40004000"
    "code_frames="
    "code=user-readonly stack=user-writable kernel=supervisor"
    "stack_frames="
    "SLOPOS-PROCESS: pid=1 initial_stack abi=linux-x86_64 rsp=0x40003ec0 aligned=16 stack_pages=2 argc=2 argv0=/sbin/slop-init argv1=--system envc=3 auxv_pairs=9 bytes=320"
    "SLOPOS-PROCESS: pid=2 parent=1 source=vfs path=/sbin/slop-shell argv1=--session format=elf64 entry=0x40000000 segments=1 file_bytes=33016 load_bytes=6456 memory_bytes=6456"
    "SLOPOS-PROCESS: pid=2 initial_stack abi=linux-x86_64 rsp=0x40003ea0 aligned=16 stack_pages=2 argc=2 argv0=/sbin/slop-shell argv1=--session envc=4 auxv_pairs=9 bytes=352"
    ...
    "SLOPOS-WAYLAND-SERVER: commit accepted pid=2 generation=1 transport=syscall-bootstrap-v1 objects=registry/compositor/shm/xdg_toplevel surface=6 buffer=8 callback=11 geometry=32x24 stride=128 format=1 title=\"SlopOS Userspace\" app_id=slopos-system wire_bytes=332 pixel_bytes=3072"
    "SLOPOS-SYSCALL: pid=2 abi=slopos-wayland-bootstrap-v1 entry=syscall return=sysretq nr=1397489667 envelope_bytes=3436 wire_bytes=332 pixel_bytes=3072 generation=1 origin=cpl3 result=0"
    "SLOPOS-WAYLAND-SERVER: commit acknowledged generation=1 renderer=desktop active_bank=0"
