...
required_markers=(
    ...
    "SLOPOS-VFS: executable loaded path=/sbin/slop-shell inode=24 bytes=38176 blocks=10 matches_boot=not-required role=desktop-service"
    ...
    "user_code=0x40000000 user_stack=0x40006000"
    "code_frames="
    "code=user-readonly stack=user-writable kernel=supervisor"
    "stack_frames="
    "SLOPOS-PROCESS: pid=1 initial_stack abi=linux-x86_64 rsp=0x40005ec0 aligned=16 stack_pages=3 argc=2 argv0=/sbin/slop-init argv1=--system envc=3 auxv_pairs=9 bytes=320"
    "SLOPOS-PROCESS: pid=2 parent=1 source=vfs path=/sbin/slop-shell argv1=--session format=elf64 entry=0x40000000 segments=1 file_bytes=38176 load_bytes=11592 memory_bytes=11592"
    "SLOPOS-PROCESS: pid=2 initial_stack abi=linux-x86_64 rsp=0x40005ea0 aligned=16 stack_pages=3 argc=2 argv0=/sbin/slop-shell argv1=--session envc=4 auxv_pairs=9 bytes=352"
    ...
    "SLOPOS-WAYLAND-SERVER: request received pid=2 transport=AF_UNIX/SOCK_STREAM path=/run/slopos/wayland-0 wire_bytes=296 scm_rights=false"
    "SLOPOS-WAYLAND-SERVER: event sent pid=2 transport=AF_UNIX/SOCK_STREAM kind=2 sequence=2 wire_bytes=256 read_bytes=256"
    "SLOPOS-SYSCALL: pid=2 abi=linux-x86_64 entry=syscall return=sysretq nr=319 memfd_create name=slopos-wayland-shm flags=0 fd=4 object=0:1 origin=cpl3"
    "SLOPOS-SYSCALL: pid=2 abi=linux-x86_64 entry=syscall return=sysretq nr=77 ftruncate fd=4 bytes=3072 object=0:1 origin=cpl3 result=0"
    "SLOPOS-SYSCALL: pid=2 abi=linux-x86_64 entry=syscall return=sysretq nr=9 mmap address=0x40006000 bytes=4096 prot=read/write flags=MAP_SHARED fd=4 offset=0 object=0:1 object_bytes=3072 physical="
    ...
    "SLOPOS-WAYLAND-SERVER: event sent pid=2 transport=AF_UNIX/SOCK_STREAM kind=3 sequence=3 wire_bytes=56 read_bytes=56"
