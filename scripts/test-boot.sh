...
required_markers=(
    ...
    "load_bytes=2160 memory_bytes=2160 address_space="
    "user_code=0x40000000 user_stack=0x40002000"
    "code=user-readonly stack=user-writable kernel=supervisor"
    "SLOPOS-PROCESS: pid=1 initial_stack abi=linux-x86_64 rsp=0x40001ec0 aligned=16 argc=2 argv0=/sbin/slop-init envc=3 auxv_pairs=9 bytes=320"
    ...
    "SLOPOS-SYSCALL: pid=1 abi=linux-x86_64 entry=syscall return=suspended nr=1 write fd=3 requested=16 origin=cpl3"
    "SLOPOS-VFS: process write complete pid=1 fd=3 inode=31 offset=123 requested=16 bytes=16 async=true flushed=true"
    "SLOPOS-VFS: process read complete pid=1 fd=3 inode=31 offset=123 requested=16 bytes=16 async=true"
    ...
    "SLOPOS-PROCESS: pid=1 state=exited status=0 syscalls=15 retained=true kernel_return=true"
