...
required_markers=(
    ...
    "SLOPOS-EXT4: multiblock file valid inode=23 inode_group=1 bytes=6144 logical_blocks=2 path=/usr/share/slopos/multiblock.bin"
    "SLOPOS-EXT4: depth-one extent valid inode=20 leaf_block=85 logical_block=8 bytes=4096 metadata_checksum=valid path=/usr/share/slopos/deep-extent.bin"
    "SLOPOS-EXT4: sparse read valid inode=20 logical_block=7 zero_bytes=4096"
    "SLOPOS-EXT4: cross-block directory valid directory_inode=21 directory_blocks=2 entry_block=1 target_inode=22 target_bytes=40 metadata_checksums=valid path=/usr/share/slopos/large-directory/tail-29"
    "SLOPOS-FS: block cache entries=8 hits=35 misses=33 batched_pairs=1"
    "SLOPOS-VIRTIO: bounded block sequence complete requests=34 max_in_flight=2 interrupts=33 queue_interrupts=33"
