...
required_markers=(
    ...
    "SLOPOS-VFS: executable loaded path=/sbin/slop-shell inode=24 bytes=26560 blocks=7 matches_boot=not-required role=desktop-service"
    ...
    "SLOPOS-EXT4: multiblock file valid inode=30 inode_group=1 bytes=6144 logical_blocks=2 format=P3 comment_padding=valid path=/usr/share/slopos/vfs-wallpaper.ppm"
    ...
    "SLOPOS-VFS: writable descriptor valid fd=3 inode=31 physical_block=116 offset=123 bytes=73 writes=2 flushes=2 cache_invalidations=2 restored=true path=/usr/share/slopos/write-probe.bin"
    ...
    "SLOPOS-EXT4: journal records staged sequence=1 target_block=116 descriptor_block=32802 data_block=32803 commit_block=32804 writes=6 flushes=3 verified=true restored=true active=false"
    ...
    "SLOPOS-EXT4: active journal transaction valid sequence=1 target_block=116 recovery=true start=1 records=descriptor/data/commit replayable_readback=true home_checkpointed=true next_sequence=2 test_sequence_rewound=true restored=true writes=13 flushes=10"
    ...
    "SLOPOS-PROCESS: pid=2 parent=1 source=vfs path=/sbin/slop-shell argv1=--session format=elf64 entry=0x40000000 segments=1 file_bytes=26560 load_bytes=2528 memory_bytes=2528"
    ...
    "SLOPOS-VFS: process read complete pid=2 fd=3 inode=17 offset=0 requested=256 bytes=172 user_pages=1 cross_page=false async=true"
    "SLOPOS-DESKTOP-SERVICE: policy submitted pid=2 generation=1 protocol=1 capabilities=waybar-provider/swww-policy cpu=0 memory=36 wallpaper=aurora"
    ...
    "SLOPOS-SHELL: config loaded niri_workspaces=3 named=2 binds=56 rules=1 active_columns=2 gaps=16 default_width=50% center=never waybar_position=top height=40 spacing=10 modules=1/1/4 module_configs=6 css_rules=12"
    ...
    "SLOPOS-DESKTOP-SERVICE: policy applied generation=1 owner_pid=2 capabilities=waybar-provider/swww-policy cpu=0 memory=36 wallpaper=/usr/share/backgrounds/slopos-aurora.ppm renderer=kernel-mechanism"
