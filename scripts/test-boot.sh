...
required_markers=(
    ...
    "SLOPOS-EXT4: depth-one extent valid inode=27 leaf_block=92 logical_block=8 bytes=4096 metadata_checksum=valid path=/usr/share/slopos/deep-extent.bin"
    ...
    "SLOPOS-VFS: writable descriptor valid fd=3 inode=31 physical_block=105 offset=123 bytes=73 writes=2 flushes=2 cache_invalidations=2 restored=true path=/usr/share/slopos/write-probe.bin"
    ...
    "SLOPOS-EXT4: journal records staged sequence=1 target_block=105 descriptor_block=32802 data_block=32803 commit_block=32804 writes=6 flushes=3 verified=true restored=true active=false"
    ...
    "SLOPOS-EXT4: active journal transaction valid sequence=1 target_block=105 recovery=true start=1 records=descriptor/data/commit replayable_readback=true home_checkpointed=true next_sequence=2 test_sequence_rewound=true restored=true writes=13 flushes=10"
    ...
    "SLOPOS-EXT4: fd append journal transactions valid fd=3 inode=31 block=106 bitmap_block=33 group_descriptor_block=1 inode_table_block=38 append_bytes=4096 size=4096/8192/4096 extent_blocks=1/2/1 checksums=superblock/group/bitmap/inode/data transactions=2 sequences=1/2 final_sequence=3 test_sequence_rewound=true restored=true"
    "SLOPOS-EXT4: VFS create journal transactions valid fd=3 inode=32 parent_inode=26 inode_bitmap_block=36 group_descriptor_block=1 inode_table_block=38 directory_block=90 free_inodes=1/0/1 size=0 access=readwrite checksums=superblock/group/bitmap/inode/directory transactions=2 sequences=1/2 final_sequence=3 test_sequence_rewound=true restored=true path=/usr/share/slopos/create-probe"
    "SLOPOS-FS: block cache entries=8 hits=134 misses=85 batched_pairs=1 invalidations=16"
    "SLOPOS-VIRTIO: bounded block sequence complete requests=463 max_in_flight=2 interrupts=462 queue_interrupts=462"
