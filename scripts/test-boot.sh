...
required_markers=(
    ...
    "SLOPOS-EXT4: fd append journal transactions valid fd=3 inode=25 block=99 bitmap_block=33 group_descriptor_block=1 inode_table_block=38 append_bytes=4096 size=4096/8192/4096 extent_blocks=1/2/1 checksums=superblock/group/bitmap/inode/data transactions=2 sequences=1/2 final_sequence=3 test_sequence_rewound=true restored=true"
    "SLOPOS-EXT4: VFS create journal transactions valid fd=3 inode=26 parent_inode=20 inode_bitmap_block=36 group_descriptor_block=1 inode_table_block=38 directory_block=83 free_inodes=7/6/7 size=0 access=readwrite checksums=superblock/group/bitmap/inode/directory transactions=2 sequences=1/2 final_sequence=3 test_sequence_rewound=true restored=true path=/usr/share/slopos/create-probe"
    ...
    "SLOPOS-SHELL: config loaded niri_workspaces=3 named=2 binds=7 rules=1 active_columns=2 gaps=16 default_width=50% center=never waybar_position=top height=40 spacing=10 modules=1/1/4"
